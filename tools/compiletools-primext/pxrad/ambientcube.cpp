...
template<typename T>
static void VectorCastAndCopy(const vec3_t src, T (&dest)[3])
{
	dest[0] = static_cast<T>(src[0]);
	dest[1] = static_cast<T>(src[1]);
	dest[2] = static_cast<T>(src[2]);
}

template<typename T>
static void VectorCastAndCopy(const T (&src)[3], vec3_t dest)
{
	dest[0] = static_cast<vec_t>(src[0]);
	dest[1] = static_cast<vec_t>(src[1]);
	dest[2] = static_cast<vec_t>(src[2]);
}

static inline float BSPPlaneDiff(const vec3_t point, const dplane_t* plane)
{
...
static vec_t
LightAngle(const dworldlight_t* wl, const vec3_t lnormal, const vec3_t snormal, const vec3_t delta, float dist)
{
	vec_t dot, dot2;
...
static void AddEmitSurfaceLights(int threadnum, const vec3_t vStart, vec3_t lightBoxColor[6])
{
	...
	for ( int iLight = 0; iLight < g_numworldlights; iLight++ )
	{
		...
		VectorCastAndCopy(wl->origin, wlOrigin);  // short to float
...
*/
static bool
R_RecursiveLightPoint(const int nodenum, float p1f, float p2f, const vec3_t start, const vec3_t end, lightpoint_t* info)
{
	vec3_t mid;
	...
	// calculate mid point
	float front = BSPPlaneDiff(start, plane);
	float back = BSPPlaneDiff(end, plane);
...
static void
ComputeLightmapColorFromPoint(lightpoint_t* info, dworldlight_t* pSkylight, float scale, vec3_t radcolor, bool average)
{
	vec3_t color;
...
//-----------------------------------------------------------------------------
static void CalcRayAmbientLighting(
	const vec3_t vStart,
	const vec3_t vEnd,
	dworldlight_t* pSkyLight,
	float tanTheta,
	vec3_t radcolor)
{
	lightpoint_t info;
...
void GenerateLeafSamplePosition(
	int leafIndex,
	ambientlocallist_t* list,
	const leafplanes_t* leafPlanes,
	vec3_t samplePosition)
{
	dleaf_t* pLeaf = g_dleafs + leafIndex;
	vec3_t vCenter, leafMins, leafMaxs;

	VectorCastAndCopy(pLeaf->mins, leafMins);
	VectorCastAndCopy(pLeaf->maxs, leafMaxs);
	bool bValid = false;
	...
	for ( int i = 0; i < 1024 && !bValid; i++ )
	{
		...
		{
			float d = BSPPlaneDiff(samplePosition, leafPlanes->planes + k);
...
void ComputeAmbientForLeaf(int threadnum, int leafID, ambientlocallist_t* list)
{
	...
	// this heuristic tries to generate at least one sample per volume (chosen to be similar to the size of a player) in
	// the space
...
void ComputeLeafAmbientLighting(void)
{
	...
	for ( i = 0; i < g_numworldlights; i++ )
	{
		...
		if ( IsLeafAmbientSurfaceLight(wl) )
			...
			ClearBits(wl->flags, DWL_FLAGS_INAMBIENTCUBE);
	...
	MsgDev(
		D_REPORT,
		"%d of %d (%d%% of) surface lights went in leaf ambient cubes.\n",
		nInAmbientCube,
		nSurfaceLights,
