...
int GetVisCache(int lastoffset, int offset, byte* pvs)
{
	...
	else if ( offset != lastoffset )
	{
		if ( offset == -1 )
			...
			DecompressVis(&g_dvisdata[offset], pvs);
		lastoffset = offset;
	}
...
// =====================================================================================
vec_t CalcSightArea(
	const vec3_t receiver_origin,
	const vec3_t receiver_normal,
	const winding_t* emitter_winding,
	int skylevel)
{
	...
	for ( i = 0, pnormal = g_skynormals[skylevel], psize = g_skynormalsizes[skylevel]; i < g_numskynormals[skylevel];
		  i++, pnormal++, psize++ )
	{
		...
		for ( j = 0; j < g_num_texlights; j++ )
		{
			...
			if ( !Q_strcmp(tl->name, ep->key) )
			{
				...
				else if ( tl->value[0] != r || tl->value[1] != g || tl->value[2] != b )
				{
					MsgDev(
						D_REPORT,
						"overriding '%s' from '%s' with info_texlights map entity!\n",
						tl->name,
						tl->filename);
				}
				else
				{
					MsgDev(
						D_WARN,
						"redundant '%s' def in '%s' AND info_texlights map entity!\n",
						tl->name,
						tl->filename);
				}
				break;
...
int ParseSurfaceLights(entity_t* mapent)
{
	...
	if ( CheckKey(mapent, "_light") )
		...
		pLight = ValueForKey(mapent, "light");
	...
	if ( argCnt <= 1 )
	{
		...
		if ( CheckKey(mapent, "_color") )
			...
			pColor = ValueForKey(mapent, "color");
	...
	if ( g_texlights[j].fade <= 0.0 )
		g_texlights[j].fade = 1.0;
...
static bool PlacePatchInside(patch_t* patch)
{
	...
	if ( HuntForWorld(point, face_offset, plane, 4, 0.2, offset) ||
		 HuntForWorld(point, face_offset, plane, 4, 0.8, offset) )
	{
		VectorSubtract(point, center, v);
		dist = VectorLength(v);
		pointsfound++;
...
static void SubdividePatch(patch_t* patch)
{
	...
	// ATTENTION: We let SortPatches relink all the ->next correctly! instead of doing it here too which is somewhat
	// complicated
}
...
static bool getEmitMode(const patch_t* patch)
{
	...
	if ( !VectorIsNull(patch->reflectivity) )
		...
		value = VectorAvg(patch->baselight);
...
static void MakeTransfers(int threadnum)
{
	...
	while ( 1 )
	{
		...
		for ( j = 0, patch2 = g_patches; j < g_num_patches; j++, patch2++ )
		{
			...
			if ( DotProduct(patch1->origin, plane2->normal) <=
				 PatchPlaneDist(patch2) + ON_EPSILON - patch1->emitter_range )
				continue;
			...
			if ( dist < patch1->emitter_range - ON_EPSILON )
				...
				VectorCopy(patch1->origin, origin1);
		...
		for ( j = 0; j < count; j++ )
		{
			...
			if ( dist < patch2->emitter_range - ON_EPSILON )
			{
				...
				trans =
					frac * trans + (1 - frac) * (sightarea / patch2->area);  // because later we will multiply this back
			}
	...
	while ( 1 )
	{
		...
		for ( k = 0; k < iIndex; k++, tIndex++ )
		{
			...
			for ( l = 0; l < size; l++, tData++, patchnum++ )
			{
				...
				for ( int emitstyle = 0; emitstyle < MAXLIGHTMAPS && emitpatch->totalstyle[emitstyle] != 255;
					  emitstyle++ )
				{
...
static void PrintRadSettings(void)
{
	...
	Msg("compatibility mode    [ %7s ] [ %7s ]\n",
		CompatibilityMode::GetString(g_compatibility_mode),
		CompatibilityMode::GetString(DEFAULT_COMPAT_MODE));
...
int main(int argc, char** argv)
{
	...
	W_SetAllowWadFilesInPacks(true);
	...
	if ( !FS_FileExists(level_lights, false) )
		level_lights[0] = '\0';
