...
struct localtrian_t
{
	...
	CUtlVector<int> neighborfaces;  // including the face itself

	CUtlVector<Wedge> sortedwedges;  // in clockwise order (same as Winding)
	CUtlVector<HullPoint> sortedhullpoints;  // in clockwise order (same as Winding)
};
...
struct facetriangulation_t
{
	...
	int facenum;
	CUtlVector<int> neighbors;  // including the face itself
	CUtlVector<Wall> walls;
	CUtlVector<localtrian_t*> localtriangulations;
	CUtlVector<int> usedpatches;
};
...
// If the surface formed by the face and its neighbor faces is not flat, the surface should be unfolded onto the face
// plane CalcAdaptedSpot calculates the coordinate of the unfolded spot on the face plane from the original position on
// the surface CalcAdaptedSpot(center) = {0,0,0} CalcAdaptedSpot(position on the face plane) = position - center Param
// position: must include g_face_offset
...
static bool CalcWeight(const localtrian_t* lt, const vec3_t spot, vec_t* weight_out)
{
	...
	{
		istoofar =
			false;  // if we change this "false" to "true", we will see many places turned "green" in "-drawlerp" mode
...
static void ApplyInterpolation(
	const interpolation_t* interp,
	int numstyles,
	const int* styles,
	vec3_t* outs,
	vec3_t* outs_dir = NULL)
{
	int i, j;
...
// if one of the angles in a triangle exceeds this threshold, the most distant point will be removed or the triangle
// will break into convex-type wedge.
static void GatherPatches(localtrian_t* lt, const facetriangulation_t* facetrian)
{
	int patchnum2;
	int facenum2;
	const patch_t* patch2;
	CUtlVector<localtrian_t::Wedge> points;
	CUtlVector<pair_t> angles;
...
static void PurgePatches(localtrian_t* lt)
{
	...
	for ( i = 0; i < points.Count(); i++ )
	{
		...
		while ( next[cur] != cur && valid[next[cur]] != 2 )
		{
			...
			if ( fabs(angle) <= DEG2RAD(1.0) ||
				 GetAngleDiff(angle, 0) <= M_PI + NORMAL_EPSILON &&
...
static void PlaceHullPoints(localtrian_t* lt)
{
	...
	CUtlVector<localtrian_t::HullPoint> spots;
	CUtlVector<pair_t> angles;
	const localtrian_t::Wedge* w;
	const localtrian_t::Wedge* wnext;
	CUtlVector<localtrian_t::HullPoint> arc_spots;
	CUtlVector<vec_t> arc_angles;
	CUtlVector<int> next;
	CUtlVector<int> prev;
	vec_t frac;
	vec_t len;
