...
#include "CompileTools/engine/alias.h"
...
void* LoadAllSkins(daliashdr_t* phdr, daliasskintype_t* pskintype)
{
	...
	for ( i = 0; i < phdr->numskins; i++ )
	{
		if ( pskintype->type == ALIAS_SKIN_SINGLE )
			...
			pskintype = (daliasskintype_t*)LoadGroupSkin(pskintype, size);
	}
...
void LoadAlias(entity_t* ent, void* extradata, int fileLength, int flags)
{
	...
	for ( i = 0; i < phdr->numframes; i++ )
	{
		...
		if ( frametype == ALIAS_SINGLE )
			...
			pframetype = (daliasframetype_t*)LoadAliasGroup(phdr, pframetype + 1);
	}

	size_t memsize =
		sizeof(tmesh_t) + (sizeof(tface_t) * phdr->numtris) + sizeof(timage_t) + sizeof(tvert_t) * phdr->numverts;
	...
	if ( ent->cache )
		Mem_Free(ent->cache);  // throw previous instance
	...
	for ( i = 0; i < phdr->numverts; i++ )
	{
		...
		s = static_cast<float>(st->s);
		t = static_cast<float>(st->t);
	...
	MsgDev(
		D_REPORT,
		"%s: build time %g secs, size %s\n",
		modname,
		I_FloatTime() - start,
		Q_memprint(static_cast<float>(memsize)));
}
