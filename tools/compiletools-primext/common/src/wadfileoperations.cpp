...
static qboolean g_AllowWadFilesInPacks = false;
...
{
	return g_AllowWadFilesInPacks;
}

void W_SetAllowWadFilesInPacks(qboolean allow)
{
	g_AllowWadFilesInPacks = allow;
}

bool W_HasFSPointer(wfile_t* wad)
{
	return wad && wad->fsptr;
}

static size_t LocalSeek(wfile_t& wad, size_t offset, int from)
{
	if ( wad.fsptr )
	{
		return FS_Seek(wad.fsptr, offset, from);
	}
	else
	{
		return PlatformLib_LSeek(wad.fdhandle, offset, from);
	}
}

static size_t LocalTell(wfile_t& wad)
{
	if ( wad.fsptr )
	{
		return FS_Tell(wad.fsptr);
	}
	else
	{
		return PlatformLib_LSeek(wad.fdhandle, 0, SEEK_CUR);
	}
}

static size_t LocalRead(wfile_t& wad, void* buffer, size_t bufferSize)
{
	if ( wad.fsptr )
	{
		return FS_Read(wad.fsptr, buffer, bufferSize);
	}
	else
	{
		return PlatformLib_Read(wad.fdhandle, buffer, (unsigned int)bufferSize);
	}
}

static size_t LocalWrite(wfile_t& wad, const void* buffer, size_t bufferSize)
{
	if ( wad.fsptr )
	{
		return FS_Write(wad.fsptr, buffer, bufferSize);
	}
	else
	{
		return PlatformLib_Write(wad.fdhandle, buffer, (unsigned int)bufferSize);
	}
}

static void LocalClose(wfile_t& wad)
{
	if ( wad.fsptr )
	{
		FS_Close(wad.fsptr);
	...
	{
		PlatformLib_Close(wad.fdhandle);
	}
}
...
int W_GetHandle(wfile_t* wad)
{
	...
	if ( wad->fsptr )
	{
		// Takes precedence, and doesn't apply here
		return -1;
	}
	...
	while ( left <= right )
	{
		...
		if ( !diff )
		{
			if ( wad->lumps[middle].img_type > newlump->img_type )
				...
				MsgDev(D_WARN, "Wad %s contains the file %s several times\n", wad->filename, name);
		}
...
bool W_WriteLump(wfile_t* wad, dlumpinfo_t* lump, const void* data, size_t datasize)
{
	...
	if ( LocalWrite(*wad, data, datasize) == datasize )
	{
		return true;
	}
...
===========
*/
wfile_t* W_Open(const char* filename, const char* mode, int* error, qboolean ext_path)
{
	dwadinfo_t header;
	...
	if ( g_AllowWadFilesInPacks )
	{
		// NOTE: FS_Open is load wad file from the first pak in the list (while ext_path is false)
		if ( ext_path )
		{
			wad->fsptr = FS_Open(filename, "rb", false);
		}
		else
		{
			wad->fsptr = FS_Open(COM_FileWithoutPath(filename), "rb", false);
		}
	}
	else
	{
		wad->fdhandle = PlatformLib_OpenWithPermissions(filename, mod | opt, 0666);
	}
	...
	LocalSeek(*wad, 0, SEEK_END);
	size_t wadsize = LocalTell(*wad);
	LocalSeek(*wad, 0, SEEK_SET);
	...
	if ( mod == O_WRONLY || !wadsize )
	{
		...
		LocalWrite(*wad, &hdr, sizeof(hdr));
		LocalWrite(*wad, comment, Q_strlen(comment) + 1);
		wad->infotableofs = static_cast<int>(LocalTell(*wad));
	}
	else if ( mod == O_RDWR || mod == O_RDONLY )
	{
		...
		if ( LocalRead(*wad, &header, sizeof(dwadinfo_t)) != sizeof(dwadinfo_t) )
		{
		...
		if ( LocalRead(*wad, srclumps, lat_size) != lat_size )
		{
		...
		for ( int i = 0; i < lumpcount; i++ )
		{
			...
			if ( k )
			{
				name[Q_strlen(name) - k] = '!';  // quake1 issues (can't save images that contain '*' symbol)
			}
		...
		{
			LocalSeek(*wad, wad->infotableofs, SEEK_SET);
		}
	}
...
void W_Close(wfile_t* wad)
{
	...
	if ( W_CheckHandle(wad) && (wad->mode == O_APPEND || wad->mode == O_WRONLY) )
	{
		...
		// write the lumpinfo
		size_t ofs = LocalTell(*wad);
		LocalWrite(*wad, wad->lumps, wad->numlumps * sizeof(dlumpinfo_t));
...
int W_SaveLump(wfile_t* wad, const char* lump, const void* data, size_t datasize, int type, char attribs)
{
		...
		switch ( replace_level )
		{
			...
			case REP_FORCE:
			{
				...
				if ( datasize != (size_t)find->size )
				{
					MsgDev(
						D_ERROR,
						"W_ReplaceLump: %s [%s] should be [%s]\n",
						lumpname,
						Q_memprint(static_cast<float>(datasize)),
						Q_memprint(static_cast<float>(find->size)));
					return -1;
				}
	...
	newlump.filepos = static_cast<int>(LocalTell(*wad));
	newlump.attribs = attribs;
	newlump.img_type = hint;
	newlump.type = static_cast<int8_t>(type);
	...
	MsgDev(D_REPORT, "W_SaveLump: %s, size %s\n", newlump.name, Q_memprint(static_cast<float>(newlump.disksize)));
...
void W_SearchForFile(wfile_t* wad, const char* pattern, stringlist_t* resultlist)
{
	...
	COM_FileBase(wad->filename, temp2, sizeof(temp2));
		...
		while ( temp[0] )
		{
			if ( matchpattern(temp, wadpattern, true) )
			{
				...
				if ( resultlistindex == resultlist->numstrings )
				{
					...
					Q_snprintf(temp2, sizeof(temp2), "%s/%s.%s", wadfolder, temp, W_ExtFromType(wad->lumps[i].type));
