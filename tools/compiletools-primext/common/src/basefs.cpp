...
static dpackfile_t* FS_AddFileToPack(const char* name, pack_t* pack, int offset, int size)
{
	...
	while ( left <= right )
	{
		...
		if ( !diff )
		{
			MsgDev(D_WARN, "Package %s contains the file %s several times\n", pack->filename, name);
		}
...
pack_t* FS_LoadPackPAK(const char* packfile, int* error)
{
	...
	packhandle = PlatformLib_OpenWithPermissions(packfile, O_RDONLY | O_BINARY, 0666);
	...
	PlatformLib_Read(packhandle, (void*)&header, sizeof(header));
	...
	if ( header.dirlen != PlatformLib_Read(packhandle, (void*)info, header.dirlen) )
	{
...
static bool FS_AddWad_Fullpath(const char* wadfile, bool* already_loaded, int flags)
{
	...
	if ( !Q_stricmp(ext, "wad") )
		wad = W_Open(wadfile, "rb", &errorcode, fs_ext_path);
	else
		MsgDev(D_ERROR, "\"%s\" doesn't have a wad extension\n", wadfile);
...
static bool FS_AddPak_Fullpath(const char* pakfile, bool* already_loaded, int flags)
{
	...
	if ( !Q_stricmp(ext, "pak") )
		...
		MsgDev(D_ERROR, "\"%s\" does not have a pack extension\n", pakfile);
...
void FS_ReadGameInfo(const char* gamedir)
{
	...
	if ( FS_FileTime(liblist, false) > FS_FileTime(gameinfo, false) )
		...
		FS_ParseGameInfo(gameinfo, gamedir);
}
...
{
	char workdir[MAX_SYSPATH];
	char mapdir[MAX_SYSPATH];
	char hullfile[MAX_SYSPATH];
	char mapfile[MAX_SYSPATH];
	char* pathend;
	...
	if ( source[0] == '.' && (source[1] == '/' || source[1] == '\\') )
		...
		Q_snprintf(mapdir, sizeof(mapdir), "%s.map", source);
	...
	size_t length = Q_strlen(fs_rootdir);
...
static file_t* FS_SysOpen(const char* filepath, const char* mode)
{
	...
	file->real_length = PlatformLib_LSeek(file->handle, 0, SEEK_END);
	...
	for ( search = fs_searchpaths; search; search = search->next )
	{
		...
		else if ( search->wad )
		{
			...
			if ( lump )
			{
				if ( index )
				{
					*index = (int)(lump - search->wad->lumps);
				}
...
file_t* FS_Open(const char* filepath, const char* mode, bool gamedironly)
{
	...
	if ( mode[0] == 'w' || mode[0] == 'a' || mode[0] == 'e' || Q_strchr(mode, '+') )
	{
		...
		Q_snprintf(real_path, sizeof(real_path), "%s", filepath);
...
size_t FS_Read(file_t* file, void* buffer, size_t buffersize)
{
	...
	if ( buffersize > sizeof(file->buff) / 2 )
	{
		...
		nb = PlatformLib_Read(file->handle, &((byte*)buffer)[done], (unsigned int)count);
	...
	else
	{
		...
		PlatformLib_LSeek(file->handle, file->offset + file->position, SEEK_SET);
		nb = PlatformLib_Read(file->handle, file->buff, (unsigned int)count);
...
int FS_Seek(file_t* file, int64_t offset, int whence)
{
	...
	if ( offset < 0 || (size_t)offset > file->real_length )
	{
		return -1;
	}

	// if we have the data in our read buffer, we don't need to actually seek
	if ( file->position - file->buff_len <= (size_t)offset && (size_t)offset <= file->position )
	{
...
int FS_Write(file_t* file, const void* data, size_t datasize)
{
	...
	result = PlatformLib_Write(file->handle, data, (unsigned int)datasize);
	file->position = PlatformLib_LSeek(file->handle, 0, SEEK_CUR);
...
search_t* FS_Search(const char* pattern, int caseinsensitive, int gamedironly)
{
	...
	for ( searchpath = fs_searchpaths; searchpath; searchpath = searchpath->next )
	{
		...
		else
		{
			...
			Q_snprintf(netpath, sizeof(netpath), "%s%s", searchpath->filename, basepath);
			stringlistinit(&dirlist);
