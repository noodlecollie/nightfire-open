...
void CheckHullFile(const char* filename)
{
	...
	// open up hull file
	FILE* f = PlatformLib_FOpen(filename, "r");
	...
	for ( int i = 0; i < MAX_MAP_HULLS; i++ )
	{
		...
		argCnt = PlatformLib_SScanF(scan, "( %f %f %f ) ( %f %f %f ) ", &x1, &y1, &z1, &x2, &y2, &z2);
...
void DecompressVis(byte* in, byte* decompressed)
{
	...
	do
	{
		...
		c = in[1];
		if ( !c )
			COM_FatalError("DecompressVis: 0 repeat\n");
...
static int CopyExtraLump(int lump, void* dest, int size, const dheader_t* inHeader)
{
	dextrahdr_t* extrahdr = (dextrahdr_t*)((byte*)inHeader + sizeof(dheader_t));
	...
	memcpy(dest, (byte*)inHeader + ofs, length);
...
void LoadBSPFile(const char* filename)
{
	...
	if ( !header )
		COM_FatalError("couldn't load: %s\n", filename);
...
void AddLump(int lumpnum, void* data, int len)
{
	...
	lump->fileofs = (int32_t)PlatformLib_LSeek(wadfile, 0, SEEK_CUR);
	lump->filelen = len;
	SafeWrite(wadfile, data, (len + 3) & ~3);
}

static void AddExtraLump(int lumpnum, void* data, int len, dextrahdr_t* inHeader)
{
	dlump_t* lump = &inHeader->lumps[lumpnum];
	lump->fileofs = (int32_t)PlatformLib_LSeek(wadfile, 0, SEEK_CUR);
	lump->filelen = len;
	SafeWrite(wadfile, data, (len + 3) & ~3);
}
...
void WriteBSPFile(const char* filename)
{
	...
	PlatformLib_LSeek(wadfile, 0, SEEK_SET);
	SafeWrite(wadfile, header, sizeof(dheader_t));
	SafeWrite(wadfile, extrahdr, sizeof(dextrahdr_t));

	PlatformLib_Close(wadfile);

	if ( g_dvislightdata )
	{
		Mem_Free(g_dvislightdata);
	}
	...
	if ( g_dflightdata )
	{
		Mem_Free(g_dflightdata);
	}

	if ( g_dnormaldata )
	{
		Mem_Free(g_dnormaldata);
	}
...
static void DoAllocBlock(lightmapblock_t* blocks, int w, int h)
{
	...
	for ( lightmapblock_t* block = blocks; block; block = block->next )
	{
		...
		if ( !block->next )
		{
			...
			block->next = (lightmapblock_t*)Mem_Alloc(sizeof(lightmapblock_t));
...
{
	lightmapblock_t* blocks = (lightmapblock_t*)Mem_Alloc(sizeof(lightmapblock_t));
	ASSERT(blocks != NULL);
	...
	for ( int k = 0; k < g_numfaces; k++ )
	{
		...
		if ( !Q_strncmp(texname, "sky", 3)  // sky, no lightmap allocation
			 || !Q_strncmp(texname, "!", 1) || !Q_strnicmp(texname, "water", 5) ||
			 !Q_strnicmp(texname, "laser", 5)  // water, no lightmap allocation
		...
		if ( extents[0] < 0 || extents[1] < 0 || extents[0] > Q_max(512, MAX_SURFACE_EXTENT * TEXTURE_STEP) ||
			 extents[1] > Q_max(512, MAX_SURFACE_EXTENT * TEXTURE_STEP) )
		// the default restriction from the engine is 512, but place 'max (512, MAX_SURFACE_EXTENT * TEXTURE_STEP)' here
		// in case someone raise the limit
		{
			MsgDev(
				D_WARN,
...
void PrintBSPFileSizes(bool goldsrcCompatibility)
{
	...
	totalmemory +=
		ArrayUsage("models", g_nummodels, goldsrcCompatibility ? 400 : ENTRIES(g_dmodels), ENTRYSIZE(g_dmodels));
	totalmemory +=
		ArrayUsage("planes", g_numplanes, goldsrcCompatibility ? 32767 : ENTRIES(g_dplanes), ENTRYSIZE(g_dplanes));
	...
	totalmemory +=
		ArrayUsage("texinfos", g_numtexinfo, goldsrcCompatibility ? 8192 : ENTRIES(g_texinfo), ENTRYSIZE(g_texinfo));
	...
	totalmemory +=
		ArrayUsage("leaves", g_numleafs, goldsrcCompatibility ? 8192 : ENTRIES(g_dleafs), ENTRYSIZE(g_dleafs));
	totalmemory += ArrayUsage("marksurfaces", g_nummarksurfaces, ENTRIES(g_dmarksurfaces), ENTRYSIZE(g_dmarksurfaces));
	totalmemory += ArrayUsage(
		"surfedges",
		g_numsurfedges,
		goldsrcCompatibility ? 512000 : ENTRIES(g_dsurfedges),
		ENTRYSIZE(g_dsurfedges));
	totalmemory +=
		ArrayUsage("edges", g_numedges, goldsrcCompatibility ? 256000 : ENTRIES(g_dedges), ENTRYSIZE(g_dedges));
	...
	if ( g_found_extradata )
	{
		...
		totalmemory +=
			ArrayUsage("direct lights", g_numworldlights, ENTRIES(g_dworldlights), ENTRYSIZE(g_dworldlights));
	...
	Msg("=== Total BSP file data space used: %s ===\n", Q_memprint(static_cast<float>(totalmemory)));
}
...
void LightMatrixFromTexMatrix(const dtexinfo_t* tx, float lmvecs[2][4])
{
	...
	VectorNormalize2(lmvecs[0], lmvecs[0]);
	VectorNormalize2(lmvecs[1], lmvecs[1]);
...
int MapPlaneTypeForNormal(const vec3_t normal)
{
	...
	if ( (ax > 1.0f - DIR_EPSILON) && (ay < DIR_EPSILON) && (az < DIR_EPSILON) )
	{
		return PLANE_X;
	}

	if ( (ay > 1.0f - DIR_EPSILON) && (az < DIR_EPSILON) && (ax < DIR_EPSILON) )
	{
		return PLANE_Y;
	}

	if ( (az > 1.0f - DIR_EPSILON) && (ax < DIR_EPSILON) && (ay < DIR_EPSILON) )
	{
		return PLANE_Z;
	}
...
int SnapNormal(vec3_t normal)
{
	...
	if ( type < PLANE_NONAXIAL )
	{
		for ( int i = 0; i < 3; i++ )
		{
			if ( i == type )
			{
				normal[i] = normal[i] > 0.0f ? 1.0f : -1.0f;
			}
			else
			{
				normal[i] = 0;
	...
	else
	{
		for ( int i = 0; i < 3; i++ )
		{
			if ( fabsf(fabsf(normal[i]) - 0.707106f) < DIR_EPSILON )
			{
				normal[i] = normal[i] > 0 ? 0.707106f : -0.707106f;
				renormalize = true;
			}
		}
	}
...
bool ParseEntity(void)
{
	...
	if ( Q_strcmp(token, "{") )
	{
		if ( g_numentities == 0 )
			...
			return false;  // probably entity string is broken at the end
	}
...
void UnparseEntities(void)
{
	...
	char* buf = g_dentdata;
	char* end = buf;
	char* const bufend = buf + sizeof(g_dentdata);
	*end = 0;
...
{
	return Q_atoi(ValueForKey(ent, key));
}

bool BoolForKey(entity_t* ent, const char* key)
{
	if ( Q_atoi(ValueForKey(ent, key)) )
	{
		return true;
	}
...
int GetVectorForKey(entity_t* ent, const char* key, vec3_t vec)
{
	...
	count = PlatformLib_SScanF(k, "%lf %lf %lf", &v1, &v2, &v3);
...
void SetVectorForKey(entity_t* ent, const char* key, vec3_t vec, bool precise)
{
	...
	if ( precise )
		Q_snprintf(string, sizeof(string), "%.2f %.2f %.2f", vec[0], vec[1], vec[2]);
	else
		Q_snprintf(string, sizeof(string), "%i %i %i", (int)vec[0], (int)vec[1], (int)vec[2]);
