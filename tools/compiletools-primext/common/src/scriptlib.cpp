...
void ParseFromMemory(const char* buffer, size_t size)
{
	...
	Q_strcpy(script->filename, sizeof(script->filename), "memory buffer");
...
skipspace:
		 ...
		 (*script->script_p == '/' && *((script->script_p) + 1) == '/') )
	{
		...
		if ( script->script_p[1] == 'D' && script->script_p[2] == 'X' )
			g_DXspecial = Q_atoi(script->script_p + 3);  // DX#####"-style comment
...
bool GetTokenAppend(char* buffer, size_t bufferSize, bool crossline)
{
	bool result;
	int i;
...
bool TokenAvailable(void)
{
	...
	if ( *search_p == ';' || *search_p == '#' || (*search_p == '/' && *((search_p) + 1) == '/') )
	{
		...
		if ( *search_p == 'D' && *((search_p) + 1) == 'X' )
			g_DXspecial = Q_atoi(((search_p) + 2));  // DX#####"-style comment
...
void Parse1DMatrixAppend(char* buffer, size_t bufferSize, int x, vec_t* m)
{
	if ( !GetTokenAppend(buffer, bufferSize, true) || Q_strcmp(token, "(") )
	{
	...
	for ( int i = 0; i < x; i++ )
	{
		if ( !GetTokenAppend(buffer, bufferSize, false) )
		{
