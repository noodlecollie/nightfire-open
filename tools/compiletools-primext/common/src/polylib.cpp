...
winding_t* BaseWindingForPlane(const vec3_t normal, vec_t dist)
{
	...
	if ( x == -1 )
		COM_FatalError("BaseWindingForPlane: no axis found\n");
...
void CheckWindingEpsilon(winding_t* w, vec_t epsilon, bool show_warnings)
{
	...
	area = WindingArea(w);
	if ( area < MICROVOLUME )
		COM_FatalError("CheckWinding: %f area\n", area);
	...
	for ( i = 0; i < w->numpoints; i++ )
	{
		...
		for ( j = 0; j < w->numpoints; j++ )
		{
			...
			if ( d > edgedist )
				COM_FatalError("CheckWinding: non-convex\n");
		}
	}
}
...
void WindingSnapPoint(const winding_t* w, const vec3_t planenormal, vec3_t point)
{
	...
	vec3_t bestpoint;
	vec_t bestdist = std::numeric_limits<float>::max();
	bool in = true;
...
vec_t WindingSnapPointEpsilon(const winding_t* w, const vec3_t planenormal, vec3_t point, vec_t width, vec_t maxmove)
{
	...
	for ( x = 0; x < w->numpoints; x++ )
	{
		...
		VectorScale(planes[numplanes].normal, 1.0f / len, planes[numplanes].normal);
	...
	// apply binary search method for 5 iterations to find the maximal distance that the point can be kept away from all
	// the edges
	for ( pass = 0; pass < 5; pass++ )
	{
		winding_t* neww = CopyWinding(w);
		vec3_t newpoint;
		vec3_t localplanenormal;
		bool failed = true;

		for ( x = 0; x < numplanes && neww != NULL; x++ )
		{
			VectorCopy(planes[x].normal, localplanenormal);
			ChopWindingInPlace(&neww, localplanenormal, planes[x].dist + newwidth, ON_EPSILON, false);
		}

		if ( neww && neww->numpoints > 0 )
		{
			VectorCopy(point, newpoint);
			WindingSnapPoint(neww, localplanenormal, newpoint);
		...
		if ( !failed )
		{
			...
			newwidth += width * powf(0.5f, static_cast<float>(pass + 1));
		}
		else
		{
			newwidth -= width * powf(0.5f, static_cast<float>(pass + 1));
