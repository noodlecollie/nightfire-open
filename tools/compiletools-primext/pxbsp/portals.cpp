...
void RemovePortalFromNode(portal_t* portal, node_t* l)
{
	...
	while ( 1 )
	{
		...
		if ( !t )
		{
			COM_FatalError("RemovePortalFromNode: portal not in leaf\n");
		}
		...
		{
			COM_FatalError("RemovePortalFromNode: portal not bounding leaf\n");
		}
	}
...
void WritePortalfile(tree_t* tree, bool leaked)
{
	...
	COM_FileBase(g_portfilename, shortname, sizeof(shortname));
	...
	if ( leaked )
	{
		MsgDev(D_INFO, "^3BSP probably has a leak, writing portal file:^7 %s.prt\n", shortname);
	}
	else
	{
