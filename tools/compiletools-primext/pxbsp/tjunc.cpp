...
static int HashVec(const vec3_t vec, int* num, int* hash)
{
	...
	*num = 0;
	...
	for ( x = -1; x <= 1; x++ )
	{
		if ( x == -1 && sdiff[0] > h_scale[0] * (2 * ON_EPSILON) ||
			 x == 1 && sdiff[0] < 1 - h_scale[0] * (2 * ON_EPSILON) )
			continue;

		for ( y = -1; y <= 1; y++ )
		{
			if ( y == -1 && sdiff[1] > h_scale[1] * (2 * ON_EPSILON) ||
				 y == 1 && sdiff[1] < 1 - h_scale[1] * (2 * ON_EPSILON) )
				continue;
...
void tjunc(node_t* headnode, bool worldmodel)
{
	...
	if ( worldmodel )
	{
		MsgDev(D_INFO, "t-junction: %s elapsed\n", str);
	}
}
