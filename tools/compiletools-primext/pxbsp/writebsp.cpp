...
static bool PlaneEqual(const plane_t* p, const vec3_t normal, vec_t dist)
{
	...
	if ( -DIST_EPSILON < (t = p->dist - dist) && t < DIST_EPSILON && -DIR_EPSILON < (t = p->normal[0] - normal[0]) &&
		 t < DIR_EPSILON && -DIR_EPSILON < (t = p->normal[1] - normal[1]) && t < DIR_EPSILON &&
...
{
	if ( texinfo == -1 )
		COM_FatalError("EmitTexinfo: texinfo == -1\n");
...
void EmitClipNodes(tree_t* tree, int modnum, int hullnum)
{
	...
	if ( tree->headnode->planenum == PLANENUM_LEAF )
		...
		bm->headnode[hullnum] = g_numclipnodes;
...
int EmitVertex(const vec3_t point)
{
	...
	{
		if ( VectorCompareEpsilon(point, g_dvertexes[i].point, EQUAL_EPSILON) )
		{
			return i;
		}
	}
