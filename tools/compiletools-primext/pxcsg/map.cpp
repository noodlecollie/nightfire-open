...
static short g_world_faceinfo =
	-1;  // shared faceinfo in case level-desginer apply it to the world but except landscapes
int g_world_luxels =
	0;  // alternative lightmap matrix will be used (luxels per world units instead of luxels per texels)
...
*/
static void
EntityApplyTransform(mapent_t* src, mapent_t* dst, bool brushentity, bool external_map, vec_t yaw_offset = 0.0)
{
	...
	else
	{
		...
		if ( temp != 0.0f )
			VectorSet(dscale, temp, temp, temp);
	...
	VectorSet(iscale, 1.0f / dscale[0], 1.0f / dscale[1], 1.0f / dscale[2]);
	...
	else
	{
		...
		else if ( CheckKey((entity_t*)dst, "scale") )
		{
			...
			if ( temp != 0.0f )
				VectorSet(dst_scale, temp, temp, temp);
		}
...
static void SetupSideContents(brush_t* brush, side_t* side)
{
	...
	if ( side->name[0] == '!' || side->name[0] == '*' )
	{
		if ( !Q_strnicmp(side->name + 1, "lava", 4) )
			...
			side->contents = CONTENTS_WATER;  // otherwise it's water
...
static void SetupTextureVectors(mapent_t* mapent, brush_t* brush, side_t* side, short& brush_type)
{
	...
	if ( g_TXcommand == '1' || g_TXcommand == '2' )
	{
		...
		else
		{
			MsgDev(
				D_WARN,
				"Degenerate QuArK-style brush texture: Entity %i, Brush %i\n",
...
static void ParseBrush(mapent_t* mapent, short entindex, short faceinfo, short& brush_type)
{
	...
	for ( i = 0; i < brush->sides.Count(); i++ )
	{
		...
		if ( (g_nullifytrigger || brush->entitynum == 0) &&
			 (!Q_strnicmp(side->name, "AAATRIGGER", 10) || !Q_strnicmp(side->name, "trigger", 7)) )
			SetBits(side->flags, FSIDE_NODRAW);
	}
...
{
	if ( g_onlyents )
		return -1;  // don't modify g_dfaceinfo!
	...
	if ( g_mapentities.Count() == 1 )
	{
		...
		else  // user specified landscape for world-entity, so all other entities will be ingnore them
		{
			...
			// now we specified a global landscape for all the world brushes but except all other entities (include
			// func_group etc)
	...
	else  // all other non-point ents
	{
		...
		if ( !CheckKey((entity_t*)mapent, "zhlt_landscape") && !CheckKey((entity_t*)mapent, "zhlt_texturestep") &&
			 !CheckKey((entity_t*)mapent, "zhlt_maxextent") )
		{
		...
		// get defaults
		if ( !max_extent )
			max_extent = global_max_extent;
		if ( !texture_step )
			texture_step = global_texture_step;
...
bool ParseMapEntity(CUtlVector<mapent_t>* entities, bool external = false)
{
	...
	if ( !Q_strcmp("func_group", classname) || !Q_strcmp("func_landscape", classname) ||
		 !Q_strncmp("func_detail", classname, 11) )
	{
	...
	if ( !Q_strcmp(classname, "misc_model") )
	{
		...
		COM_ReplaceExtension(modelpath, sizeof(modelpath), ".mdl");
	...
	if ( fabs(mapent->origin[0]) > WORLD_MAXS || fabs(mapent->origin[1]) > WORLD_MAXS ||
		 fabs(mapent->origin[2]) > WORLD_MAXS )
	{
...
bool IncludeMapFile(const char* filename, CUtlVector<mapent_t>* entities, int index, bool external_map)
{
	...
	if ( InsertASEModel(filename, mapent, static_cast<short>(index), -1) )
	{
		...
		if ( !target )
			target = &entities->Element(0);  // move to world as default
	...
	COM_ReplaceExtension(path, sizeof(path), ".map");
...
void FilterBrushesIntoArray(mapent_t* mapent, int entnum, int contents)
{
	...
	for ( int j = 0; j < mapent->brushes.Count(); j++ )
	{
		...
		if ( noclip )
			SetBits(src->flags, FBRUSH_NOCLIP);
