...
void TEX_LoadTextures(CUtlVector<mapent_t>* entities, bool merge)
{
	...
	if ( !*pszWadString )
		pszWadString = ValueForKey((entity_t*)&entities[0], "_wad");
	if ( !*pszWadString )
		return;  // may be -nowadtextures was used?
...
static int lump_sorter_by_wad_and_name(const void* lump1, const void* lump2)
{
	...
	if ( plump1->wadnum == plump2->wadnum )
		...
		return plump1->wadnum - plump2->wadnum;
}
...
void AddAnimatingTextures(void)
{
	...
	for ( int i = 0; i < base; i++ )
	{
		...
		Q_strcpy(name, sizeof(name), g_miptex[i].name);
