		...
		if ( Q_strncmp(classname, "light", 5) )
		{
			...
			switch ( style )
			{
				...
				case -2:  // backwards switchable texlight
					...
					stylenum++;
					continue;
				case -3:  // (HACK) a piggyback texlight: switched on and off by triggering a real light that has the
						  // same name
...
void LoadWadValue(void)
{
	...
	if ( *wadvalue )
		MsgDev(D_REPORT, "Wad files required to run the map: \"%s\"\n", wadvalue);
	else
		MsgDev(D_REPORT, "Wad files required to run the map: (None)\n");
...
void BoundWorld(void)
{
	...
	for ( i = 0; i < 3; i++ )
	{
		if ( world_size[i] > WORLD_SIZE )
			MsgDev(
				D_ERROR,
				"world is not fit in allowed bounds by %s-axis (%g > %g)\n",
				axis[i],
				world_size[i],
				WORLD_SIZE);
		if ( world_size[i] < 0 )
			negative_world_bounds++;
	}

	MsgDev(D_INFO, "\n^3World Size:^7 (%.f %.f %.f)\n\n", world_size[0], world_size[1], world_size[2]);
	if ( negative_world_bounds )
		COM_FatalError("Negative world bounds\n");
}
