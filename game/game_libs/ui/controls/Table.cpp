...
CMenuTable::CMenuTable() :
	...
	bShowScrollBar(true),
	szBackground(),
	szUpArrow(UI_UPARROW),
	szUpArrowFocus(UI_UPARROWFOCUS),
	szUpArrowPressed(UI_UPARROWPRESSED),
	szDownArrow(UI_DOWNARROW),
...
void CMenuTable::Draw()
{
	...
	UI_FillRect(upArrow.x, upArrow.y + arrow.h, arrow.w, downArrow.y - upArrow.y - arrow.h, uiInputFgColor);
		...
		else
		{
			...
			else if ( eFocusAnimation == QM_PULSEIFFOCUS )
			{
				...
				UI_DrawPic(
					downArrow,
					arrow,
					(downFocus) ? color : (int)colorBase,
					(downFocus) ? szDownArrowFocus : szDownArrow);
			}
		}
	}
