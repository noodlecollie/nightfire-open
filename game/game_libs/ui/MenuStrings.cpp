...
static void Localize_AddToDictionary(const char* name, const char* lang)
{
	...
	if ( pFileBuf[0] == '\xFF' && pFileBuf[1] == '\xFE' )
	{
		...
		{
			Con_Printf(
				"Localize_AddToDict( %s ): couldn't parse file. UTF-32 little endian isn't supported\n",
				filename);
			goto error;
		}
		isUtf16 = true;
	...
	{
		Con_Printf(
			"Localize_AddToDict( %s ): couldn't parse file. UTF-16/UTF-32 big endian isn't supported\n",
			filename);
		goto error;
	}
