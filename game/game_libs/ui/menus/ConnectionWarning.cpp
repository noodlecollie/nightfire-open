...
public:
	CMenuConnectionWarning() :
		CMenuBaseWindow("ConnectionWarning")
	{
	}
	void _Init() override;
...
void CMenuConnectionWarning::_Init()
{
	...
	SET_EVENT(normal.onChanged, ((CMenuConnectionWarning*)pSelf->Parent())->WriteSettings(EPRESET_NORMAL));

	dsl.szName = L("DSL or PPTP with limited packet size");
	dsl.SetCoord(20, 200);
	SET_EVENT(dsl.onChanged, ((CMenuConnectionWarning*)pSelf->Parent())->WriteSettings(EPRESET_DSL));

	slowest.szName = L("Slow connection mode (64kbps)");
	slowest.SetCoord(20, 260);
	SET_EVENT(slowest.onChanged, ((CMenuConnectionWarning*)pSelf->Parent())->WriteSettings(EPRESET_SLOW));
...
void CMenuConnectionWarning::WriteSettings(const EPresets preset)
{
	const struct
	{
		...
		float cl_updaterate;
		float rate;
	} presets[EPRESET_LAST] = {{1400, 0, 30, 60, 25000}, {1200, 1000, 30, 60, 25000}, {900, 700, 25, 30, 7500}};
