...
public:
	...
	CAdvancedControls() :
		CMenuFramework("CAdvancedControls")
	{
	}
...
void CAdvancedControls::_Init(void)
{
	...
	lookSpring.SetNameAndStatus(
		L("Look spring"),
		L("Causes the screen to 'spring' back to looking straight ahead when you move forward"));
	lookSpring.iFlags |= QMF_NOTIFY;
	lookSpring.SetCoord(72, 410);

	lookStrafe.SetNameAndStatus(
		L("Look strafe"),
