...
void UI_Touch_Menu(void)
{
	if ( !menu_touch->WasInit() )
	{
		...
		menu_touch->AddButton(
			L("Touch options"),
			L("Touch sensitivity and profile options"),
			"gfx/shell/btn_touch_options",
			UI_TouchOptions_Menu,
			QMF_NOTIFY);

		menu_touch->AddButton(
			L("Touch buttons"),
			L("Add, remove, edit touch buttons"),
			"gfx/shell/btn_touch_buttons",
