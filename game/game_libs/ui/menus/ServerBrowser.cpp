...
struct server_t
{
	...
	int Rank(const server_t& other) const
	{
		if ( isLegacy > other.isLegacy )
			return 100;
		else if ( isLegacy < other.isLegacy )
			return -100;
		return 0;
	}
...
#define GENERATE_COMPAR_FN(method) \
	...
	{ \
		return ((const server_t*)a)->Rank(*((const server_t*)b)) + \
			((const server_t*)a)->method(*((const server_t*)b)); \
	} \
	static int method##Descend(const void* a, const void* b) \
	{ \
		return ((const server_t*)a)->Rank(*((const server_t*)b)) + \
			((const server_t*)b)->method(*((const server_t*)a)); \
	}
	...
	const char* GetCellText(int line, int column) override
	{
		switch ( column )
		{
			case COLUMN_PASSWORD:
				return servers[line].havePassword ? ART_BANNER_LOCK : NULL;
			case COLUMN_NAME:
				return servers[line].name;
			case COLUMN_MAP:
				return servers[line].mapname;
			case COLUMN_PLAYERS:
				return servers[line].clientsstr;
			case COLUMN_PING:
				return servers[line].pingstr;
			case COLUMN_IP:
				return servers[line].ipstr;
			default:
				return NULL;
		}
	}
...
public:
	CMenuServerBrowser() :
		CMenuFramework("CMenuServerBrowser"),
		gameListModel(this)
	{
	}
	void Draw() override;
