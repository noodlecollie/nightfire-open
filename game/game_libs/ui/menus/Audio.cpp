...
{
	static const char* lerpingStr[] = {L("GameUI_Disable"), L("Balance"), L("Quality")};
	...
	AddItem(background);
	AddItem(banner);
	AddButton(L("Done"), L("Go back to the Configuration Menu"), PC_DONE, VoidCb(&CMenuAudio::SaveAndPopMenu));
	AddItem(soundVolume);
	AddItem(musicVolume);
	AddItem(suitVolume);
	AddItem(lerping);
