...
public:
	CMenuTouchButtons() :
		CMenuFramework("CMenuTouchButtons"),
		model(this)
	{
	}
...
public:
	...
	{
	public:
		CButtonListModel(CMenuTouchButtons* parent) :
			CStringArrayModel((const char*)&buttons, sizeof(buttons[0]), 0),
			parent(parent)
		{
		}
		...
		void
		AddButtonToList(const char* name, const char* texture, const char* command, unsigned char* color, int flags);
...
void CMenuTouchButtons::CButtonListModel::AddButtonToList(
	const char* name,
	const char* texture,
	const char* command,
	unsigned char* color,
