...
void UI_Video_Menu(void)
{
	if ( !menu_video->WasInit() )
	{
		...
		menu_video->AddButton(
			L("Video options"),
			L("Set video options such as screen size, gamma and image quality."),
			PC_VID_OPT,
			UI_VidOptions_Menu,
			QMF_NOTIFY);
		menu_video->AddButton(
			L("Video modes"),
			L("Set video modes and configure 3D accelerators."),
			PC_VID_MODES,
			UI_VidModes_Menu,
			QMF_NOTIFY);
		menu_video->AddButton(
