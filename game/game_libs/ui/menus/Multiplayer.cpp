...
public:
	CMenuMultiplayer() :
		CMenuFramework("CMenuMultiplayer")
	{
	}

	void AskPredictEnable()
	{
		msgBox.Show();
	}
	void Show() override
	{
		...
		if ( EngFuncs::GetCvarFloat("menu_mp_firsttime") && EngFuncs::GetCvarFloat("cl_disable_movement_prediction") )
		{
			AskPredictEnable();
		}
...
void CMenuMultiplayer::_Init(void)
{
	...
	if ( !EngFuncs::NET_MasterServersDisabled() )
	{
		AddButton(
			L("Internet game"),
			L("View list of a game internet servers and join the one of your choice"),
			PC_INET_GAME,
			UI_InternetGames_Menu,
			QMF_NOTIFY);
	}

	// AddButton( L( "Spectate game" ), L( "Spectate internet games" ), PC_SPECTATE_GAMES, NoopCb, QMF_GRAYED |
	// QMF_NOTIFY );
	AddButton(L("LAN game"), L("Set up the game on the local area network"), PC_LAN_GAME, UI_LanGame_Menu, QMF_NOTIFY);
	AddButton(
		L("GameUI_GameMenu_Customize"),
		L("Choose your player name, and select visual options for your character"),
		PC_CUSTOMIZE,
		UI_PlayerSetup_Menu,
		QMF_NOTIFY);
	...
	msgBox.SetMessage(
		L("It is recomended to enable client movement prediction.\nPress OK to enable it now or enable it later in "
	...
	SET_EVENT_MULTI(msgBox.onPositive, {
		(void)pExtra;
		EngFuncs::CvarSetValue("cl_disable_movement_prediction", 0.0f);
