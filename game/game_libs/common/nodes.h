...
public:
	...
	char m_szLinkEntModelname[4];  // the unique name of the brush model that blocks the connection (this is kept for
								   // save/restore)
...
#define _GRAPH_VERSION \
	(16)  // !!!increment this whenever graph/node/link classes change, to obsolesce older disk files.
#endif
...
#define NUM_RANGES 256
	...
	// we keep track of the last node we searched from and store it here. Subsequent searches by other monsters will
	// pick up where the last search stopped.
	int m_iLastActiveIdleSearch;

	// another such system used to track the search for cover nodes, helps greatly with two monsters trying to get to
	// the same node.
...
public:
	...
	{
		return (m_cSize == MAX_STACK_NODES);
	}
