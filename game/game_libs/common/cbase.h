...
extern "C" EXPORT int
Server_GetPhysicsInterface(int iVersion, server_physics_api_t* pfuncsFromEngine, physics_interface_t* pFunctionTable);
...
#define CLASS_BARNACLE \
	99  // special because no one pays attention to it, and it eats a wide cross-section of creatures.
...
public:
	...
	// classnames still realize that they are teammates. (overridden for monsters that form groups)
	...
	static CBaseEntity*
	Create(const char* szName, const Vector& vecOrigin, const Vector& vecAngles, edict_t* pentOwner = NULL);

	virtual BOOL FBecomeProne(void)
	{
		return FALSE;
	};
	inline edict_t* edict() const
	{
		return ENT(pev);
	};
	inline EOFFSET eoffset() const
	{
		return OFFSET(pev);
	};
...
// If the touch function is not being called, the entity may not be
// linked up to the BSP tree. Make sure the solidity is not SOLID_NOT,
...
public:
	...
	string_t
		m_strChangeTarget;  // if this field is not null, this is an index into the engine string array.
