...
const char* CHalfLifeTeamplay::SetDefaultPlayerTeam(CBasePlayer* pPlayer)
{
	...
	// NFTODO: Should we inform BGRI here of the team change?
...
void CHalfLifeTeamplay::ChangePlayerTeam(CBasePlayer* pPlayer, const char* pTeamName, BOOL bKill, BOOL bGib)
{
	...
	if ( bKill )
	{
		...
		pPlayer->TakeDamage(pevWorld, pevWorld, 2.0f * pPlayer->pev->health, damageFlags);
	...
	if ( bgri )
	{
		bgri->ClientTeamChange(pPlayer->edict(), g_pGameRules->GetTeamIndex(pPlayer->m_szTeamName) + 1);
	}
