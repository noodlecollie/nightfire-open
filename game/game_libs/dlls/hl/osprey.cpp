...
void COsprey::Flight()
{
	...
	float idealTilt = (160 - flSpeed) / 10.0f;
	...
	else
	{
		...
		if ( pPlayer )
		{
			...
			if ( pitch != m_iPitch )
			{
				m_iPitch = static_cast<int>(pitch);
...
void COsprey::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector, const TraceResult* ptr, int bitsDamageType)
{
	...
	if ( ptr->iHitgroup == 3 )
	{
		...
		m_iDoLeftSmokePuff = static_cast<int>(3 + (flDamage / 5.0f));
	}
	...
	if ( ptr->iHitgroup == 2 )
	{
		...
		m_iDoRightSmokePuff = static_cast<int>(3 + (flDamage / 5.0f));
	}
