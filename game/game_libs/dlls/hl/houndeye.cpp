...
public:
	...
	Vector m_vecPackCenter;  // the center of the pack. The leader maintains this by averaging the origins of all pack
							 // members.
};
...
BOOL CHoundeye::FValidateHintType(short sHint)
{
	...
	for ( i = 0; i < SIZE_OF_ARRAY(sHoundHints); i++ )
	{
			...
			if ( !FClassnameIs(pEntity->pev, "monster_houndeye") )
			{
				...
				if ( SquadCount() > 1 )
				{
					...
					flAdjustedDamage = gSkillData.houndeyeDmgBlast +
						gSkillData.houndeyeDmgBlast * (HOUNDEYE_SQUAD_BONUS * (SquadCount() - 1));
				}
				else
				{
					// solo
				...
				if ( !FVisible(pEntity) )
				{
					if ( pEntity->IsPlayer() )
					{
						// if this entity is a client, and is not in full view, inflict half damage. We do this so that
						// players still take the residual damage if they don't totally leave the houndeye's effective
						// radius. We restrict it to clients so that monsters in other parts of the level don't take the
...
Schedule_t slHoundGuardPack[] = {
	{tlHoundGuardPack,
	 ...
	 bits_SOUND_COMBAT |  // sound flags
		 bits_SOUND_WORLD | bits_SOUND_MEAT | bits_SOUND_PLAYER,
	 "GuardPack"},
};
