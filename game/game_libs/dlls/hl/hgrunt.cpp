...
#define HGRUNT_MINIMUM_HEADSHOT_DAMAGE \
	15  // must do at least this much damage in one shot to head to score a headshot kill
...
#define HGRUNT_AE_CAUGHT_ENEMY \
	(10)  // grunt established sight with an enemy (player only) that had previously eluded the squad.
...
enum
{
	...
	SCHED_GRUNT_ESTABLISH_LINE_OF_FIRE,  // move to a location to set up an attack against the enemy. (usually when a
										 // friendly is in the way).
	...
	SCHED_GRUNT_TAKECOVER_FAILED,  // special schedule type that forces analysis of conditions and picks the best
								   // possible schedule to recover from this type of failure.
...
BOOL CHGrunt::CheckMeleeAttack1(float flDot, float flDist)
{
	...
	if ( flDist <= 64 && flDot >= 0.7 && pEnemy->Classify() != CLASS_ALIEN_BIOWEAPON &&
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	switch ( pEvent->event )
	{
		...
		{
			UTIL_MakeVectors(pev->angles);
			// CGrenade::ShootTimed( pev, pev->origin + gpGlobals->v_forward * 34 + Vector( 0, 0, 32 ),
			// m_vecTossVelocity, 3.5 );
		...
		case HGRUNT_AE_GREN_LAUNCH:
		{
			...
			if ( g_iSkillLevel == SKILL_HARD )
				m_flNextGrenadeCheck =
					gpGlobals->time + RANDOM_FLOAT(2, 5);  // wait a random amount of time before shooting again
			else
		...
		case MONSTERSTATE_COMBAT:
		{
			...
			else if ( HasConditions(bits_COND_LIGHT_DAMAGE) )
			{
				...
				if ( iPercent <= 90 && m_hEnemy != 0 )
				{
					...
					if ( FOkToSpeak() )  // && RANDOM_LONG( 0, 1 ) )
					{
						// SENTENCEG_PlayRndSz( ENT( pev ), "HG_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0,
						// m_voicePitch );
			...
			else if ( HasConditions(bits_COND_ENEMY_OCCLUDED) )
			{
				...
				else if ( OccupySlot(bits_SLOTS_HGRUNT_ENGAGE) )
				{
					...
					if ( FOkToSpeak() )  // && RANDOM_LONG( 0, 1 ) )
					{
						// SENTENCEG_PlayRndSz( ENT( pev ), "HG_CHARGE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0,
						// m_voicePitch );
...
Schedule_t* CHGrunt::GetScheduleOfType(int Type)
{
	switch ( Type )
	{
		...
		case SCHED_GRUNT_SUPPRESS:
		{
			...
			{
				m_fFirstEncounter =
					FALSE;  // after first encounter, leader won't issue handsigns anymore when he has a new enemy
