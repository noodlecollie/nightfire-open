...
public:
	...
	{
		return (pev->spawnflags & SF_TANK_ACTIVE) ? TRUE : FALSE;
	}
...
void CFuncTank::AdjustAnglesForBarrel(Vector& angles, float distance)
{
	...
	if ( m_barrelPos.y != 0 || m_barrelPos.z != 0 )
	{
		...
		if ( m_barrelPos.y )
		{
			...
			angles.y += (180.0f / static_cast<float>(M_PI)) * atan2f(m_barrelPos.y, sqrtf(d2 - r2));
		}
		if ( m_barrelPos.z )
		{
			r2 = m_barrelPos.z * m_barrelPos.z;
			angles.x += (180.0f / static_cast<float>(M_PI)) * atan2f(-m_barrelPos.z, sqrtf(d2 - r2));
		}
	}
}
