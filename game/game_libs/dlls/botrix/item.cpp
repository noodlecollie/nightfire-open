...
namespace Botrix
{
	...
	//     return !FLAG_SOME_SET( EFL_NO_PHYSCANNON_INTERACTION, iEFlags ) && ( FLAG_SOME_SET(
	//     SF_PHYSPROP_ALWAYS_PICK_UP, iSpawnFlags ) ||
	//                                                                          !FLAG_SOME_SET(
	//                                                                          SF_PHYSPROP_PREVENT_PICKUP, iSpawnFlags
	//                                                                          ) );
	// }
					 ...
					 CBotrixEngineUtil::IsVisible(vHead, vOrigin, EVisibilityWorld, false) )
				{
					...
					if ( FLAG_SOME_SET(FItemDrawStats, pClient->iItemDrawFlags) )
					{
						...
						else if ( iEntityType == EItemTypeObject )
						{
							...
							const char* sFlags =
								CTypeToString::EntityClassFlagsToString(pEntity->iFlags, false).c_str();
