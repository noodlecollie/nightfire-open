...
namespace good
{
	...
	public:  // Types.
		...
		struct config
		{
			...
			config(
				const ini_string& sKey,
				const ini_string& sValue,
				const ini_string& sJunk,
				bool bIsComment,
				bool bEOLAfterJunk) :
				key(sKey),
				value(sValue),
				junk(sJunk),
				junkIsComment(bIsComment),
				eolAterJunk(bEOLAfterJunk)
			{
			}
	...
	public:  // Methods.
		...
		//--------------------------------------------------------------------------------------------------------
		ini_section(const ini_string& sName) :
			name(sName),
			junkAfterName(""),
			m_lKeyValues()
		{
		...
		ini_string& operator[](const ini_string& sKey)
		{
			...
			{
				it =
					m_lKeyValues.insert(m_lKeyValues.end(), config(sKey, ini_string(""), ini_string(""), false, false));
			}
			return it->value;
		}
