...
namespace good
{
	...
	public:
		...
		//--------------------------------------------------------------------------------------------------------
		priority_queue(int iCapacity) :
			m_cContainer(iCapacity)
		{
		}
		...
		void modify(const T& elem)
		{
			...
			good::heap_modify(&*m_cContainer.begin(), static_cast<int>(&*it - &*m_cContainer.begin()), size(), m_cLess);
		}
