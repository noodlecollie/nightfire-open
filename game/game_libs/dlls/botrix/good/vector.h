...
namespace good
{
	...
	//************************************************************************************************************
	template<typename T, typename Alloc = good::allocator<T>>
	class vector
	{
	public:
		...
		public:
			...
			const_iterator(T* n = NULL) :
				m_pCurrent(n)
			{
			}
			// Copy constructor.
			const_iterator(const_iterator const& itOther) = default;
			...
			/// Operator -.
			int operator-(const const_iterator& itOther) const
			{
				GoodAssert(m_pCurrent);
				return static_cast<int>(m_pCurrent - itOther.m_pCurrent);
			}

			/// Operator +.
			const_iterator operator+(size_type iOffset) const
			{
				GoodAssert(m_pCurrent);
				return const_iterator(m_pCurrent + iOffset);
			}
			/// Operator +=.
			const_iterator& operator+=(size_type iOffset)
			{
				GoodAssert(m_pCurrent);
				m_pCurrent += iOffset;
				return *this;
		...
		public:
			...
			// Constructor by value.
			iterator(pointer n = NULL) :
				base_class(n)
			{
			}
			// Copy constructor.
			iterator(iterator const& itOther) = default;

			/// Operator +.
			iterator operator+(size_type iOffset) const
			{
				GoodAssert(this->m_pCurrent);
				return iterator(this->m_pCurrent + iOffset);
			}
			/// Operator +=.
			iterator& operator+=(size_type iOffset)
			{
				GoodAssert(this->m_pCurrent);
				this->m_pCurrent += iOffset;
				return *this;
			}
			...
			{
				GoodAssert(this->m_pCurrent);
				return static_cast<int>(this->m_pCurrent - other.m_pCurrent);
			}
			...
			/// Dereference.
			reference operator[](int iOffset) const
			{
				GoodAssert(this->m_pCurrent);
				return this->m_pCurrent[iOffset];
			}
			/// Element selection through pointer.
			pointer operator->() const
			{
		...
		{
			return const_iterator(m_pBuffer + m_iSize);
		}
		...
		iterator end()
		{
			return iterator(m_pBuffer + m_iSize);
		}
		...
		//--------------------------------------------------------------------------------------------------------
		T const& operator[](size_type iIndex) const
		{
			GoodAssert(0 <= iIndex && iIndex < m_iSize);
			return m_pBuffer[iIndex];
		}
		...
		/// Element at index.
		//--------------------------------------------------------------------------------------------------------
		T const& at(size_type iIndex) const
		{
			GoodAssert(0 <= iIndex && iIndex < m_iSize);
			return m_pBuffer[iIndex];
		}
		...
		{
			GoodAssert(m_iSize > 0);
			return m_pBuffer[m_iSize - 1];
		}

		//--------------------------------------------------------------------------------------------------------
		/// Add element to end of buffer.
		//--------------------------------------------------------------------------------------------------------
		void push_back(const T& tElem)
		{
			insert(m_iSize, tElem);
		}

		//--------------------------------------------------------------------------------------------------------
		/// Add element to end of buffer.
		//--------------------------------------------------------------------------------------------------------
		void push_front(const T& tElem)
		{
			insert(0, tElem);
		}
		...
		{
			int iPos = static_cast<int>(it.m_pCurrent - m_pBuffer);
			insert(iPos, tElem);
			return iterator(m_pBuffer + iPos);
		}
