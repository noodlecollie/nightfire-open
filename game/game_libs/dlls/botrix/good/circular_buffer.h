...
namespace good
{
	...
	//************************************************************************************************************
	template<typename T, template<typename, typename> class Container = vector>
	...
	public:
		...
		public:
			...
			/// Operator +.
			const_iterator operator+(int iOffset) const
			{
				GoodAssert(m_pCurrent);
				return const_iterator(m_pCurrent + iOffset);
			}
			/// Operator +=.
			const_iterator& operator+=(int iOffset)
			{
				GoodAssert(m_pCurrent);
				m_pCurrent += iOffset;
				return *this;
		...
		public:
			...
			// Copy constructor.
			iterator(iterator const& itOther) :
				base_class(itOther)
			{
			}
			...
			{
				GoodAssert(m_pCurrent);
				return iterator(m_pCurrent + iOffset);
			}
			/// Operator +=.
		...
		public:
			...
			/// Post-increment.
			util_reverse_iterator operator++ (int) { GoodAssert(m_pCurrent); iterator tmp(*this); m_pCurrent--; return
		tmp; }
			/// Post-decrement.
			util_reverse_iterator operator-- (int) { GoodAssert(m_pCurrent); iterator tmp(*this); m_pCurrent++; return
		tmp; }
		...
		base_circular_buffer() :
			...
			//--------------------------------------------------------------------------------------------------------
			base_circular_buffer(int iSize) :
			m_iFirst(0),
			m_iLast(0)
		{
		...
		{
			return m_cContainer.begin() + iBegin;
		}
		...
		//--------------------------------------------------------------------------------------------------------
		T const& operator[](int iIndex) const
		{
			GoodAssert(0 <= iIndex && iIndex < m_iSize);
			return m_pBuffer[iIndex];
		}
