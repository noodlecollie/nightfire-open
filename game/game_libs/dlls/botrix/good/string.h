...
namespace good
{
	 ...
	 */
	template<typename Char = TChar, typename Alloc = good::allocator<Char>>
	class base_string
	{
	public:
		...
		/// Copy constructor.
		//--------------------------------------------------------------------------------------------------------
		base_string(const base_string& other, bool bCopy) :
			m_pBuffer((char*)""),
			m_iSize(0),
			m_iStatic(1)
		{
		...
		//--------------------------------------------------------------------------------------------------------
		base_string(const Char* szStr, bool bCopy = false, bool bDealloc = false, size_type iSize = npos) :
			m_iStatic(true)
		{
#ifdef DEBUG_STRING_PRINT
			...
			else
			{
				...
				// Technically the bit field represents -1 because the int is signed.
				// CBA to change it to unsigned and have to check the rest of this code.
				m_iStatic = -1;
			}
		}
		...
		base_string& assign(const Char* s, size_type iSize = npos, bool bCopy = false)
		{
			...
			if ( bCopy )
			{
				if ( m_iStatic )
					...
					m_pBuffer =
						m_cAlloc.reallocate(m_pBuffer, (iSize + 1) * sizeof(Char), (m_iSize + 1) * sizeof(Char));
...
#ifdef DEBUG_STRING_PRINT
				DebugPrint(
					"base_string deallocate(): %s; free: %d\n",
					(m_iSize ? m_pBuffer : "null"),
					!m_iStatic && m_pBuffer);
#endif
	...
	protected:
		...
		Char* m_pBuffer;  ///< Buffer. It contains an extra int before string (at position -1) which is a reference
						  ///< counter.
