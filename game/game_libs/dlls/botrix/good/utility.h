...
namespace good
{
	...
	public:
		...
		// Default constructor.
		util_reverse_iterator() :
			base_class(NULL)
		{
		}

		// Copy constructor.
		util_reverse_iterator(const base_class& itOther) :
			m_cCurrent(itOther)
		{
		}

		/// Pre-increment.
		util_reverse_iterator& operator++()
		{
			m_cCurrent--;
			return *this;
		}
		...
		/// Post-increment.
		util_reverse_iterator operator++(int)
		{
			util_reverse_iterator tmp(*this);
			m_cCurrent--;
			return tmp;
		}
		/// Post-decrement.
		...
		{
			return m_cCurrent[iOffset - 1];
		}
		/// Element selection through pointer.
		pointer operator->() const
		{
			return (m_cCurrent - 1).operator->();
		}
