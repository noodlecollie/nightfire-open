...
enum TWaypointFlag
{
	...
	FWaypointButton = 1 << 9,  ///< A button is there. Arguments are angle (low word)  button index+1 (3rd byte), door
							   ///< index+1 (4th byte).
	FWaypointSeeButton = 1 << 10,  ///< Button is visible. Arguments are angle (low word), button index+1 (3rd byte),
...
enum TPathFlag
{
	...
	FPathDemo =
		1 << 15,  ///< Flag for use demo to reach adjacent waypoints. Demo number is at lower bits. Not implemented yet.
...
enum TFightStrategyFlagIds
{
	...
	// TODO:
	// FFightStrategyForceStayFar    = 1<<2,        ///< Bot will always stay far away from enemy even if enemy is not
	// visible. FFightStrategyMeleeIfClose    = 1<<3,        ///< Bot will switch to melee weapon if enemy is close.
