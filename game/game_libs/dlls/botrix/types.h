...
namespace Botrix
{
	...
	enum TWaypointFlag
	{
		...
		FWaypointCamper =
			1 << 1,  ///< Camp at spot and wait for enemy. Argument are 2 angles to aim (low and high words).
		...
		FWaypointHealthMachine =
			1 << 7,  ///< Health machine is there. Arguments are 1 angle (low word) and health amount (3rd byte).

		// Second byte.
		FWaypointArmorMachine =
			1 << 8,  ///< Armor machine is there. Arguments are 1 angle (low word) and health amount (3rd byte).
		FWaypointButton = 1 << 9,  ///< A button is there. Arguments are angle (low word)  button index+1 (3rd byte),
								   ///< door index+1 (4th byte).
		FWaypointSeeButton = 1 << 10,  ///< Button is visible. Arguments are angle (low word), button index+1 (3rd
									   ///< byte), door index+1 (4th byte).
		FWaypointUse = 1 << 11,  ///< Always need to USE (along with first angle).
		FWaypointElevator =
			1 << 12,  ///< Waypoint should be marked as button. Instead of door's index it has elevator's index.
	...
	enum TPathFlag
	{
		...
		FPathStop =
			1 << 5,  ///< Stop at current waypoint to go in straight line to next one. Usefull when risk of falling.
		...
		FPathElevator =
			1 << 9,  ///< Stand still until reach next waypoint. Argument has elevator number / button number.
		...
		FPathDemo = 1 << 15,  ///< Flag for use demo to reach adjacent waypoints. Demo number is at lower bits. Not
							  ///< implemented yet.
	...
	enum TWeaponFlagIds
	{
		...
		FWeaponDontAddClip = 1 << 11,  ///< Bug for some mods: some weapons add clip size to extra ammo when weapon is
									   ///< picked (crossbow for hl2dm).
		FWeaponDefaultClipEmpty = 1 << 12,  ///< Clip is empty when respawned / grab this weapon.
		FWeaponBackgroundReload =
			1 << 13,  ///< Bug for some mods: some weapons reload while you hold another weapon (pistol for hl2dm).
	...
	enum TFightStrategyFlagIds
	{
		...
		// TODO:
		// FFightStrategyForceStayFar    = 1<<2,        ///< Bot will always stay far away from enemy even if enemy is
		// not visible. FFightStrategyMeleeIfClose    = 1<<3,        ///< Bot will switch to melee weapon if enemy is
		// close.
	...
	enum TConsoleAutoCompleteArgs
	{
		...
		EConsoleAutoCompleteArgValues,  ///< Auto-complete with given fixed array of strings in m_cAutoCompleteArguments
										///< once.
