...
good::string aBorzhTasks[EBorzhTaskTotal] = {
	...
	"EBorzhTaskWaitPlayer",  ///< Wait for other player to do something. The bot will say "done" phrase when it
							 ///< finishes.
	"EBorzhTaskWaitButton",  ///< Wait for other player to push a button. The other player must say "i will push button
							 ///< now".
	"EBorzhTaskWaitDoor",  ///< Wait for other player to check a door. The other player must say "the door is
						   ///< opened/closed".
	"EBorzhTaskWaitIndications",  ///< Wait for commands of other player.

	// Next tasks are tasks that consist of several atomic tasks. Ordered by priority, i.e. explore has higher priority
	// than try button.
	"EBorzhTaskButtonTryDoors",  ///< Check which doors opens a button. Argument: index is button, type is bool (already
								 ///< pushed or still not).
