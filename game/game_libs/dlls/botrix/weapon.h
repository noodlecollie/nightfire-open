...
namespace Botrix
{
	...
	public:
		...
		CWeapon()
		{
			...
			fMaxDistanceSqr[0] = fMaxDistanceSqr[1] = CBotrixEngineUtil::iMaxMapSizeSqr;
		...
		TWeaponId iId = 0;  ///< Weapon id.
		const CItemClass* pWeaponClass = nullptr;  ///< Pointer to weapon class.

		TClass iClass = 0;  ///< Classes that can uses this weapon. Those are really flags.
		TTeam iTeam = 0;  ///< Only this team can buy this weapon. Those are really flags.

		TWeaponType iType = 0;  ///< Weapon type.
		TWeaponAim iAim[2] = {0, 0};  ///< Where to aim with this weapon.
		TWeaponFlags iFlags[2] = {0, 0};  ///< Attack flags.

		float fMinDistanceSqr[2] = {
			0.0f,
			0.0f
		};  ///< Minimum distance to enemy to safely use this weapon (0 by default).
		...
		float fReloadStartTime[2] = {0.0f, 0.0f};  ///< Time to start reload (primary and secondary).
		float fReloadTime[2] = {0.0f, 0.0f};  ///< Duration of reload (primary and secondary).
		float fDamage[2] = {0.0f, 0.0f};  ///< How much damage one bullet does.

		unsigned char iClipSize[2] = {0, 0};  ///< How much bullets fit in one clip.
		...
		int iParabolicDistance45[2] = {0, 0};  ///< Distance that bullet does looking up 45grades until falls.

		TBotIntelligence iBotPreference = 0;  ///< Smart bots will prefer weapons with higher preference.
		bool bForbidden = false;  ///< True if weapon is forbidden.
	...
	public:
		...
		inline bool HasAmmoInClip(int iSecondary) const
		{
			return m_iBulletsInClip[iSecondary] > 0;
		}

		/// Return true if weapon has ammo, beside the ammo in clip.
		inline bool HasAmmoExtra(int iSecondary) const
		{
			return m_iBulletsExtra[iSecondary] > 0;
		}

		/// Return true if weapon has full ammunition.
		inline bool HasFullAmmo(int iSecondary) const
		{
			return m_iBulletsExtra[iSecondary] == m_pWeapon->iMaxAmmo[iSecondary];
		}

		/// Return true if weapon has ammo.
		inline bool HasAmmo(int iSecondary) const
		{
			return HasAmmoInClip(iSecondary) || HasAmmoExtra(iSecondary);
		}

		/// Return true if weapon has ammo.
		inline bool HasAmmo() const
		{
			return HasAmmo(0) || HasAmmo(1);
		}

		/// Return true if this weapon has no bullets.
		inline bool Empty() const
		{
			return !IsMelee() && !IsPhysics() && !HasAmmo();
		}
		...
		inline bool CanUse() const
		{
			...
			return !IsShooting() && !IsChanging() &&
				(!IsReloading() || ((m_pWeapon->iType >= EWeaponShotgun) && Bullets(0)));
		}
		...
		{
			return (HasAmmoInClip(CWeapon::PRIMARY) &&
					(m_pWeapon->fMaxDistanceSqr[CWeapon::PRIMARY] < fDistanceSqrToEnemy)) ||
				(HasAmmoInClip(CWeapon::SECONDARY) &&
				 (m_pWeapon->fMaxDistanceSqr[CWeapon::SECONDARY] < fDistanceSqrToEnemy));
		}
		...
		/// Set weapon presence.
		inline void SetPresent(bool bPresent)
		{
			m_bWeaponPresent = bPresent;
		}

		/// Remove all bullets for this weapon.
		inline void SetEmpty()
		{
			m_iBulletsInClip[0] = m_iBulletsInClip[1] = 0;
			m_iBulletsExtra[0] = m_iBulletsExtra[1] = 0;
		}

		/// Return damage per second this weapon can do with primary ammo.
		inline float DamagePerSecond() const
		{
			return m_pWeapon->fDamage[0] / m_pWeapon->fShotTime[0];
		}

		/// Return amount of bullets this weapon has for primary or secondary attacks.
		inline int Bullets(int iSecondary) const
		{
			return m_iBulletsInClip[iSecondary];
		}

		/// Return amount of extra bullets this weapon has for primary or secondary attacks.
		inline int ExtraBullets(int iSecondary) const
		{
			return m_iBulletsExtra[iSecondary];
		}

		/// Return approximate damage this weapon can do with bullets in clip.
		inline float Damage(int iSecondary) const
		{
			return m_pWeapon->fDamage[iSecondary];
		}

		/// Return approximate damage this weapon can do with bullets in clip.
		inline float TotalDamage(int iSecondary) const
		{
			return Bullets(iSecondary) * Damage(iSecondary);
		}

		/// Return approximate damage this weapon can do without reload.
		inline float TotalDamage() const
		{
			return TotalDamage(0) + TotalDamage(1);
		}

		/// Return true if weapon needs to be reloaded.
		bool NeedReload(int iSecondary) const
		{
			return (m_iBulletsInClip[iSecondary] < m_pWeapon->iClipSize[iSecondary]) && HasAmmoExtra(iSecondary);
		}

		/// Return true if weapon should be reloaded.
		bool ShouldReload(int iSecondary) const
		{
			return (m_iBulletsInClip[iSecondary] == 0) && HasAmmoExtra(iSecondary);
		}
	...
	public:
		...
		static bool IsValid(TWeaponId iId)
		{
			return 0 <= iId && iId < Size();
		}

		/// Return weapons count.
		static inline int Size()
		{
			return m_aWeapons.size();
		}

		/// Get weapon from weapon id.
		static const CWeaponWithAmmo* Get(TWeaponId iWeaponId)
		{
			return iWeaponId < 0 || iWeaponId >= m_aWeapons.size() ? NULL : &m_aWeapons[iWeaponId];
		}
		...
		/// Allow given weapon.
		static void Allow(TWeaponId iWeaponId)
		{
			((CWeapon*)m_aWeapons[iWeaponId].GetBaseWeapon())->bForbidden = false;
		}

		/// Forbid given weapon.
		static void Forbid(TWeaponId iWeaponId)
		{
			((CWeapon*)m_aWeapons[iWeaponId].GetBaseWeapon())->bForbidden = true;
		}
