...
namespace Botrix
{
	...
	public:  // Methods.
		...
		/// Bot is created for testing path between two waypoints. Will be kicked if fails, killed, or reaches
		/// destination.
	...
	protected:  // Methods.
		...
		// If m_bNeedMove then moves toward m_vDestination. If m_bNeedAim then smoothly aims to m_vLook until
		// m_fEndAimTime.
	...
	protected:  // Members.
		...
		Vector
			m_vForward;  // Bot will look at this forward vector most time it is moving.
		...
		good::vector<CPickedItem> m_aPickedItems;  // Picked items (like health or weapon), for bot to know which items
												   // are available to pick on map.
