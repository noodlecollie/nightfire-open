...
namespace Botrix
{
	...
	public:  // Members and constants.
		...
		/// Trace against all entities in analyze. The issue is that ray tracing sometimes returns some entity (like box
		/// shell or object) instead of hit world. By enabling all trace, we make sure that we won't be adding waypoints
	...
	public:  // Methods.
		...
		CWaypoint(const Vector& vOrigin, int iFlags = FWaypointNone, int iArgument = 0, TAreaId iAreaId = 0) :
			vOrigin(vOrigin),
			iFlags(static_cast<TWaypointFlags>(iFlags)),
			iArgument(iArgument),
			iAreaId(iAreaId)
		{
		}
		...
		static inline bool IsValid(TWaypointId id)
		{
			return (id >= 0);
		}
		...
		static void GetFirstAngle(Vector& a, int iArgument)
		{
			int iPitch = (iArgument << 16) >> (16 + 9);  // 7 higher bits of low word (-64..63).
			int iYaw =
				(iArgument << (16 + 7)) >> (16 + 7);  // 9 lower bits of low word (-256..255 but used -180..+180).
			a.x = static_cast<float>(iPitch);
			a.y = static_cast<float>(iYaw);
			a.z = 0;
		}
		...
		static void GetSecondAngle(Vector& a, int iArgument)
		{
		...
		/// Get ammo from waypoint argument. If the ammo is for secondary weapon function (right click), then
		/// bIsSecondary is set.
		...
		static void SetAmmo(int iAmmo, bool bIsSecondary, int& iArgument)
		{
			SET_1ST_BYTE(iAmmo | (bIsSecondary ? 0x80 : 0), iArgument);
		}

		/// Get weapon index from waypoint argument.
		static TWeaponId GetWeaponId(int iArgument)
		{
			return GET_2ND_BYTE(iArgument);
		}

		/// Set weapon for waypoint argument.
		static void SetWeaponId(TWeaponId iId, int& iArgument)
		{
			SET_2ND_BYTE(iId, iArgument);
		}

		/// Get armor from waypoint argument.
		static int GetArmor(int iArgument)
		{
			return GET_3RD_BYTE(iArgument);
		}

		/// Set armor for waypoint argument.
		static void SetArmor(int iArmor, int& iArgument)
		{
			SET_3RD_BYTE(iArmor, iArgument);
		}

		/// Get health from waypoint argument.
		static int GetHealth(int iArgument)
		{
			return GET_4TH_BYTE(iArgument);
		}

		/// Set health for waypoint argument.
		static void SetHealth(int iHealth, int& iArgument)
		{
			SET_4TH_BYTE(iHealth, iArgument);
		}

		/// Get button index from waypoint argument.
		static TItemIndex GetButton(int iArgument)
		{
			return GET_3RD_BYTE(iArgument) - 1;
		}

		/// Set button index for waypoint argument.
		static void SetButton(TItemIndex iButton, int& iArgument)
		{
			SET_3RD_BYTE(iButton + 1, iArgument);
		}

		/// Get button's door index from waypoint argument.
		static TItemIndex GetDoor(int iArgument)
		{
			return GET_4TH_BYTE(iArgument) - 1;
		}

		/// Set button's door index for waypoint argument.
		static void SetDoor(TItemIndex iDoor, int& iArgument)
		{
			SET_4TH_BYTE(iDoor + 1, iArgument);
		}

		/// Get button's elevator index from waypoint argument.
		static TItemIndex GetElevator(int iArgument)
		{
			return GetDoor(iArgument);
		}

		/// Set button's elevator index for waypoint argument.
		static void SetElevator(TItemIndex iElevator, int& iArgument)
		{
			SetDoor(iElevator, iArgument);
		}
	...
	public:
		...
		bool IsDoor()
		{
			return FLAG_SOME_SET(FPathDoor, iFlags);
		}
		bool HasDoorNumber()
		{
			return IsDoor() && GetDoorNumber() >= 0;
		}
		TItemIndex GetDoorNumber()
		{
			return GET_1ST_BYTE(iArgument) - 1;
		}
		void SetDoorNumber(TItemIndex iDoor)
		{
			SET_1ST_BYTE(iDoor + 1, iArgument);
		}

		bool IsElevator()
		{
			return FLAG_SOME_SET(FPathElevator, iFlags);
		}
		bool HasElevatorNumber()
		{
			return IsElevator() && GetElevatorNumber() >= 0;
		}
		TItemIndex GetElevatorNumber()
		{
			return GetDoorNumber();
		}
		void SetElevatorNumber(TItemIndex iElevator)
		{
			SetDoorNumber(iElevator);
		}
		...
		TItemIndex GetButtonNumber()
		{
			return GET_2ND_BYTE(iArgument) - 1;
		}
		void SetButtonNumber(TItemIndex iButton)
		{
			SET_2ND_BYTE(iButton + 1, iArgument);
		}

		int ActionTime()
		{
			return GET_1ST_BYTE(iArgument);
		}
		void SetActionTime(int iTime)
		{
			SET_1ST_BYTE(iTime, iArgument);
		}
		...
		void SetActionDuration(int iTime)
		{
			SET_2ND_BYTE(iTime, iArgument);
		}

		bool HasDemo()
		{
			return FLAG_SOME_SET(FPathDemo, iFlags);
		}
		int GetDemoNumber()
		{
			return iArgument;
		}
		void SetDemoNumber(int iDemo)
		{
			iArgument = static_cast<TPathArgument>(iDemo);
		}

		float fLength;  ///< Path length. Needed for route calculation.
		TPathFlags iFlags;  ///< Path flags.

		/// Path argument. At 1rst byte there is time to wait before action (in deciseconds). 2nd byte is action
		/// duration.
	...
	public:  // Methods.
		...
		static bool IsValid(TWaypointId id)
		{
			return m_cGraph.is_valid(id);
		}

		/// Return waypoints count.
		static int Size()
		{
			return (int)m_cGraph.size();
		}
		...
		/// Get waypoint.
		static inline CWaypoint& Get(TWaypointId id)
		{
			return m_cGraph[id].vertex;
		}
		...
		static WaypointNode& GetNode(TWaypointId id)
		{
			return m_cGraph[id];
		}
		...
		/// Add waypoint.
		static TWaypointId
		Add(const Vector& vOrigin, TWaypointFlags iFlags = FWaypointNone, int iArgument = 0, int iAreaId = 0);
		...
		static void CreatePathsWithAutoFlags(
		...
		static TAreaId GetAreaId(const good::string& sName)
		{
			...
			if ( it == m_aAreas.end() )
			{
				return EAreaIdInvalid;
			}

			return static_cast<TAreaId>(it - m_aAreas.begin());
		}

		/// Add new area name.
		static TAreaId AddAreaName(const good::string& sName)
		{
			m_aAreas.push_back(sName);
			return static_cast<TAreaId>(m_aAreas.size() - 1);
		}
		...
		static void ClearUnreachablePaths()
		{
			m_aUnreachablePaths.clear();
		}

		/// Analyze waypoints.
		static void Analyze(struct edict_s* pClient, bool bShowLines = false);
		...
		{
			return m_iAnalyzeStep != EAnalyzeStepTotal;
		}
		...
		static void AnalyzeClear(TAnalyzeWaypoints iWhich)
		{
			m_aWaypointsToAddOmitInAnalyze[iWhich].clear();
		}
	...
	protected:
		...
		// Analyze one waypoint (for AnalyzeStep()). Return true, if waypoint has nearby waypoints or new waypoint is
		// added.
		...
		static const int BUCKETS_SPACE_X = CBotrixEngineUtil::iMaxMapSize / BUCKETS_SIZE_X;  // 341.33 = 341
		static const int BUCKETS_SPACE_Y = CBotrixEngineUtil::iMaxMapSize / BUCKETS_SIZE_Y;
		static const int BUCKETS_SPACE_Z = CBotrixEngineUtil::iMaxMapSize / BUCKETS_SIZE_Z;

		// Get bucket indexes in array of buckets. Normalize position first to be 0..32768 instead of -16384..16384
		// (iHalfMaxMapSize).
		static int GetBucketX(float fPositionX)
		{
			return (int)(fPositionX + CBotrixEngineUtil::iHalfMaxMapSize) / BUCKETS_SPACE_X;
		}
		static int GetBucketY(float fPositionY)
		{
			return (int)(fPositionY + CBotrixEngineUtil::iHalfMaxMapSize) / BUCKETS_SPACE_Y;
		}
		static int GetBucketZ(float fPositionZ)
		{
			return (int)(fPositionZ + CBotrixEngineUtil::iHalfMaxMapSize) / BUCKETS_SPACE_Z;
		}
		...
		static Bucket m_cBuckets[BUCKETS_SIZE_X][BUCKETS_SIZE_Y]
								[BUCKETS_SIZE_Z];  // 3D hash table of arrays of waypoint IDs.
		...
		public:
			bool a[3][3];
		};  // From left bottom to right upper.

		static good::vector<CNeighbour> m_aWaypointsNeighbours;  // To know if waypoint made check for neighbours near,
																 // initialized during first step.
		...
		static struct edict_s* m_pAnalyzer;  // The person who launched console command "analyze".
	};
}  // namespace Botrix
