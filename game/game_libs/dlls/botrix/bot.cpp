...
namespace Botrix
{
	...
	void CBotrixBot::AddWeapon(TWeaponId iWeaponId)
	{
		...
		{
			GetPlayerClassPtr()->SelectItem(
				CWeapons::Get(iWeaponId)->GetBaseWeapon()->pWeaponClass->sClassName.c_str()
			);
	...
	void CBotrixBot::CurrentWaypointJustChanged()
	{
		if ( m_bNeedMove && m_bUseNavigatorToMove )
		{
			if ( m_cNavigator.SearchEnded() )
			{
				...
				{
					BLOG_W(
						"%s -> Invalid current waypoint %d (should be %d).",
						GetName(),
						iCurrentWaypoint,
						iNextWaypoint
					);
			...
			else if ( FLAG_SOME_SET(FWaypointUse | FWaypointButton, w.iFlags) )
			{
				...
				if ( !m_bNeedUse )
				{
					...
					if ( pCurrentPath )
					{
						...
						{
							m_bNeedUse =
								CItems::IsDoorClosed(iDoor) != 0;  // Only use the button if the door is closed.
						}
					}
				}
				m_bUsingHealthMachine = false;
	...
	void CBotrixBot::DoPathAction()
	{
		...
		if ( CWaypoint::IsValid(iNextWaypoint) && (iCurrentWaypoint != iNextWaypoint) )
		{
			...
			if ( m_bNeedAttack || m_bNeedJump || m_bNeedJumpDuck )
			{
				...
				int iDuration =
					pCurrentPath
						->ActionDuration();  // Action duration (duck for jump / attack for break) in deciseconds.
				if ( iDuration == 0 )
				{
...
//================================================================================================================
	...
	bool CBotrixBot::ResolveStuckMove()
	{
		...
		for ( int iIndexStuckObject = 0; iIndexStuckObject < m_aNearestItems[EItemTypeObject].size();
			  ++iIndexStuckObject )
		{
		...
		if ( pStuckObject )
		{
			...
			else if ( bCanThrow )
			{
				...
				m_fStartActionTime = m_fEndAimTime = CBotrixServerPlugin::GetTime() + GetEndLookTime();
				m_fEndActionTime =
					m_fStartActionTime + 10.0f;  // Give enough time to switch weapon/aim/look back/throw.
		...
		else if ( m_bUseNavigatorToMove )
		{
			...
			else  // Got stuck, because bot falled down or someone pushed it.
			{
				...
				m_bNeedJumpDuck = m_bNeedJump = m_bNeedAttack2 = m_bNeedAttack = m_bNeedSprint = m_bNeedDuck =
					m_bNeedWalk = false;
		...
		if ( !m_bDontAttack )
		{
			...
			else  // No enemy.
			{
				...
				if ( pWeapon )
				{
					...
					else if ( m_bNeedReload && !pWeapon->IsReloading() && !m_bStuckUsePhyscannon &&
							  !m_bStuckBreakObject )
					{
						WeaponChoose();
					}
			...
			if ( !m_bAlreadyUsed )
			{
				...
				if ( m_bUsingHealthMachine || m_bUsingArmorMachine )
				{
					if ( CBotrixServerPlugin::GetTime() >= m_fEndActionTime )
					{
						...
						int iHealthArmor =
							m_bUsingHealthMachine ? m_PlayerInfo.GetHealth() : m_PlayerInfo.GetArmorValue();
		...
		switch ( iNewTask )
		{
			...
			case EBotTaskFindAmmo:
			{
				...
				else
				{
					...
					pEntityClass =
						(iAmmoEntitiesSize > 0) ? pWeapon->aAmmos[bSecondary][rand() % iAmmoEntitiesSize] : NULL;
				}
		...
		while ( retries < maxretries )
		{
			...
			if ( !outOfRetries )
			{
				switch ( iNewTask )
				{
					case EBotTaskFindWeapon:
					{
						...
						{
							// None found, search for worst weapons.
							for ( TBotIntelligence iPreference = iWeaponPreference - 1; iPreference >= 0;
								  --iPreference )
							{
								iWeapon = CWeapons::GetRandomWeapon(iPreference, m_cSkipWeapons);
			...
			if ( bForce || (m_iCurrentTask != iNewTask) )
			{
				...
				if ( (m_iTaskDestination == EWaypointIdInvalid) || (m_iTaskDestination == iCurrentWaypoint) )
				{
					...
					// Bot searched for item at current waypoint, but item was not there. Add it to array of picked
					// items.
					TaskFinished();
				}
				else
