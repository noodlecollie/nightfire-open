...
//----------------------------------------------------------------------------------------------------------------
	...
	TCommandResult CWaypointDrawFlagCommand::Execute(CBotrixClient* pClient, int argc, const char** argv)
	{
		...
		if ( argc == 0 )
		{
			...
			BULOG_I(
				pClient->GetEdict(),
				"Waypoint draw flags: %s.",
				(sType.size() != 0) ? sType.c_str() : sNone.c_str()
			);
			return ECommandPerformed;
		}
	...
	CWaypointAnalyzeDebugCommand::CWaypointAnalyzeDebugCommand()
	{
		...
		m_sDescription =
			"Parameter: (on / off / clear) (current / destination / waypoint id). 'clear' will remove all debug "
			"waypoints.";
		m_iAccessLevel = FCommandAccessWaypoint;
	...
	TCommandResult CWaypointArgumentCommand::Execute(CBotrixClient* pClient, int argc, const char** argv)
	{
		...
		else if ( sButton == argv[0] )
		{
			...
			if ( !bButton )
			{
				BULOG_W(
					pClient->GetEdict(),
					"Error, first you need to set waypoint type accordingly (button/see-button)."
				);
				return ECommandError;
			}
			if ( bAngle2 )
			...
			if ( (i1 < 0) || (i1 >= iButtonsCount) )
			{
				BULOG_W(
					pClient->GetEdict(),
					"Error, invalid button argument (must be from 0 to %d).",
					iButtonsCount - 1
				);
				return ECommandError;
			}
		...
		else if ( sElevator == argv[0] )
		{
			...
			if ( (i1 < 0) || (i1 > iElevatorsCount) )
			{
				BULOG_W(
					pClient->GetEdict(),
					"Error, invalid elevator argument (must be from 0 to %d).",
					iElevatorsCount
				);
				return ECommandError;
			}
	...
	CWaypointVisibilityCommand::CWaypointVisibilityCommand()
	{
		...
		m_sDescription = good::string("Can be 'none' / 'all' / 'next' or mix of: ") +
			CTypeToString::PathDrawFlagsToString(FPathDrawAll);
		m_iAccessLevel = FCommandAccessWaypoint;
	...
	*/
	...
	TCommandResult CPathCreateCommand::Execute(CBotrixClient* pClient, int argc, const char** argv)
	{
		...
		{
			float fHeight =
				pClient->GetPlayerInfo()->GetPlayerMaxs().z - pClient->GetPlayerInfo()->GetPlayerMins().z + 1;
	...
	CItemDrawTypeCommand::CItemDrawTypeCommand()
	{
		...
		m_sDescription = good::string("Can be 'none' / 'all' / 'next' or mix of: ") +
			CTypeToString::ItemDrawFlagsToString(FItemDrawAll);
		m_iAccessLevel = FCommandAccessWaypoint;
...
//----------------------------------------------------------------------------------------------------------------
	...
	void CBotrixCommand::RegisterServerCommands()
	{
		g_engfuncs.pfnAddServerCommand(
			...
			[]()
			{
				CHalfLifeMultiplay* gameRules =
					g_pGameRules ? dynamic_cast<CHalfLifeMultiplay*>(g_pGameRules) : nullptr;
