...
namespace Botrix
{
	...
	public:
		...
		CItemClass() :
			szEngineName(NULL),
			iFlags(0),
			fPickupDistanceSqr(0.0f)
		{
		}

		CItemClass(const char* name, TItemFlags flags) :
			sClassName(name),
			iFlags(flags)
		{
		}
		...
		/// Set class argument.
		void SetArgument(int iArgument)
		{
			SET_2ND_WORD(iArgument, iFlags);
		}

		/// Get class argument.
		int GetArgument() const
		{
			return GET_2ND_WORD(iFlags);
		}

		/// == operator.
		bool operator==(const CItemClass& cOther) const
		{
			return (szEngineName && (szEngineName == cOther.szEngineName)) || (sClassName == cOther.sClassName);
		}
	...
	public:
		...
		{
			return FLAG_SOME_SET(FObjectExplosive, iFlags);
		}

		/// Return true if item can't be picked with physcannon.
		bool IsHeavy() const
		{
			return FLAG_SOME_SET(FObjectHeavy, iFlags);
		}
	...
	public:
		...
		CPickedItem(TItemType iType, TItemIndex iIndex, float fRemoveTime = 0.0f) :
			iType(iType),
			iIndex(iIndex),
			fRemoveTime(fRemoveTime)
		{
		}

		/// == operator.
		bool operator==(const CPickedItem& other) const
		{
			return (other.iType == iType) && (other.iIndex == iIndex);
		}
	...
	public:
		...
		/// Get item classes.
		static const good::list<CItemClass>* GetClasses()
		{
			return m_aItemClasses;
		}
		...
		static TItemType
		GetEntityType(const char* szClassName, CItemClass*& pEntityClass, TItemType iFrom, TItemType iTo);

		/// Get nearest item for a class (for example some item_battery or item_suitcharger for armor), skipping picked
		/// items in aSkip array.
		static TItemIndex GetNearestItem(
		...
		{
			return pEntityClass->szEngineName != NULL;
		}

		/// Add item class (for example item_healthkit for health class).
		static const CItemClass* AddItemClassFor(TItemType iEntityType, const CItemClass& cItemClass)
		{
		...
		static const good::vector<TItemId>& GetObjectsFlags()
		{
			return m_aObjectFlags;
		}

		/// Set all objects flags.
		static void SetObjectsFlags(const good::vector<TItemId>& aObjectsFlags)
		{
			m_aObjectFlags = aObjectsFlags;
		}
		...
		static int IsElevatorHigh(TItemIndex iElevator)
		{
			return IsDoorClosed(iElevator);
		}
	...
	protected:
		...
		static good::list<CItemClass> m_aItemClasses[EItemTypeKnownTotal];  // List of item classes. Pointer are used so
																			// it should not be reallocated.
		...
		static good::vector<struct edict_s*> m_aOthers;  // Array of other entities.
		...
		// This one is to have models specific flags (for example car model with 'heavy' flag, or barrel model with
		// 'explosive' flag).
