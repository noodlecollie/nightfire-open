...
public:
	...
	/// Setup searching a path between given waypoints, avoiding certain areas, and setting search step size
	/// (iMaxWaypointsInLoop).
	bool SearchSetup(
		TWaypointId iFrom,
		TWaypointId iTo,
		good::vector<TAreaId> const& aAvoidAreas,
		int iMaxWaypointsInLoop = MAX_WAYPOINTS_IN_LOOP);
	...
	bool SearchStarted() const
	{
		return m_bSearchStarted;
	}

	/// Return true if search is ended.
	bool SearchEnded() const
	{
		return m_bSearchEnded;
	}
	...
	{
		return m_bSearchEnded && m_cAstar.has_path();
	}

	/// Return true if founded path has more coordinates.
	bool HasMoreCoords() const
	{
		return 0 <= m_iPathIndex && m_iPathIndex < (int)m_cAstar.path().size();
	}
...
protected:
	...
	public:
		// Constructor.
		CCanUseWaypoint(good::vector<TAreaId> const& aAvoidedAreas) :
			m_cAvoidAreas(aAvoidedAreas)
		{
		}
		...
		{
			return find(m_cAvoidAreas.begin(), m_cAvoidAreas.end(), w.vertex.iAreaId) ==
				m_cAvoidAreas.end();  // Waypoint's area is not in avoided areas.
		}
	...
	public:
		...
		{
			return (w2.vOrigin - w1.vOrigin).Length();
		}
	};
	...
	public:
		...
		float operator()(CWaypointPath const& cPath) const
		{
			return cPath.fLength;
		}
	};

protected:
	typedef good::astar<CWaypoint, CWaypointPath, float, CWaypointDistance, CWaypointPathLength, CCanUseWaypoint>
		astar_t;
