...
namespace Botrix
{
	...
	bool
	CBotrixEngineUtil::IsVisible(const Vector& vSrc, const Vector& vDest, TVisibility /* iVisibility */, bool bUsePVS)
	{
		if ( bUsePVS )
		{
	...
	TReach CBotrixEngineUtil::CanClimbSlope(const Vector& vSrc, const Vector& vDest)
	{
		...
		return CanPassSlope(ang.x, CBotrixParameterVars::MAX_CLIMBABLE_SLOPE_PITCH) ? EReachReachable
																					: EReachNotReachable;
	}
