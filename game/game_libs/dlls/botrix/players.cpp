...
void CPlayer::PreThink()
{
	...
	if ( CBotrixServerPlugin::GetTime() >= m_fNextDrawHullTime )
	{
		m_fNextDrawHullTime = CBotrixServerPlugin::GetTime() + fDrawTime;
		Vector vPos = m_PlayerInfo.GetAbsOrigin();
		Vector vMins = m_PlayerInfo.GetPlayerMins();
		Vector vMaxs = m_PlayerInfo.GetPlayerMaxs();
		CBotrixEngineUtil::DrawBox(vPos, vMins, vMaxs, fDrawTime, 0xFF, 0xFF, 0xFF);
	}
#endif
...
)
{
	...
	{
		iIntelligence =
			(rand() % (CBotrixBot::iMaxIntelligence - CBotrixBot::iMinIntelligence + 1)) + CBotrixBot::iMinIntelligence;
	}
