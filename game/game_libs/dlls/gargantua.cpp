...
void CStomp::Spawn(void)
{
	...
	pev->renderamt = 0;
	EMIT_SOUND_DYN(edict(), CHAN_BODY, GARG_STOMP_BUZZ_SOUND, 1, ATTN_NORM, 0, static_cast<int>(PITCH_NORM * 0.55f));
}
...
void CGargantua::EyeUpdate(void)
{
	if ( m_pEyeGlow )
	{
		m_pEyeGlow->pev->renderamt = UTIL_Approach(static_cast<float>(m_eyeBrightness), m_pEyeGlow->pev->renderamt, 26);
...
void CGargantua::FlameCreate(void)
{
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		if ( m_pFlame[i] )
		{
			...
			CSoundEnt::InsertSound(bits_SOUND_COMBAT, posGun, 384, 0.3f);
		}
	}
