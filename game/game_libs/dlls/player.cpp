...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] = {
	...
	// DEFINE_FIELD( CBasePlayer, m_nCustomSprayFrames, FIELD_INTEGER ), // Don't restore, depends on server message
	// after spawning and only matters in multiplayer DEFINE_FIELD( CBasePlayer, m_vecAutoAim, FIELD_VECTOR ), // Don't
	// save/restore - this is recomputed DEFINE_ARRAY( CBasePlayer, m_rgAmmoLast, FIELD_INTEGER, MAX_AMMO_SLOTS ), //
	// Don't need to restore DEFINE_FIELD( CBasePlayer, m_fOnTarget, FIELD_BOOLEAN ), // Don't need to restore
...
int CBasePlayer::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	// Armor.
	if ( !(pev->flags & FL_GODMODE) && pev->armorvalue &&
		 !(bitsDamageType & (DMG_FALL | DMG_DROWN)) )  // armor doesn't protect against fall or drown damage!
	{
	...
	fTookDamage = CBaseMonster::TakeDamage(pevInflictor, pevAttacker, floorf(flDamage), bitsDamageType);
	...
	g_pGameRules->PlayerHurt(this, pevAttacker);
...
void CBasePlayer::RemoveAllItems(BOOL removeSuit)
{
	...
	for ( i = 0; i < MAX_ITEM_TYPES; i++ )
	{
		...
		while ( m_pActiveItem )
		{
			...
			m_pActiveItem->DetachFromPlayer(this);
			m_pActiveItem->Drop();
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if ( pev->modelindex && (!m_fSequenceFinished) && (pev->deadflag == DEAD_DYING) )
	{
		...
		m_iRespawnFrames++;  // Note, these aren't necessarily real "frames", so behavior is dependent on # of client
							 // movement commands
		if ( m_iRespawnFrames < 120 )  // Animations should be no longer than this
			return;
	}

	// once we're done animating our death and we're on the ground, we want to set movetype to None so our dead body
	// won't do collisions and stuff anymore this prevents a bug where the dead body would go to a player's head if he
	// walked over it while the dead player was clicking their button to respawn
	...
	// choose to respawn.
	if ( g_pGameRules->IsMultiplayer() && (gpGlobals->time > (m_fDeadTime + 6)) &&
...
void CBasePlayer::StartObserver(Vector vecPosition, Vector vecViewAngle)
{
	...
	pev->fov = static_cast<float>(m_iFOV);
	MESSAGE_BEGIN(MSG_ONE, gmsgSetFOV, NULL, pev);
	WRITE_BYTE(0);
	MESSAGE_END();
...
void CBasePlayer::PlayerUse(void)
{
	...
	if ( pObject )
	{
		...
		else if ( (m_afButtonReleased & IN_USE) && (pObject->ObjectCaps() & FCAP_ONOFF_USE) )  // BUGBUG This is an
																							   // "off" use
		{
			...
			if ( pEntity->Classify() == CLASS_PLAYER )
			{
				...
				if ( g_pGameRules->PlayerRelationship(this, pEntity) == GR_TEAMMATE )
				{
					...
					newSBarState[SBAR_ID_TARGETARMOR] =
						(int)pEntity->pev->armorvalue;  // No need to get it % based since 100 it's the max.
				}
...
void CBasePlayer::PreThink(void)
{
	...
	if ( m_afPhysicsFlags & PFLAG_ONTRAIN )
	{
		...
		iGearId = TrainSpeed(static_cast<int>(pTrain->pev->speed), pTrain->pev->impulse);
...
void CBasePlayer::UpdatePlayerSound(void)
{
	...
	{
		// since the extra sound that a weapon emits only lasts for one client frame, we keep that sound around for a
		// server frame or two after actual emission to make sure it gets heard.
		...
		if ( m_rgpPlayerItems[i] )
		{
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					{
						gun->m_flEnqueuedMechanicInvocationTime =
							Q_max(gun->m_flEnqueuedMechanicInvocationTime - gpGlobals->frametime, -0.001f);
					}
...
void CBasePlayer::Precache(void)
{
	...
	pev->fov = static_cast<float>(m_iFOV);  // Vit_amiN: restore the FOV on level change or map/saved game load
}
...
int CBasePlayer::RemovePlayerItem(CBasePlayerItem* pItem, bool bCallHolster)
{
	...
	pItem->DetachFromPlayer(this);

	// In some cases an item can be both the active and last item, like for instance dropping all weapons and only
	// having an exhaustible weapon left. - Solokiller
