...
public:
	...
	{
		pev->frags = static_cast<float>(points);
	}
...
public:
	...
	{
		return (pev->spawnflags & SF_ENVTEXT_ALLPLAYERS);
	}
...
// Save parms as a block.  Will break save/restore if the structure changes, but this entity didn't ship with Half-Life,
...
void CGameText::KeyValue(KeyValueData* pkvd)
{
	...
	else if ( FStrEq(pkvd->szKeyName, "color") )
	{
		...
		m_textParms.r1 = static_cast<byte>(color[0]);
		m_textParms.g1 = static_cast<byte>(color[1]);
		m_textParms.b1 = static_cast<byte>(color[2]);
		m_textParms.a1 = static_cast<byte>(color[3]);
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq(pkvd->szKeyName, "color2") )
	{
		int color[4];
		UTIL_StringToIntArray(color, 4, pkvd->szValue);
		m_textParms.r2 = static_cast<byte>(color[0]);
		m_textParms.g2 = static_cast<byte>(color[1]);
		m_textParms.b2 = static_cast<byte>(color[2]);
		m_textParms.a2 = static_cast<byte>(color[3]);
		pkvd->fHandled = TRUE;
	}
...
public:
	...
	{
		return (pev->spawnflags & SF_TEAMMASTER_FIREONCE) ? TRUE : FALSE;
	}
	inline BOOL AnyTeam(void)
	{
		return (pev->spawnflags & SF_TEAMMASTER_ANYTEAM) ? TRUE : FALSE;
	}
...
public:
	...
	inline BOOL RemoveOnFire(void)
	{
		return (pev->spawnflags & SF_TEAMSET_FIREONCE) ? TRUE : FALSE;
	}
	inline BOOL ShouldClearTeam(void)
	{
		return (pev->spawnflags & SF_TEAMSET_CLEARTEAM) ? TRUE : FALSE;
	}
...
public:
	...
	inline BOOL RemoveOnFire(void)
	{
		return (pev->spawnflags & SF_PKILL_FIREONCE) ? TRUE : FALSE;
	}
...
public:
	...
	inline BOOL RemoveOnFire(void)
	{
		return (pev->spawnflags & SF_GAMECOUNT_FIREONCE) ? TRUE : FALSE;
	}
	inline BOOL ResetOnFire(void)
	{
		return (pev->spawnflags & SF_GAMECOUNT_RESET) ? TRUE : FALSE;
	}
...
private:
	...
	{
		pev->frags = static_cast<float>(value);
	}
	inline void SetInitialValue(int value)
	{
		pev->dmg = static_cast<float>(value);
	}
};
...
public:
	...
	inline BOOL RemoveOnFire(void)
	{
		return (pev->spawnflags & SF_GAMECOUNTSET_FIREONCE) ? TRUE : FALSE;
	}
...
public:
	...
	{
		return (pev->spawnflags & SF_PLAYEREQUIP_USEONLY) ? TRUE : FALSE;
	}
...
private:
	inline BOOL RemoveOnFire(void)
	{
		return (pev->spawnflags & SF_PTEAM_FIREONCE) ? TRUE : FALSE;
	}
	inline BOOL ShouldKillPlayer(void)
	{
		return (pev->spawnflags & SF_PTEAM_KILL) ? TRUE : FALSE;
	}
	inline BOOL ShouldGibPlayer(void)
	{
		return (pev->spawnflags & SF_PTEAM_GIB) ? TRUE : FALSE;
	}
