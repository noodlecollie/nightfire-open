...
{
	CSquidSpit* pSpit = GetClassPtr<CSquidSpit>();
	pSpit->Spawn();
...
int CBullsquid::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	// if the squid is running, has an enemy, was hurt by the enemy, hasn't been hurt in the last 3 seconds, and isn't
...
BOOL CBullsquid::CheckMeleeAttack2(float flDot, float flDist)
{
	if ( flDist <= 85 && flDot >= 0.7 &&
		 !HasConditions(bits_COND_CAN_MELEE_ATTACK1) )  // The player & bullsquid can be as much as their bboxes
...
BOOL CBullsquid::FValidateHintType(short sHint)
{
	...
	for ( i = 0; i < XASH_ARRAY_SIZE(sSquidHints); i++ )
	{
...
{
	return bits_SOUND_WORLD | bits_SOUND_COMBAT | bits_SOUND_CARCASS | bits_SOUND_MEAT | bits_SOUND_GARBAGE |
		bits_SOUND_PLAYER;
}
...
void CBullsquid::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch ( pEvent->event )
	{
		...
		{
			CBaseEntity* pHurt =
				CheckTraceHullAttack(70, static_cast<int>(gSkillData.bullsquidDmgWhip), DMG_CLUB | DMG_ALWAYSGIB);
		...
		case BSQUID_AE_THROW:
		{
			...
			if ( pHurt )
			{
				...
				// screeshake transforms the viewmodel as well as the viewangle. No problems with seeing the ends of the
				// viewmodels.
...
Schedule_t slSquidRangeAttack1[] = {
	{tlSquidRangeAttack1,
	 XASH_ARRAY_SIZE(tlSquidRangeAttack1),
...
Schedule_t slSquidChaseEnemy[] = {
	{tlSquidChaseEnemy1,
	 XASH_ARRAY_SIZE(tlSquidChaseEnemy1),
...
Schedule_t slSquidHurtHop[] = {{tlSquidHurtHop, XASH_ARRAY_SIZE(tlSquidHurtHop), 0, 0, "SquidHurtHop"}};
...
Schedule_t slSquidSniffAndEat[] = {
	{tlSquidSniffAndEat,
	 XASH_ARRAY_SIZE(tlSquidSniffAndEat),
	 bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_NEW_ENEMY,
	 // even though HEAR_SOUND/SMELL FOOD doesn't break this schedule, we need this mask
	 // here or the monster won't detect these sounds at ALL while running this schedule.
	 bits_SOUND_MEAT | bits_SOUND_CARCASS,
...
MONSTERSTATE CBullsquid::GetIdealState(void)
{
	...
	switch ( m_MonsterState )
	{
		case MONSTERSTATE_COMBAT:
			...
			*/
			{
				if ( m_hEnemy != 0 && (iConditions & bits_COND_LIGHT_DAMAGE || iConditions & bits_COND_HEAVY_DAMAGE) &&
					 FClassnameIs(m_hEnemy->pev, "monster_headcrab") )
				{
					// if the squid has a headcrab enemy and something hurts it, it's going to forget about the crab for
					// a while.
					m_hEnemy = NULL;
