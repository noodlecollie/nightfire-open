...
void CFuncTrackTrain::StopSound(void)
{
	...
	if ( m_soundPlaying && pev->noise )
	{
		...
		event << Flags(FEV_RELIABLE | FEV_UPDATE) << Invoker(edict()) << EventIndex(m_usAdjustPitch)
			  << IntParam1(us_encode) << BoolParam1(true);
...
void CFuncTrackTrain::UpdateSound(void)
{
	...
	flpitch = TRAIN_STARTPITCH + (fabsf(pev->speed) * (TRAIN_MAXPITCH - TRAIN_STARTPITCH) / TRAIN_MAXSPEED);
	...
	else
	{
		/*
				// update pitch
				EMIT_SOUND_DYN( ENT( pev ), CHAN_STATIC, STRING( pev->noise ), m_flVolume, ATTN_NORM, SND_CHANGE_PITCH,
		   (int)flpitch );
		*/
