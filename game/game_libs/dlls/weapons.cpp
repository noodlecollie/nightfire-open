...
int MaxAmmoCarry(int iszName)
{
	...
	{
		if ( CBasePlayerItem::ItemInfoArray[i].pszAmmo1 &&
			 !strcmp(STRING(iszName), CBasePlayerItem::ItemInfoArray[i].pszAmmo1) )
			return CBasePlayerItem::ItemInfoArray[i].iMaxAmmo1;
		if ( CBasePlayerItem::ItemInfoArray[i].pszAmmo2 &&
			 !strcmp(STRING(iszName), CBasePlayerItem::ItemInfoArray[i].pszAmmo2) )
...
void W_Precache(void)
{
	...
	CWeaponRegistry::StaticInstance().ForEach(
		[](const WeaponAtts::WACollection& atts)
		{
			...
			FOR_EACH_VEC(atts.Precache.Classes, index)
			{
				UTIL_PrecacheOther(atts.Precache.Classes[index]);
			}

			FOR_EACH_VEC(atts.Precache.Sounds, index)
			{
				PRECACHE_SOUND(atts.Precache.Sounds[index]);
			}

			FOR_EACH_VEC(atts.Precache.Models, index)
			{
...
TYPEDESCRIPTION CBasePlayerWeapon::m_SaveData[] = {
	...
	// DEFINE_FIELD( CBasePlayerWeapon, m_iClientClip, FIELD_INTEGER ), reset to zero on load so hud gets updated
	// correctly DEFINE_FIELD( CBasePlayerWeapon, m_iClientWeaponState, FIELD_INTEGER ), reset to zero on load so hud
...
CBaseEntity* CBasePlayerItem::Respawn(void)
{
	...
	CBaseEntity* pNewWeapon =
		CBaseEntity::Create(STRING(pev->classname), g_pGameRules->VecWeaponRespawnSpot(this), pev->angles, pev->owner);
...
BOOL CBasePlayerWeapon::AddPrimaryAmmo(int iCount, char* szName, int iMaxClip, int iMaxCarry)
{
	...
	if ( iIdAmmo > 0 )
	{
		...
		if ( m_pPlayer->HasPlayerItem(this) )
		{
			...
			// if the player is just getting this gun for the first time, DefaultTouch will play the "picked up gun"
			// sound for us.
