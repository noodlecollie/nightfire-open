	...
	switch ( pTask->iTask )
	{
		...
		case TASK_DIE:
		{
			if ( m_fSequenceFinished && pev->frame >= 255 )
			{
				...
				if ( !BBoxFlat() )
				{
					// a bit of a hack. If a corpses' bbox is positioned such that being left solid so that it can be
					// attacked will block the player on a slope or stairs, the corpse is made nonsolid.
					// pev->solid = SOLID_NOT;
