...
donetrigger "whatever" - can be any other triggerable entity such as another sequence, train, door, or a special case
like "die" or "remove" moveto - if set the monster first moves to this nodes position range # - only search this far to
find the target spawnflags - (stop if blocked, stop if player seen)
*/
...
void CCineMonster::PossessEntity(void)
{
	...
	if ( pTarget )
	{
		...
		// ALERT( at_aiconsole, "\"%s\" found and used (INT: %s)\n", STRING( pTarget->pev->targetname ), FBitSet(
		// pev->spawnflags, SF_SCRIPT_NOINTERRUPT )? "No" : "Yes" );
...
BOOL CCineAI::StartSequence(CBaseMonster* pTarget, int iszSeq, BOOL completeOnEmpty)
{
	if ( iszSeq == 0 && completeOnEmpty )
	{
		...
		// and remove any non-repeatable CineAI entities here ( because there is code elsewhere that handles those
		// tasks, but not until the animation sequence is finished. We have to manually take care of these things where
...
BOOL CBaseMonster::CineCleanup()
{
	...
	if ( pev->deadflag == DEAD_DYING )
	{
		...
		pev->effects |=
			EF_NOINTERP;  // Don't interpolate either, assume the corpse is positioned in its final resting place
		return FALSE;
	}
	...
	while ( !FNullEnt(pentTarget) )
	{
		...
		if ( pMonster != NULL )
		{
			if ( AcceptableSpeaker(pMonster) )
				return pMonster;
			// ALERT( at_console, "%s (%s), not acceptable\n", STRING( pMonster->pev->classname ), STRING(
			// pMonster->pev->targetname ) );
		}
		pentTarget = FIND_ENTITY_BY_TARGETNAME(pentTarget, STRING(m_iszEntity));
	}
