...
Variable sized trigger. Triggers once, then removes itself.  You must set the key "target" to the name of another object
in the level that has a matching "targetname".  If "health" is set, the trigger must be killed to activate. If notouch
is set, the trigger is only fired by other entities, not by touching. if "killtarget" is set, any objects that have a
matching "target" will be removed when the trigger is fired. if "angle" is set, the trigger will only fire when someone
is facing the direction of the angle.  Use "360" for an angle of 0. sounds 1)      secret 2)      beep beep 3) large
switch 4)
*/
...
void CBaseTrigger::CounterUse(CBaseEntity* pActivator, CBaseEntity*, USE_TYPE, float)
{
	...
	BOOL fTellActivator = (m_hActivator != 0) && FClassnameIs(m_hActivator->pev, "player") &&
...
public:
	...
	static int AddTransitionToList(
		LEVELLIST* pLevelList,
		int listCount,
		const char* pMapName,
...
void CChangeLevel::ChangeLevelNow(CBaseEntity* pActivator)
{
	...
	if ( m_changeTarget )
	{
		CFireAndDie* pFireAndDie = GetClassPtr<CFireAndDie>();
		if ( pFireAndDie )
		{
...
int CChangeLevel::AddTransitionToList(
	LEVELLIST* pLevelList,
	int listCount,
	const char* pMapName,
	const char* pLandmarkName,
	...
	while ( !FNullEnt(pentVolume) )
	{
		...
		if ( pVolume && FClassnameIs(pVolume->pev, "trigger_transition") )
		{
			if ( pVolume->Intersects(pEntity) )  // It touches one, it's in the volume
				...
				inVolume =
					0;  // Found a trigger_transition, but I don't intersect it -- if I don't find another, don't go!
		}
...
int CChangeLevel::ChangeList(LEVELLIST* pLevelList, int maxList)
{
	...
	while ( !FNullEnt(pentChangelevel) )
	{
		...
		pTrigger = GetClassPtrFromEdict<CChangeLevel>(pentChangelevel);
		if ( pTrigger )
		{
