	...
	if ( pEntity )
	{
		...
		if ( pEntity && pEntity->pev->globalname )
		{
			...
			else
			{
				...
				// ALERT( at_console, "Added global entity %s (%s)\n", STRING( pEntity->pev->classname ), STRING(
...
int DispatchRestore(edict_t* pent, SAVERESTOREDATA* pSaveData, int globalEntity)
{
	...
	if ( pEntity && pSaveData )
	{
		...
		if ( globalEntity )
		{
			...
			if ( pNewEntity )
			{
				// ALERT( at_console, "Overlay %s with %s\n", STRING( pNewEntity->pev->classname ), STRING(
				// tmpVars.classname ) );
			...
			{
				// This entity will be freed automatically by the engine.  If we don't do a restore on a matching entity
				// (below) or call EntityUpdate() to move it to this level, we haven't changed global state at all.
				return 0;
			}
		}
...
int CBaseEntity::Save(CSave& save)
{
	if ( save.WriteEntVars("ENTVARS", pev) )
		return save.WriteFields("BASE", this, m_SaveData, XASH_ARRAY_SIZE(m_SaveData));
...
int CBaseEntity::Restore(CRestore& restore)
{
	...
	if ( status )
		status = restore.ReadFields("BASE", this, m_SaveData, XASH_ARRAY_SIZE(m_SaveData));
