...
void CBaseMonster::Listen(void)
{
	...
	while ( iSound != SOUNDLIST_EMPTY )
	{
		...
		if ( pCurrentSound && (pCurrentSound->m_iType & iMySounds) &&
			 (pCurrentSound->m_vecOrigin - EarPosition()).Length() <= pCurrentSound->m_iVolume * hearingSensitivity )

		// if( ( g_pSoundEnt->m_SoundPool[iSound].m_iType & iMySounds ) && (
		// g_pSoundEnt->m_SoundPool[iSound].m_vecOrigin - EarPosition()).Length () <=
		// g_pSoundEnt->m_SoundPool[iSound].m_iVolume * hearingSensitivity )
		{
...
{
	return (pSound->m_iVolume - ((Vector(pSound->m_vecOrigin) - Vector(pev->origin)).Length()));
}
...
void CBaseMonster::Look(int iDistance)
{
	...
	if ( !FBitSet(pev->spawnflags, SF_MONSTER_PRISONER) )
	{
		...
		Vector delta =
			Vector(static_cast<float>(iDistance), static_cast<float>(iDistance), static_cast<float>(iDistance));

		// Find only monsters/clients in box, NOT limited to PVS
		int count = UTIL_EntitiesInBox(
			pList,
			100,
			Vector(pev->origin) - delta,
			Vector(pev->origin) + delta,
		...
		for ( int i = 0; i < count; i++ )
		{
				 ...
				 pSightEnt->pev->health > 0 )
			{
				...
				if ( IRelationship(pSightEnt) != R_NO && FInViewCone(pSightEnt) &&
					 !FBitSet(pSightEnt->pev->flags, FL_NOTARGET) && FVisible(pSightEnt) )
				{
					if ( pSightEnt->IsPlayer() )
					{
						if ( pev->spawnflags & SF_MONSTER_WAIT_TILL_SEEN )
						{
							...
							// don't link this client in the list if the monster is wait till seen and the player isn't
							// facing the monster
...
CSound* CBaseMonster::PBestScent(void)
{
	...
	while ( iThisScent != SOUNDLIST_EMPTY )
	{
		...
		if ( pSound->FIsScent() )
		{
			flDist = (Vector(pSound->m_vecOrigin) - Vector(pev->origin)).Length();
...
void CBaseMonster::RouteSimplify(CBaseEntity* pTargetEnt)
{
	...
	for ( i = 0; i < count - 1; i++ )
	{
		...
		if ( !ShouldSimplify(m_Route[m_iRouteIndex + i].iType) )
		{
			...
			CheckLocalMove(vecStart, m_Route[m_iRouteIndex + i + 1].vecLocation, pTargetEnt, NULL) == LOCALMOVE_VALID )
		{
			// Skip vert
			continue;
		}
...
void CBaseMonster::CheckAttacks(CBaseEntity* pTarget, float flDist)
{
	...
	vec2LOS = (Vector(pTarget->pev->origin) - Vector(pev->origin)).Make2D();
...
int CBaseMonster::CheckEnemy(CBaseEntity* pEnemy)
{
	...
	flDistToEnemy = (vecEnemyPos - Vector(pev->origin)).Length();
	vecEnemyPos.z += pEnemy->pev->size[VEC3_Z] * 0.5f;

	// distance to enemy's head
	float flDistToEnemy2 = (vecEnemyPos - Vector(pev->origin)).Length();
	...
	else
	{
		...
		vecEnemyPos.z -= pEnemy->pev->size[VEC3_Z];
		flDistToEnemy2 = (vecEnemyPos - Vector(pev->origin)).Length();
	...
	if ( HasConditions(bits_COND_SEE_ENEMY) )
	{
		...
		if ( pEnemy->pev->velocity != Vector(0, 0, 0) )
		{
			...
			m_vecEnemyLKP = m_vecEnemyLKP - Vector(pEnemy->pev->velocity) * RANDOM_FLOAT(-0.05f, 0.0f);
		}
	}
...
int CBaseMonster::CheckLocalMove(const Vector& vecStart, const Vector& vecEnd, CBaseEntity* pTarget, float* pflDist)
{
	...
	*/
	...
	if ( iReturn == LOCALMOVE_VALID && !(pev->flags & (FL_FLY | FL_SWIM)) &&
		 (!pTarget || (pTarget->pev->flags & FL_ONGROUND)) )
	{
		// The monster can move to a spot UNDER the target, but not to it. Don't try to triangulate, go directly to the
		// node graph. UNDONE: Magic # 64 -- this used to be pev->size.z but that won't work for small creatures like
		// the headcrab
		if ( fabs(vecEnd[VEC3_Z] - pev->origin[VEC3_Z]) > 64 )
		{
		...
		if ( !(m_Route[m_iRouteIndex].iType & bits_MF_IS_GOAL) )
		{
				 ...
				 (m_Route[m_iRouteIndex + 1].iType & bits_MF_TO_NODE) == bits_MF_TO_NODE )
			{
				...
				if ( iLink >= 0 && WorldGraph.m_pLinkPool[iLink].m_pLinkEnt != NULL )
				{
					...
					if ( WorldGraph.HandleLinkEnt(
							 iSrcNode,
							 WorldGraph.m_pLinkPool[iLink].m_pLinkEnt,
							 m_afCapability,
...
BOOL CBaseMonster::BuildRoute(const Vector& vecGoal, int iMoveFlag, CBaseEntity* pTarget)
{
	...
	// try to triangulate around any obstacles.
		...
		iLocalMove != LOCALMOVE_INVALID_DONT_TRIANGULATE &&
		FTriangulate(pev->origin, vecGoal, flDist, pTarget, &vecApex) )
	{
...
//=========================================================
BOOL CBaseMonster::FTriangulate(
	...
	Vector* pApex)
{
	...
	Vector vecFarSide;  // the spot that we'll move to after hitting the triangulated point, before moving on to our
						// normal goal.
	int i;
	float sizeX, sizeZ;
	...
	vecLeft = Vector(pev->origin) + (vecForward * (flDist + sizeX)) - vecDir * (sizeX * 3);
	vecRight = Vector(pev->origin) + (vecForward * (flDist + sizeX)) + vecDir * (sizeX * 3);

	if ( pev->movetype == MOVETYPE_FLY )
	{
		vecTop = Vector(pev->origin) + (vecForward * flDist) + (vecDirUp * sizeZ * 3);
		vecBottom = Vector(pev->origin) + (vecForward * flDist) - (vecDirUp * sizeZ * 3);
	}
...
//	DrawRoute( pev, m_Route, m_iRouteIndex, 0, 200, 0 );
	...
	flWaypointDist = (m_Route[m_iRouteIndex].vecLocation - Vector(pev->origin)).Length2D();
		 ...
		 LOCALMOVE_VALID )
	{
		...
		if ( pBlocker && m_moveWaitTime > 0 && pBlocker->IsMoving() && !pBlocker->IsPlayer() &&
			 (gpGlobals->time - m_flMoveWaitFinished) > 3.0 )
		{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					// ALERT( at_aiconsole, "%f, %f, %f\n", pev->origin.z, ( pev->origin + ( vecDir * flCheckDist ) ).z,
					// m_Route[m_iRouteIndex].vecLocation.z );
				}
				return;
			}
		}
...
int CBaseMonster::IRelationship(CBaseEntity* pTarget)
{
	static int iEnemy[14][14] = {
		//   NONE	 MACH	 PLYR	 HPASS	 HMIL	 AMIL	 APASS	 AMONST	APREY	 APRED	 INSECT	PLRALY PBWPN
		//   ABWPN
...
BOOL CBaseMonster::FindCover(Vector vecThreat, Vector vecViewOffset, float flMinDist, float flMaxDist)
{
	...
	for ( i = 0; i < WorldGraph.m_cNodes; i++ )
	{
		...
		WorldGraph.m_iLastCoverSearch =
			nodeNumber + 1;  // next monster that searches for cover node will start where we left off here.
		...
		{
			UTIL_TraceLine(
				node.m_vecOrigin + vecViewOffset,
				vecLookersOffset,
				ignore_monsters,
				ignore_glass,
				ENT(pev),
			...
			{
				// ..and is also closer to me than the threat, or the same distance from myself and the threat the node
				// is good.
	...
	while ( pNextEnt != NULL )
	{
		if ( pNextEnt->IsAlive() )
		{
			if ( IRelationship(pNextEnt) > iBestRelationship )
			{
				...
				iNearest = static_cast<int>((Vector(pNextEnt->pev->origin) - Vector(pev->origin)).Length());
				pReturn = pNextEnt;
			}
			else if ( IRelationship(pNextEnt) == iBestRelationship )
			{
				...
				iDist = static_cast<int>((Vector(pNextEnt->pev->origin) - Vector(pev->origin)).Length());
...
void CBaseMonster::MakeIdealYaw(Vector vecTarget)
{
	...
	if ( m_movementActivity == ACT_STRAFE_LEFT )
	{
		...
		pev->ideal_yaw = UTIL_VecToYaw(vecProjection - Vector(pev->origin));
	}
	...
	{
		pev->ideal_yaw = UTIL_VecToYaw(vecTarget - Vector(pev->origin));
	}
}
...
float CBaseMonster::ChangeYaw(int yawSpeed)
{
	...
	current = UTIL_AngleMod(pev->angles[YAW]);
	ideal = pev->ideal_yaw;
	if ( current != ideal )
	{
		...
		pev->angles[YAW] = UTIL_AngleMod(current + move);

		// turn head in desired direction only if they have a turnable head
		if ( m_afCapability & bits_CAP_TURN_HEAD )
		{
			float yaw = pev->ideal_yaw - pev->angles[YAW];
...
void CBaseMonster::SetEyePosition(void)
{
	...
	VectorCopy(vecEyePosition, pev->view_ofs);
	...
	switch ( pEvent->event )
	{
		...
		case SCRIPT_EVENT_NOT_DEAD:
			if ( m_MonsterState == MONSTERSTATE_SCRIPT )
			{
				...
				// This is for life/death sequences where the player can determine whether a character is dead or alive
				// after the script
...
Vector CBaseMonster::GetGunPosition()
{
	...
	Vector vecSrc = Vector(pev->origin) + Vector(gpGlobals->v_forward) * m_HackedGunPos.y +
		Vector(gpGlobals->v_right) * m_HackedGunPos.x + Vector(gpGlobals->v_up) * m_HackedGunPos.z;
...
int CBaseMonster::FindHintNode(void)
{
	...
	for ( i = 0; i < WorldGraph.m_cNodes; i++ )
	{
		...
		if ( node.m_sHintType )
		{
			// this node has a hint. Take it if it is visible, the monster likes it, and the monster has an animation to
			// match the hint's activity.
			if ( FValidateHintType(node.m_sHintType) )
			{
				if ( !node.m_sHintActivity || LookupActivity(node.m_sHintActivity) != ACTIVITY_NOT_AVAILABLE )
				{
					...
					if ( tr.flFraction == 1.0 )
					{
						WorldGraph.m_iLastActiveIdleSearch =
							nodeNumber + 1;  // next monster that searches for hint nodes will start where we left off.
...
void CBaseMonster::ReportAIState(void)
{
	...
	if ( (int)m_MonsterState < SIZE_OF_ARRAY(pStateNames) )
...
BOOL CBaseMonster::FCheckAITrigger(void)
{
	...
	switch ( m_iTriggerCondition )
	{
		...
		case AITRIGGER_SEEPLAYER_NOT_IN_COMBAT:
			if ( HasConditions(bits_COND_SEE_CLIENT) && m_MonsterState != MONSTERSTATE_COMBAT &&
				 m_MonsterState != MONSTERSTATE_PRONE && m_MonsterState != MONSTERSTATE_SCRIPT )
			{
				fFireTarget = TRUE;
			}
			break;
...
BOOL CBaseMonster::FindLateralCover(const Vector& vecThreat, const Vector& vecViewOffset)
{
	...
	UTIL_MakeVectors(pev->angles);
	vecStepRight = Vector(gpGlobals->v_right) * COVER_DELTA;
	vecStepRight.z = 0;
	...
	for ( i = 0; i < COVER_CHECKS; i++ )
	{
		...
		// it's faster to check the SightEnt's visibility to the potential spot than to check the local move, so we do
		// that first.
		UTIL_TraceLine(
			vecThreat + vecViewOffset,
			vecLeftTest + pev->view_ofs,
			ignore_monsters,
			ignore_glass,
			ENT(pev) /*pentIgnore*/,
			&tr);
		...
		if ( tr.flFraction != 1.0 )
		{
			if ( FValidateCover(vecLeftTest) &&
				 CheckLocalMove(pev->origin, vecLeftTest, NULL, NULL) == LOCALMOVE_VALID )
			{
		...
		if ( tr.flFraction != 1.0 )
		{
			if ( FValidateCover(vecRightTest) &&
				 CheckLocalMove(pev->origin, vecRightTest, NULL, NULL) == LOCALMOVE_VALID )
			{
...
Vector CBaseMonster::ShootAtEnemy(const Vector& shootOrigin)
{
	...
	if ( pEnemy )
	{
		return ((pEnemy->BodyTarget(shootOrigin) - Vector(pEnemy->pev->origin)) + m_vecEnemyLKP - shootOrigin)
			.Normalize();
	}
	else
	{
...
void CBaseMonster::PlayScriptedSentence(
...
BOOL CBaseMonster::BBoxFlat(void)
{
	...
	flXSize = pev->size[VEC3_X] / 2;
	flYSize = pev->size[VEC3_Y] / 2;

	vecPoint.x = pev->origin[VEC3_X] + flXSize;
	vecPoint.y = pev->origin[VEC3_Y] + flYSize;
	vecPoint.z = pev->origin[VEC3_Z];

	UTIL_TraceLine(vecPoint, vecPoint - Vector(0, 0, 100), ignore_monsters, ENT(pev), &tr);
	flLength = (vecPoint - Vector(tr.vecEndPos)).Length();

	vecPoint.x = pev->origin[VEC3_X] - flXSize;
	vecPoint.y = pev->origin[VEC3_Y] - flYSize;

	UTIL_TraceLine(vecPoint, vecPoint - Vector(0, 0, 100), ignore_monsters, ENT(pev), &tr);
	flLength2 = (vecPoint - Vector(tr.vecEndPos)).Length();
	...
	vecPoint.x = pev->origin[VEC3_X] - flXSize;
	vecPoint.y = pev->origin[VEC3_X] + flYSize;
	UTIL_TraceLine(vecPoint, vecPoint - Vector(0, 0, 100), ignore_monsters, ENT(pev), &tr);
	flLength2 = (vecPoint - Vector(tr.vecEndPos)).Length();
...
BOOL CBaseMonster::GetEnemy(void)
{
	...
	if ( HasConditions(bits_COND_SEE_HATE | bits_COND_SEE_DISLIKE | bits_COND_SEE_NEMESIS) )
	{
		...
		{
			// DO NOT mess with the monster's m_hEnemy pointer unless the schedule the monster is currently running will
			// be interrupted by COND_NEW_ENEMY. This will eliminate the problem of monsters getting a new enemy while
			// they are in a schedule that doesn't care, and then not realizing it by the time they get to a schedule
...
CBaseEntity* CBaseMonster::DropItem(const char* pszItemName, const Vector& vecPos, const Vector& vecAng)
{
	...
	if ( pItem )
	{
		...
		VectorCopy(pev->velocity, pItem->pev->velocity);
		VectorCopy(Vector(0, RANDOM_FLOAT(0, 100), 0), pItem->pev->avelocity);
