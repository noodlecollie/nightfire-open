...
{
	// ALERT( at_console, "%s %.2f %d %d\n", STRING( pev->targetname ), pev->origin.z, m_MonsterState,
	// m_IdealMonsterState );
	pev->nextthink = gpGlobals->time + 0.1;

	// ALERT( at_console, "%s %d %d %d %f %f\n", STRING( pev->targetname ), pev->sequence, m_iGoalAnim, m_iDir,
	// pev->framerate, pev->health );
	...
	switch ( pEvent->event )
	{
		case 1:
			// bang
			{
				...
				// Vector vecSrc = pev->origin + m_flTapRadius * Vector( cos( pev->angles.y * ( 3.14192653 / 180.0 ) ),
		...
		case 3:
			// start killing swing
			m_iHitDmg = 200;
			// UTIL_EmitAmbientSound( ENT( pev ), pev->origin + Vector( 0, 0, MyHeight() ),
			// "tentacle/te_swing1.wav", 1.0, ATTN_NORM, 0, 100 );
			break;
		case 4:
			// end killing swing
			m_iHitDmg = 25;
		...
		case 5:
			// just "whoosh" sound
			// UTIL_EmitAmbientSound( ENT( pev ), pev->origin + Vector( 0, 0, MyHeight() ),
			// "tentacle/te_swing2.wav", 1.0, ATTN_NORM, 0, 100 );
			break;
		case 2:
			// tap scrape
		case 6:
			// light tap
			{
				Vector vecSrc = pev->origin +
					m_flTapRadius *
						Vector(cos(pev->angles.y * (M_PI / 180.0)), sin(pev->angles.y * (M_PI / 180.0)), 0.0);
