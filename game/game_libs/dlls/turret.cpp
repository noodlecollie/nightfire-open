...
void CBaseTurret::KeyValue(KeyValueData* pkvd)
{
	...
	else if ( FStrEq(pkvd->szKeyName, "turnrate") )
	{
		...
		FStrEq(pkvd->szKeyName, "style") || FStrEq(pkvd->szKeyName, "height") || FStrEq(pkvd->szKeyName, "value1") ||
		FStrEq(pkvd->szKeyName, "value2") || FStrEq(pkvd->szKeyName, "value3") )
		pkvd->fHandled = TRUE;
	else
...
void CTurret::Spawn()
{
	...
	UTIL_SetSize(
		pev,
		Vector(-32.0f, -32.0f, static_cast<float>(-m_iRetractHeight)),
		Vector(32.0f, 32.0f, static_cast<float>(m_iRetractHeight)));
...
void CMiniTurret::Spawn()
{
	...
	UTIL_SetSize(
		pev,
		Vector(-16.0f, -16.0f, static_cast<float>(-m_iRetractHeight)),
		Vector(16.0f, 16.0f, static_cast<float>(m_iRetractHeight)));
...
int CBaseTurret::MoveTurret(void)
{
	...
	{
		float flDir = m_vecGoalAngles.x > m_vecCurAngles.x ? 1.0f : -1.0f;
	...
	if ( m_vecCurAngles.y != m_vecGoalAngles.y )
	{
		...
		float flDist = fabsf(m_vecGoalAngles.y - m_vecCurAngles.y);
