...
/// <summary>Updates mouseThreadActive using the global variables mouseactive, iVisibleMouse and m_bRawInput. Should be
/// called after any of these is changed.</summary> <remarks>Has to be interlocked manually by programmer! Use
/// MouseThread_ActiveLock_Enter and MouseThread_ActiveLock_Exit.</remarks>
...
void IN_ScaleMouse(float* x, float* y)
{
	...
	if ( m_customaccel->value != 0 )
	{
		...
		*y *= accelerated_sensitivity;
		...
		// This means that they will need to up their value for m_customaccel_scale greatly (>40x) since m_pitch/yaw
		// default
		//  to 0.022
...
void GoldSourceInput::IN_Init(void)
{
	...
	m_mousethread_sleep =
		gEngfuncs.pfnRegisterVariable("m_mousethread_sleep", "1", FCVAR_ARCHIVE);  // default to less than 1000 Hz
