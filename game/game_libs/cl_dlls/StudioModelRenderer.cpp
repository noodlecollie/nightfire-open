...
====================
*/
void CStudioModelRenderer::StudioCalcBoneAdj(
	float dadt,
	float* adj,
	...
	byte mouthopen)
{
	...
	for ( j = 0; j < m_pStudioHeader->numbonecontrollers; j++ )
	{
		...
		switch ( pbonecontroller[j].type & STUDIO_TYPES )
		{
			...
			case STUDIO_YR:
			case STUDIO_ZR:
				adj[j] = value * (static_cast<float>(M_PI) / 180.0f);
				break;
			case STUDIO_X:
			case STUDIO_Y:
			case STUDIO_Z:
...
====================
*/
void CStudioModelRenderer::StudioCalcBoneQuaterion(
	int frame,
	float s,
	mstudiobone_t* pbone,
	mstudioanim_t* panim,
...
====================
*/
void CStudioModelRenderer::StudioCalcBonePosition(
	int frame,
	float s,
	mstudiobone_t* pbone,
	mstudioanim_t* panim,
		...
		if ( panim->offset[j] != 0 )
		{
			...
			*/
			...
			else
			{
				...
				if ( panimvalue->num.total <= k + 1 )
				{
					pos[j] += (panimvalue[panimvalue->num.valid].value * (1.0f - s) +
...
void CStudioModelRenderer::StudioPlayerBlend(mstudioseqdesc_t* pseqdesc, int* pBlend, float* pPitch)
{
	...
	*pBlend = static_cast<int>(*pPitch * 3);
	...
	else
	{
		...
		{
			*pBlend = static_cast<int>(
				255 * (*pBlend - pseqdesc->blendstart[0]) / (pseqdesc->blendend[0] - pseqdesc->blendstart[0]));
		}
...
void CStudioModelRenderer::StudioSetUpTransform(int trivial_accept)
{
	...
	if ( m_pCurrentEntity->curstate.movetype == MOVETYPE_STEP )
	{
		...
		{
			f = static_cast<float>(m_clTime - m_pCurrentEntity->curstate.animtime) /
				(m_pCurrentEntity->curstate.animtime - m_pCurrentEntity->latched.prevanimtime);
	...
	Matrix3x4_AnglesToMatrix(angles, (*m_protationmatrix));
...
float CStudioModelRenderer::StudioEstimateInterpolant(void)
{
	...
	{
		dadt = static_cast<float>((m_clTime - m_pCurrentEntity->curstate.animtime) / 0.1f);
		if ( dadt > 2.0f )
		{
			dadt = 2.0f;
		}
...
====================
*/
void CStudioModelRenderer::StudioCalcRotations(
	float pos[][3],
	vec4_t* q,
	...
	float f)
{
	...
	// Con_DPrintf( "%d %.4f %.4f %.4f %.4f %d\n", m_pCurrentEntity->curstate.sequence, m_clTime,
	// m_pCurrentEntity->animtime, m_pCurrentEntity->frame, f, frame );

	// Con_DPrintf( "%f %f %f\n", m_pCurrentEntity->angles[ROLL], m_pCurrentEntity->angles[PITCH],
...
void CStudioModelRenderer::StudioFxTransform(cl_entity_t* ent, float transform[3][4])
{
	switch ( ent->curstate.renderfx )
	{
		...
		case kRenderFxHologram:
			if ( gEngfuncs.pfnRandomLong(0, 49) == 0 )
			{
				...
				if ( axis == 1 )  // Choose between x & z
					axis = 2;
				VectorScale(transform[axis], gEngfuncs.pfnRandomFloat(1, 1.484f), transform[axis]);
			}
		...
		case kRenderFxExplode:
		{
			...
			scale = static_cast<float>(1.0f + (m_clTime - ent->curstate.animtime) * 10.0f);
...
float CStudioModelRenderer::StudioEstimateFrame(mstudioseqdesc_t* pseqdesc)
{
	...
	if ( m_fDoInterp )
	{
		...
		{
			dfdt =
				(m_clTime - m_pCurrentEntity->curstate.animtime) * m_pCurrentEntity->curstate.framerate * pseqdesc->fps;
		}
	}
	else
	{
...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	pseqdesc =
		(mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pCurrentEntity->curstate.sequence;
	...
	if ( pseqdesc->numblends > 1 )
	{
		...
		s = (m_pCurrentEntity->curstate.blending[0] * dadt +
			 m_pCurrentEntity->latched.prevblending[0] * (1.0f - dadt)) /
			255.0f;
		...
		if ( pseqdesc->numblends == 4 )
		{
			...
			s = (m_pCurrentEntity->curstate.blending[1] * dadt +
				 m_pCurrentEntity->latched.prevblending[1] * (1.0f - dadt)) /
				255.0f;
		 ...
		 (m_pCurrentEntity->latched.prevsequence < m_pStudioHeader->numseq) )
	{
		...
		s = static_cast<float>(1.0f - (m_clTime - m_pCurrentEntity->latched.sequencetime) / 0.2f);
	...
	for ( i = 0; i < m_pStudioHeader->numbones; i++ )
	{
		Matrix3x4_FromQuat(bonematrix, q[i]);
...
void CStudioModelRenderer::StudioSaveBones(void)
{
	...
	for ( i = 0; i < m_pStudioHeader->numbones; i++ )
	{
		PlatformLib_StrCpy(m_nCachedBoneNames[i], sizeof(m_nCachedBoneNames[i]), pbones[i].name);
		Matrix3x4_Copy(m_rgCachedBoneTransform[i], (*m_pbonetransform)[i]);
		Matrix3x4_Copy(m_rgCachedLightTransform[i], (*m_plighttransform)[i]);
	}
}
...
void CStudioModelRenderer::StudioMergeBones(model_t* subModel)
{
	...
	for ( i = 0; i < m_pStudioHeader->numbones; i++ )
	{
		for ( j = 0; j < m_nCachedBones; j++ )
		{
			...
			{
				Matrix3x4_Copy((*m_pbonetransform)[i], m_rgCachedBoneTransform[j]);
				Matrix3x4_Copy((*m_plighttransform)[i], m_rgCachedLightTransform[j]);
				break;
			}
		}
		if ( j >= m_nCachedBones )
...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if ( m_pCurrentEntity->curstate.renderfx == kRenderFxDeadPlayer )
	{
		...
		deadplayer = *(IEngineStudio.GetPlayerState(
			...
			1));  // cl.frames[cl.parsecount & CL_UPDATE_MASK].playerstate[m_pCurrentEntity->curstate.renderamt - 1];
	...
	if ( flags & STUDIO_EVENTS )
	{
		...
		if ( m_pCurrentEntity->index > 0 )
		{
			...
			{
				VectorCopy(m_pCurrentEntity->attachment[index], ent->attachment[index]);
			}
		}
	}
...
void CStudioModelRenderer::StudioEstimateGait(entity_state_t* pplayer)
{
	...
	{
		m_pPlayerInfo->gaityaw = (atan2f(est_velocity[1], est_velocity[0]) * 180 / static_cast<float>(M_PI));
...
void CStudioModelRenderer::StudioProcessGait(entity_state_t* pplayer)
{
	...
	m_pCurrentEntity->curstate.blending[0] = static_cast<byte>(iBlend);
	...
	// adjust torso
	m_pCurrentEntity->curstate.controller[0] = static_cast<byte>(((flYaw / 4.0f) + 30) / (60.0f / 255.0f));
	m_pCurrentEntity->curstate.controller[1] = static_cast<byte>(((flYaw / 4.0f) + 30) / (60.0f / 255.0f));
	m_pCurrentEntity->curstate.controller[2] = static_cast<byte>(((flYaw / 4.0f) + 30) / (60.0f / 255.0f));
	m_pCurrentEntity->curstate.controller[3] = static_cast<byte>(((flYaw / 4.0f) + 30) / (60.0f / 255.0f));
