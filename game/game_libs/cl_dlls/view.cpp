...
void V_GetChaseOrigin(float* angles, float* origin, float distance, float* returnvec)
{
	...
	/*	if( ent )
		{
			gEngfuncs.Con_Printf( "Trace loops %i , entity %i, model %s, solid %i\n",(8-maxLoops),ent->curstate.number,
	   ent->model->name , ent->curstate.solid );
		} */
...
void V_GetChasePos(int target, float* cl_angles, float* origin, float* angles)
{
	...
	if ( gHUD.m_Spectator.m_autoDirector->value )
	{
		if ( g_iUser3 )
			...
			V_GetDirectedChasePosition(ent, (cl_entity_t*)(~0), angles, origin);
	}
	else
	{
...
void V_Init(void)
{
	...
	cl_bobcycle =
		gEngfuncs.pfnRegisterVariable("cl_bobcycle", "0.8", 0);  // best default for my experimental gun wag (sjb)
