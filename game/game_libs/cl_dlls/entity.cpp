...
void DLLEXPORT HUD_TxferPredictionData(
	struct entity_state_s* ps,
...
Because we can predict an arbitrary number of frames before the server responds with an update, we need to be able to
copy client side prediction data in from the state that the server ack'd receiving, which can be anywhere along the
predicted frame path ( i.e., we could predict 20 frames into the future and the server ack's up through 10 of those
frames, so we need to copy persistent client-side only state from the 10th predicted frame to the slot the server update
is occupying.
=========================
*/
void DLLEXPORT HUD_TxferPredictionData(
...
void DLLEXPORT HUD_StudioEvent(const struct mstudioevent_s* event, const struct cl_entity_s* entity)
{
	switch ( event->event )
	{
		case MDLEVENT_MUZZLEFLASH0:  // Muzzle flash from attachment 0
		{
			gEngfuncs.pEfxAPI->R_MuzzleFlash(
				reinterpret_cast<const float*>(&entity->attachment[0]),
				atoi(event->options));

			break;
		}

		case MDLEVENT_MUZZLEFLASH1:  // Muzzle flash from attachment 1
		{
			gEngfuncs.pEfxAPI->R_MuzzleFlash(
				reinterpret_cast<const float*>(&entity->attachment[1]),
				atoi(event->options));

			break;
		}

		case MDLEVENT_MUZZLEFLASH2:  // Muzzle flash from attachment 2
		{
			gEngfuncs.pEfxAPI->R_MuzzleFlash(
				reinterpret_cast<const float*>(&entity->attachment[2]),
				atoi(event->options));

			break;
		}

		case MDLEVENT_MUZZLEFLASH3:  // Muzzle flash from attachment 3
		{
			gEngfuncs.pEfxAPI->R_MuzzleFlash(
				reinterpret_cast<const float*>(&entity->attachment[3]),
				atoi(event->options));

			break;
		}

		case MDLEVENT_SPARK0:  // Spark from attachment 0
		{
			gEngfuncs.pEfxAPI->R_SparkEffect(
				reinterpret_cast<const float*>(&entity->attachment[0]),
				atoi(event->options),
				-100,
				100);
		...
		case MDLEVENT_SOUND0:  // Play a client-side sound at attachment 0 origin
			// (note that the sound does not follow the model)
			{
		...
		case MDLEVENT_SOUND_AMBIENT:  // Play an ambient client-side sound
		{
			gEngfuncs.pfnPlaySoundByName(event->options, 1.0f);
			break;
		}
					...
					if ( pTemp->flags & (FTENT_GRAVITY | FTENT_SLOWGRAVITY) )
					{
						...
						if ( traceNormal[2] > 0.9 )  // Hit floor?
						{
							if ( pTemp->entity.baseline.origin[2] <= 0 &&
								 pTemp->entity.baseline.origin[2] >= gravity * 3 )
							{
								damp = 0;  // Stop
								pTemp->flags &=
									~(FTENT_ROTATE | FTENT_GRAVITY | FTENT_SLOWGRAVITY | FTENT_COLLIDEWORLD |
									  FTENT_SMOKETRAIL);
