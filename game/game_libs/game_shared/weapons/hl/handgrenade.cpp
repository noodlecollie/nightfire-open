...
int CHandGrenade::GetItemInfo(ItemInfo* p)
{
	...
	p->pszAmmo1 = AmmoDef_HandGrenade.AmmoName;
...
void CHandGrenade::WeaponIdle(void)
{
	...
	if ( m_flStartThrow )
	{
		Vector angThrow(Vector(m_pPlayer->pev->v_angle) + Vector(m_pPlayer->pev->punchangle));
		...
		Vector vecThrow(Vector(gpGlobals->v_forward) * flVel + Vector(m_pPlayer->pev->velocity));
		...
		if ( !m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] )
		{
			...
			m_flTimeWeaponIdle = m_flNextSecondaryAttack = m_flNextPrimaryAttack =
				GetNextAttackDelay(0.5);  // ensure that the animation can finish playing
		}
		return;
	}
	...
	if ( m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] )
	{
		...
		if ( flRand <= 0.75 )
		{
			...
			m_flTimeWeaponIdle = UTIL_WeaponTimeBase() +
				UTIL_SharedRandomFloat(m_pPlayer->random_seed, 10, 15);  // how long till we do this again.
		}
		else
		{
