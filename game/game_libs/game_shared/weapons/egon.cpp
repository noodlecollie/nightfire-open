...
#define EGON_DISCHARGE_INTERVAL 0.1f
...
void CEgon::Attack(void)
{
	...
	switch ( m_fireState )
	{
		case FIRE_OFF:
		{
			...
			event << Flags(flags) << Invoker(m_pPlayer->edict()) << EventIndex(m_usEgonFire) << IntParam1(m_fireState)
				  << IntParam2(m_fireMode) << BoolParam1(true);
		...
		case FIRE_CHARGE:
		{
			...
			if ( pev->tuser1 <= UTIL_WeaponTimeBase() )
			{
				...
				event << Flags(flags) << Invoker(m_pPlayer->edict()) << EventIndex(m_usEgonFire)
					  << IntParam1(m_fireState) << IntParam2(m_fireMode);
...
void CEgon::UpdateEffect(const Vector& startPoint, const Vector& endPoint, float timeBlend)
{
	...
	if ( m_fireMode == FIRE_WIDE )
		m_pBeam->SetColor(
			(int)(30 + (25 * timeBlend)),
			(int)(30 + (30 * timeBlend)),
			(int)(64 + 80 * fabs(sin(gpGlobals->time * 10))));
	else
		m_pBeam->SetColor(
			(int)(60 + (25 * timeBlend)),
			(int)(120 + (30 * timeBlend)),
			(int)(64 + 80 * fabs(sin(gpGlobals->time * 10))));
...
void CEgon::EndAttack(void)
{
	...
	event << Flags(FEV_GLOBAL | FEV_RELIABLE) << Invoker(m_pPlayer->edict()) << EventIndex(m_usEgonStop)
		  << Origin(m_pPlayer->pev->origin) << Angles(m_pPlayer->pev->angles) << IntParam1(bMakeNoise)
		  << BoolParam1(true);
