...
CPlayerBitVec g_SentGameRulesMasks[VOICE_MAX_PLAYERS];  // These store the masks we last sent to each client so we can
														// determine if
...
static void VoiceServerDebug(char const* pFmt, ...)
{
	...
	PlatformLib_VSNPrintF(msg, sizeof(msg), pFmt, marker);
	va_end(marker);
