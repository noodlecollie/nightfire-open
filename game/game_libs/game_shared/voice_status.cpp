...
void ForEachBannedPlayer(char id[16])
{
	...
	sprintf(
		str,
		"Ban %d: %2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x\n",
...
int CVoiceStatus::VidInit()
{
	...
	{
		m_pAckBitmap->setColor(
			Color(255, 255, 255, 135));  // Give just a tiny bit of translucency so software draws correctly.
	}
...
void CVoiceStatus::UpdateSpeakerStatus(int entindex, qboolean bTalking)
{
	...
	{
		char msg[256];
		PlatformLib_SNPrintF(msg, sizeof(msg), "CVoiceStatus::UpdateSpeakerStatus: ent %d talking = %d\n", entindex, bTalking);
		...
		if ( bTalking )
		{
			...
			if ( !pLabel )
			{
				if ( pLabel = GetFreeVoiceLabel() )
				{
					...
					char paddedName[512];
					PlatformLib_SNPrintF(paddedName, sizeof(paddedName), "%s   ", info.name);
...
void CVoiceStatus::UpdateServerState(bool bForce)
{
	...
	if ( bForce || m_bServerModEnable != bCVarModEnable )
	{
		...
		char str[256];
		PlatformLib_SNPrintF(str, sizeof(str), "VModEnable %d", m_bServerModEnable);
		ServerCmd(str);
