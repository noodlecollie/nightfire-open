...
void PM_Duck(void)
{
	...
	if ( (pmove->cmd.buttons & IN_DUCK) || (pmove->bInDuck) || (pmove->flags & FL_DUCKING) )
	{
		if ( pmove->cmd.buttons & IN_DUCK )
		{
			...
			time = MAX_OF(0.0f, (1.0f - (float)pmove->flDuckTime / 1000.0f));
			...
			if ( pmove->bInDuck )
			{
				...
				else
				{
					...
					pmove->view_ofs[2] =
						((VEC_DUCK_VIEW_HEIGHT - fMore) * duckFraction) + (VEC_VIEW * (1 - duckFraction));
				}
			}
		}
		else
...
void PM_CheckParamters(void)
{
	...
	if ( !pmove->dead )
	{
		...
		// Set up view angles.
		pmove->angles[ROLL] =
			PM_CalcRoll(v_angle, pmove->velocity, pmove->movevars->rollangle, pmove->movevars->rollspeed) * 4;
