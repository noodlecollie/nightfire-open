...
#ifdef __MINGW32__
	pmtrace_t* (*PM_PlayerTraceEx_real)(
		pmtrace_t* retvalue,
		float* start,
		float* end,
		int traceFlags,
...
static inline pmtrace_t
PM_PlayerTrace_wrap(float* start, float* end, int traceFlags, int ignore_pe, playermove_t* pmove)
{
...
static inline pmtrace_t
PM_PlayerTraceEx_wrap(float* start, float* end, int traceFlags, int (*pfnIgnore)(physent_t* pe), playermove_t* pmove)
{
	_pm_globaltmp = pmove->touchindex[MAX_PHYSENTS - 1];
	pmove->PM_PlayerTraceEx_real(&_pm_globalresult, start, end, traceFlags, pfnIgnore);
	return _pm_globalresult;
}
#define PM_PlayerTrace(a, b, c, d) \
	touchindex[MAX_PHYSENTS - 1] = PM_PlayerTrace_wrap(a, b, c, d, pmove); \
	pmove->touchindex[MAX_PHYSENTS - 1] = _pm_globaltmp
#define PM_PlayerTraceEx(a, b, c, d) \
	touchindex[MAX_PHYSENTS - 1] = PM_PlayerTraceEx_wrap(a, b, c, d, pmove); \
	pmove->touchindex[MAX_PHYSENTS - 1] = _pm_globaltmp
#endif
