...
namespace JbnLib.Wrappers
{
  public class BondDed
  {
    ...
    public BondDed(string path) => this._Executable = new FileInfo(path);
    ...
    public void Start(BondDedArgs args) => this.Start(args.ToString(), false);

    public void Start(BondDedArgs args, bool waitforexit) => this.Start(args.ToString(), waitforexit);

    public void Start(string args, bool waitforexit)
    {
      if (!this._Executable.Exists)
        Messages.ThrowException("Wrappers.BondDed.Start(string, bool)", Messages.FILE_NOT_FOUND + this._Executable.FullName);
