...
namespace JbnLib.Wrappers
{
  public class Bsp
  {
    ...
    public Bsp(string path) => this._Executable = new FileInfo(path);
    ...
    public string Compile(string map, SharedArgs shared, BspArgs args) => this.Compile(map, shared.ToString() + args.ToString(), false);

    public string Compile(string map, SharedArgs shared, BspArgs args, bool nowindow) => this.Compile(map, shared.ToString() + args.ToString(), nowindow);

    public string Compile(string map, BspArgs args) => this.Compile(map, args.ToString(), false);

    public string Compile(string map, BspArgs args, bool nowindow) => this.Compile(map, args.ToString(), nowindow);

    public string Compile(string map, string args, bool nowindow)
    {
      if (!this._Executable.Exists)
        Messages.ThrowException("Wrappers.Bsp.Compile(string, string, bool)", Messages.FILE_NOT_FOUND + this._Executable.FullName);
      FileInfo fileInfo = new FileInfo(map);
      if (!fileInfo.Exists)
        Messages.ThrowException("Wrappers.Bsp.Compile(string, string, bool)", Messages.FILE_NOT_FOUND + map);
      ...
      if (File.Exists(fileInfo.FullName + ".log"))
      {
        ...
        while (streamReader.Peek() != -1)
        {
          ...
          if (str.StartsWith("Error: ") || str.StartsWith("ERROR: "))
            Messages.ThrowException("bbsp.exe", str.Replace("Error: ", "").Replace("ERROR: ", ""));
        }
        streamReader.Close();
      }
