...
{
	// all: never allow an empty path, as for gamedir it would access the parent directory and a non-gamedir path it is
	// just useless
...
static void FS_WriteGameInfo(const char* filepath, gameinfo_t* GameInfo)
{
	...
	FS_Printf(
		f,
		"// generated by " XASH_ENGINE_NAME " " XASH_VERSION "-%s (%s-%s)\n\n\n",
...
void FS_ParseGenericGameInfo(gameinfo_t* GameInfo, const char* buf, const qboolean isGameInfo)
{
	...
	while ( (pfile = COM_ParseFile(pfile, token, sizeof(token))) != NULL )
	{
		...
		else if ( !Q_stricmp(token, "icon") )
		{
			...
			COM_DefaultExtension(GameInfo->iconpath, sizeof(GameInfo->iconpath), ".ico");
		}
		...
		else if ( isGameInfo )
		{
			...
			else if ( !Q_strnicmp(token, "ambient", 7) )
			{
				...
				pfile = COM_ParseFile(
					pfile,
					GameInfo->ambientsound[ambientNum],
...
static qboolean FS_ParseGameInfo(const char* gamedir, gameinfo_t* GameInfo)
{
	...
	if ( (FS_CheckForQuakeGameDir(gamedir, false) || !Q_stricmp(fs_gamedir, gamedir)) &&
		 !FS_FileExists(gameinfo_path, false) )
	{
...
static qboolean FS_FindLibrary(const char* dllname, qboolean directpath, fs_dllinfo_t* dllInfo)
{
	...
	COM_DefaultExtension(dllInfo->shortPath, sizeof(dllInfo->shortPath), "." OS_LIB_EXT);  // apply ext just in case
	...
	else
	{
		...
		if ( search && search->type != SEARCHPATH_PLAIN )
		{
			...
			Con_Printf(
				S_WARN
				"%s: loading libraries from packs is unsupported on "
				"this platform\n",
				__FUNCTION__
			);
...
*/
qboolean
FS_InitStdio(qboolean caseinsensitive, const char* rootdir, const char* basedir, const char* gamedir, const char* rodir)
{
	stringlist_t dirs;
...
qboolean FS_Rename(const char* oldname, const char* newname)
{
	...
	{
		Con_Printf(
			"%s: failed to rename file %s (%s) to %s (%s): %s\n",
			__FUNCTION__,
			oldpath,
			oldname2,
			newpath,
...
search_t* FS_Search(const char* pattern, int caseinsensitive, int gamedironly, uint32_t flags)
{
	...
	for ( searchpath = fs_searchpaths; searchpath; searchpath = searchpath->next )
	{
		...
		searchpath->pfnSearch(searchpath, &resultlist, pattern, caseinsensitive, flags);
	}
...
static qboolean FS_InitInterface(int version, fs_interface_t* engfuncs)
{
	...
	{
		Con_Printf(
			S_ERROR "filesystem optional interface version mismatch: expected %d, got %d\n",
			FS_API_VERSION,
			version
		);
		return false;
