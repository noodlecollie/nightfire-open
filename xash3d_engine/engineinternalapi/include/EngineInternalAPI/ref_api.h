...
#define FCVAR_READ_ONLY \
	(1 << 17)  // cannot be set by user at all, and can't be requested by CvarGetPointer from game dlls
...
typedef struct ref_interface_s
{
	...
	// glState related calls (must use this instead of normal gl-calls to prevent de-synchornize local states between
