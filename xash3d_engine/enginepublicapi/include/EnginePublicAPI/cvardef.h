...
#define FCVAR_PRINTABLEONLY \
	(1 << 7)  // This cvar's string cannot contain unprintable characters ( e.g., used for player name etc ).
#define FCVAR_UNLOGGED \
	(1 << 8)  // If this is a FCVAR_SERVER, don't log changes to the log file / console if we are creating a log
...
#define FCVAR_LATCH \
	(1 << 30)  // Notify client that this cvar will be applied only after server restart. Only notifies, doesn't
			   // actually implement this behaviour.
