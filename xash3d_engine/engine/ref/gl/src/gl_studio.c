...
#define LIGHTING_ORIGIN_CROSS_SIZE 16.0f
...
void R_StudioInit(void)
{
	r_studio_sort_textures =
		gEngfuncs.Cvar_Get("r_studio_sort_textures", "0", FCVAR_GLCONFIG, "change draw order for additive meshes");
...
static void pfnGetEngineTimes(int* framecount, double* current, double* old)
{
	...
	if ( current )
		*current = gpGlobals->time;
	if ( old )
		*old = gpGlobals->oldtime;
}
...
void R_StudioLerpMovement(cl_entity_t* e, double time, vec3_t origin, vec3_t angles)
{
	...
	if ( g_studio.interpolate && (time < e->curstate.animtime + 1.0f) &&
		 (e->curstate.animtime != e->latched.prevanimtime) )
		f = ((float)time - e->curstate.animtime) / (e->curstate.animtime - e->latched.prevanimtime);
...
float R_StudioEstimateInterpolant(cl_entity_t* e)
{
	...
	{
		dadt = ((float)g_studio.time - e->curstate.animtime) / 0.1f;
...
void R_StudioFxTransform(cl_entity_t* ent, matrix3x4 transform)
{
	switch ( ent->curstate.renderfx )
	{
		...
		case kRenderFxExplode:
		{
			...
			scale = 1.0f + ((float)g_studio.time - ent->curstate.animtime) * 10.0f;
	...
	for ( j = 0; j < m_pStudioHeader->numbonecontrollers; j++ )
	{
		...
		else if ( i < 4 )
		{
			...
			if ( FBitSet(pbonecontroller[j].type, STUDIO_RLOOP) )
			{
				...
				{
					value = ((pcontroller1[i] * dadt + (pcontroller2[i]) * (1.0f - dadt))) * (360.0f / 256.0f) +
...
====================
*/
void R_StudioMergeBones(cl_entity_t* e, model_t* submodel)
{
	int i, j;
	...
	panim = gEngfuncs.R_StudioGetAnim(m_pStudioHeader, submodel, pseqdesc);
...
void R_StudioSetupBones(cl_entity_t* e)
{
		 ...
		 (e->latched.prevsequence < m_pStudioHeader->numseq) )
	{
		...
		s = 1.0f - ((float)g_studio.time - e->latched.sequencetime) / 0.2f;
	...
	if ( m_pPlayerInfo && m_pPlayerInfo->gaitsequence != 0 )
	{
		...
		pseqdesc =
			(mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pPlayerInfo->gaitsequence;
...
void R_StudioBuildNormalTable(void)
{
	...
	g_studio.chrome_origin[0] = cosf(r_glowshellfreq->value * (float)g_studio.time) * 4000.0f;
	g_studio.chrome_origin[1] = sinf(r_glowshellfreq->value * (float)g_studio.time) * 4000.0f;
	g_studio.chrome_origin[2] = cosf(r_glowshellfreq->value * (float)g_studio.time * 0.33f) * 4000.0f;
...
void R_StudioDynamicLight(cl_entity_t* ent, alight_t* plight)
{
	...
	if ( ent->index == 0 )
	{
		// Static entity, so the lighting offset is stored in velocity.
		VectorAdd(origin, ent->curstate.velocity, origin);
	}
	...
	plight->ambientlight = (int)(total - plight->shadelight);
...
void R_StudioSetupLighting(alight_t* plight)
{
	...
	g_studio.ambientlight = (float)plight->ambientlight;
	g_studio.shadelight = (float)plight->shadelight;
...
void R_LightLambert(vec4_t light[MAX_LOCALLIGHTS], vec3_t normal, vec3_t color, byte* out)
{
	...
	for ( i = 0; i < g_studio.numlocallights; i++ )
	{
		...
		if ( r > 0.0f )
		{
			if ( light[i][3] == 0.0f )
			{
				...
				if ( r2 > 0.0f )
				{
					light[i][3] = g_studio.locallightR2[i] / (r2 * sqrtf(r2));
				}
				else
				{
					light[i][3] = 0.0001f;
	...
	out[0] = (byte)(finalLight[0] * 255.0f);
	out[1] = (byte)(finalLight[1] * 255.0f);
	out[2] = (byte)(finalLight[2] * 255.0f);
}
...
static int R_StudioBuildIndices(qboolean tri_strip, int vertexState)
{
	...
	if ( vertexState++ < 3 )
	{
		g_studio.arrayelems[g_studio.numelems++] = (unsigned short)g_studio.numverts;
	}
	else if ( tri_strip )
	{
		...
		if ( vertexState & 1 )
		{
			...
			g_studio.arrayelems[g_studio.numelems++] = (unsigned short)g_studio.numverts - 2;
			g_studio.arrayelems[g_studio.numelems++] = (unsigned short)g_studio.numverts - 1;
			g_studio.arrayelems[g_studio.numelems++] = (unsigned short)g_studio.numverts;
		}
		else
		{
	...
	else
	{
		...
		g_studio.arrayelems[g_studio.numelems++] = (unsigned short)(g_studio.numverts - (vertexState - 1));
		g_studio.arrayelems[g_studio.numelems++] = (unsigned short)(g_studio.numverts - 1);
		g_studio.arrayelems[g_studio.numelems++] = (unsigned short)g_studio.numverts;
	}
...
static void R_StudioDrawPoints(void)
{
		 ...
		 m_pSubModel->blendnorminfoindex != 0 )
	{
		mstudioboneweight_t* pvertweight =
			(mstudioboneweight_t*)((byte*)m_pStudioHeader + m_pSubModel->blendvertinfoindex);
		mstudioboneweight_t* pnormweight =
			(mstudioboneweight_t*)((byte*)m_pStudioHeader + m_pSubModel->blendnorminfoindex);
		matrix3x4 skinMat;
...
static void R_StudioDrawBones(void)
{
	...
	// Draw lighting origin for static ents
	if ( RI.currententity->index == 0 )
	{
		vec3_t lightingOrigin;
		VectorAdd(RI.currententity->origin, RI.currententity->curstate.velocity, lightingOrigin);
		...
		for ( size_t axis = 0; axis < 3; ++axis )
		{
			...
			{
				VectorCopy(lightingOrigin, point);
				point[axis] += (step == 0 ? 1.0f : -1.0f) * (LIGHTING_ORIGIN_CROSS_SIZE / 2.0f);
				pglVertex3fv(point);
			}
		}
...
static void R_StudioClientEvents(void)
{
	...
	start = end - e->curstate.framerate * (float)gpGlobals->frametime * pseqdesc->fps;
...
static void R_StudioSetHeaderAdapter(void* pheader)
{
	R_StudioSetHeader((studiohdr_t*)pheader);
}
...
static void R_StudioSetupRenderer(int rendermode)
{
	...
	if ( phdr && FBitSet(phdr->flags, STUDIO_HAS_BONEINFO) )
	{
		...
		mstudioboneinfo_t* boneinfo =
			(mstudioboneinfo_t*)((byte*)phdr + phdr->boneindex + phdr->numbones * sizeof(mstudiobone_t));
...
void R_StudioEstimateGait(entity_state_t* pplayer)
{
	...
	dt = (float)bound(0.0f, g_studio.frametime, 1.0f);
	...
	m_flGaitMovement = (float)VectorLength(est_velocity);
	...
	{
		m_pPlayerInfo->gaityaw = (atan2f(est_velocity[1], est_velocity[0]) * 180 / M_PI_F);
...
void R_StudioProcessGait(entity_state_t* pplayer)
{
	...
	pseqdesc =
		(mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + RI.currententity->curstate.sequence;
	...
	RI.currententity->curstate.blending[0] = (byte)iBlend;
	...
	m_pPlayerInfo->gaitframe =
		m_pPlayerInfo->gaitframe - (int)(m_pPlayerInfo->gaitframe / pseqdesc->numframes) * pseqdesc->numframes;
...
static void R_StudioLoadTexture(model_t* mod, studiohdr_t* phdr, mstudiotexture_t* ptexture)
{
	...
	if ( !Q_strnicmp(ptexture->name, "DM_Base", 7) || !Q_strnicmp(ptexture->name, "remap", 5) )
	{
		...
		texturesize = ptexture->width * ptexture->height + 768;
		tx = Mem_Calloc(mod->mempool, sizeof(*tx) + texturesize);
		mod->textures[i] = tx;
		...
		memcpy(tx + 1, pixels, texturesize);
...
static engine_studio_api_t gStudioAPI = {
	...
	R_StudioDrawAbsBBox,
	R_StudioDrawBones,
	R_StudioSetupSkinAdapter,
	R_StudioSetRemapColors,
	R_StudioSetupPlayerModel,
	R_StudioClientEvents,
	R_StudioGetForceFaceFlags,
	R_StudioSetForceFaceFlags,
	R_StudioSetHeaderAdapter,
	R_StudioSetRenderModel,
	R_StudioSetupRenderer,
	R_StudioRestoreRenderer,
	R_StudioSetChromeOrigin,
