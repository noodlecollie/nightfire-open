...
static void R_ParseDetailTextures(const char* filename)
{
	...
	while ( (pfile = COM_ParseFile(pfile, token, sizeof(token))) != NULL )
	{
		...
		for ( i = 0; i < WORLDMODEL->numtextures; i++ )
		{
			...
			tex->dt_texturenum = (unsigned short)GL_LoadTexture(detail_path, NULL, 0, TF_FORCE_COLOR);
...
void R_NewMap(void)
{
	...
	for ( i = 0; i < WORLDMODEL->numtextures; i++ )
	{
		...
		if ( !Q_strncmp(tx->name, HALFLIFE_TEXPATH_SKY, sizeof(HALFLIFE_TEXPATH_SKY) - 1) &&
			 tx->width == (tx->height * 2) )
			tr.skytexturenum = i;
