...
void R_RenderInfo_f(void)
{
	...
	gEngfuncs.Con_Printf(
		"Color %d bits, Alpha %d bits, Depth %d bits, Stencil %d bits\n",
		glConfig.color_bits,
		glConfig.alpha_bits,
		glConfig.depth_bits,
...
void GL_InitCommands(void)
{
	...
	r_lighting_extended =
		gEngfuncs.Cvar_Get("r_lighting_extended", "1", FCVAR_GLCONFIG, "allow to get lighting from world and bmodels");
	r_lighting_modulate =
		gEngfuncs.Cvar_Get("r_lighting_modulate", "0.6", FCVAR_GLCONFIG, "lightstyles modulate scale");
	...
	r_lockfrustum =
		gEngfuncs.Cvar_Get("r_lockfrustum", "0", FCVAR_CHEAT, "lock frustrum area at current point (cull test)");
	r_traceglow = gEngfuncs.Cvar_Get("r_traceglow", "1", FCVAR_GLCONFIG, "cull flares behind models");

	gl_extensions =
		gEngfuncs.Cvar_Get("gl_allow_extensions", "1", FCVAR_GLCONFIG | FCVAR_READ_ONLY, "allow gl_extensions");
	...
	gl_texture_lodbias = gEngfuncs.Cvar_Get(
		"gl_texture_lodbias",
		"0.0",
		FCVAR_GLCONFIG,
...
static void R_CheckVBO(void)
{
	...
	r_vbo_dlightmode =
		gEngfuncs.Cvar_Get("gl_vbo_dlightmode", dlightmode, FCVAR_ARCHIVE, "vbo dlight rendering mode(0-1)");
}
