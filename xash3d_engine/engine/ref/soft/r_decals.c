...
static void R_DecalUnlink(decal_t* pdecal)
{
	...
	if ( pdecal->psurface )
	{
		...
		else
		{
			...
			if ( !tmp )
				gEngfuncs.Host_Error("D_DecalUnlink: bad decal list\n");
...
glpoly_t* R_DecalCreatePoly(decalinfo_t* decalinfo, decal_t* pdecal, msurface_t* surf)
{
	...
	// This return was randomly here, amongst the rest of the code that I've now ifdef'd out.
	// No idea if it's intentional or not?? Leaving it for now.
	return NULL;
...
void GAME_EXPORT R_DecalShoot(int textureIndex, int entityIndex, int modelIndex, vec3_t pos, int flags, float scale)
{
	...
	if ( entityIndex > 0 )
	{
		...
		if ( modelIndex > 0 )
			...
			model = gEngfuncs.pfnGetModelByIndex(ent->curstate.modelindex);
		else
			return;
	}
