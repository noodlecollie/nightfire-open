...
void R_StudioInit(void)
{
	r_studio_sort_textures =
		gEngfuncs.Cvar_Get("r_studio_sort_textures", "0", FCVAR_ARCHIVE, "change draw order for additive meshes");
...
void R_StudioPlayerBlend(mstudioseqdesc_t* pseqdesc, int* pBlend, float* pPitch)
{
	...
	*pBlend = (int)(*pPitch * 3.0f);
	...
	else
	{
		...
		{
			*pBlend =
				(int)(255 * (*pBlend - pseqdesc->blendstart[0]) / (pseqdesc->blendend[0] - pseqdesc->blendstart[0]));
		}
...
void R_StudioSetupLighting(alight_t* plight)
{
	...
	{
		scale = (float)RI.currententity->curstate.scale;
	}
...
void R_LightLambert(vec4_t light[MAX_LOCALLIGHTS], vec3_t normal, vec3_t color, byte* out)
{
	...
	out[0] = (byte)finalLight[0] * 255;
	out[1] = (byte)finalLight[1] * 255;
	out[2] = (byte)finalLight[2] * 255;
}
...
static void StudioSetupSkinAdapter(void* ptexturehdr, int index)
{
...
{
	const sortedmesh_t* mesh = (const sortedmesh_t*)a;
	(void)b;

	if ( FBitSet(mesh->flags, STUDIO_NF_ADDITIVE) )
	{
		return 1;
	}

	if ( FBitSet(mesh->flags, STUDIO_NF_MASKED) )
	{
		return -1;
	}
...
static void R_StudioSetHeaderAdapter(void* header)
{
...
void R_StudioEstimateGait(entity_state_t* pplayer)
{
	...
	dt = bound(0.0f, (float)g_studio.frametime, 1.0f);
...
static void R_StudioLoadTexture(model_t* mod, studiohdr_t* phdr, mstudiotexture_t* ptexture)
{
	...
	if ( !Q_strnicmp(ptexture->name, "DM_Base", 7) || !Q_strnicmp(ptexture->name, "remap", 5) )
	{
		...
		texsize = ptexture->width * ptexture->height + 768;
		tx = Mem_Calloc(mod->mempool, sizeof(*tx) + texsize);
		mod->textures[i] = tx;
