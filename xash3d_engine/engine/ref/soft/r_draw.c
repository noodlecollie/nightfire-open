...

		{
			...
			for ( u = 0; u < w; u++ )
			{
				...
				if ( vid.color != COLOR_WHITE )
					src = vid.modmap[(src & 0xff00) | (vid.color >> 8)] << 8 | (src & vid.color & 0xff) |
						((src & 0xff) >> 3);
				...
				else if ( alpha < 7 )  // && (vid.rendermode == kRenderTransAlpha || vid.rendermode ==
									   // kRenderTransTexture ) )
				{
					pixel_t screen = dest[u];  //  | 0xff & screen & src ;
					dest[u] =
						BLEND_ALPHA(alpha, src, screen);  // vid.alphamap[( alpha << 16)|(src & 0xff00)|(screen>>8)] <<
														  // 8 | (screen & 0xff) >> 3 | ((src & 0xff) >> 3);
				}
				else
					dest[u] = src;
			}
