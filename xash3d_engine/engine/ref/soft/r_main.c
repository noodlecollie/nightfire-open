...
int R_WorldToScreen(const vec3_t point, vec3_t screen)
{
	...
	screen[0] = worldToScreen[0][0] * point[0] + worldToScreen[0][1] * point[1] + worldToScreen[0][2] * point[2] +
		worldToScreen[0][3];
	screen[1] = worldToScreen[1][0] * point[0] + worldToScreen[1][1] * point[1] + worldToScreen[1][2] * point[2] +
		worldToScreen[1][3];
...
void GAME_EXPORT R_ScreenToWorld(const vec3_t screen, vec3_t point)
{
	...
	point[0] = screen[0] * screenToWorld[0][0] + screen[1] * screenToWorld[0][1] + screen[2] * screenToWorld[0][2] +
		screenToWorld[0][3];
	point[1] = screen[0] * screenToWorld[1][0] + screen[1] * screenToWorld[1][1] + screen[2] * screenToWorld[1][2] +
		screenToWorld[1][3];
	point[2] = screen[0] * screenToWorld[2][0] + screen[1] * screenToWorld[2][1] + screen[2] * screenToWorld[2][2] +
		screenToWorld[2][3];
	w = screen[0] * screenToWorld[3][0] + screen[1] * screenToWorld[3][1] + screen[2] * screenToWorld[3][2] +
		screenToWorld[3][3];
	if ( w != 0.0f )
...
void R_SetupFrustum(void)
{
	...
	{
		RI.fov_x = atan( tan( DEG2RAD( RI.fov_x ) / 2 ) * ( 0.97 + sin( gpGlobals->time * 1.5 ) * 0.03 )) * 2 / (M_PI /
	180.0); RI.fov_y = atan( tan( DEG2RAD( RI.fov_y ) / 2 ) * ( 1.03 - sin( gpGlobals->time * 1.5 ) * 0.03 )) * 2 /
	(M_PI / 180.0);
	}*/
...
//	else GL_FrustumInitProj( &RI.frustum, 0.0f, R_GetFarClip(), RI.fov_x, RI.fov_y ); // NOTE: we ignore nearplane here
//(mirrors only)
#endif
}
...
void R_DrawBEntitiesOnList(void)
{
	...
	for ( i = 0; i < tr.draw_list->num_edge_entities && !RI.onlyClientDraw; i++ )
	{
		...
		RotatedBBox(RI.currentmodel->mins, RI.currentmodel->maxs, RI.currententity->angles, mins, maxs);
#if 0
		mins[0] = mins[0] - 100;
		mins[1] = mins[1] - 100;
		mins[2] = mins[2] - 100;
...
void R_DrawBrushModel(cl_entity_t* pent)
{
	...
	{
		r_edges = (edge_t*)(((uintptr_t)&ledges[0] + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
	}
...
void GAME_EXPORT R_BeginFrame(qboolean clearScene)
{
	...
	// if( FBitSet(
	// gl_texture_nearest->flags|gl_lightmap_nearest->flags|gl_texture_anisotropy->flags|gl_texture_lodbias->flags,
	// FCVAR_CHANGED )) R_SetTextureParameters();
...
qboolean GAME_EXPORT R_Init(void)
{
	...
	sw_notransbrushes = gEngfuncs.Cvar_Get(
		"sw_notransbrushes",
		"0",
		FCVAR_GLCONFIG,
		"do not apply transparency to water/glasses (faster)");
	sw_noalphabrushes =
		gEngfuncs.Cvar_Get("sw_noalphabrushes", "0", FCVAR_GLCONFIG, "do not draw brush holes (faster)");
	...
	qfrustum.view_clipplanes[1].leftedge = qfrustum.view_clipplanes[2].leftedge = qfrustum.view_clipplanes[3].leftedge =
		false;
	qfrustum.view_clipplanes[0].rightedge = qfrustum.view_clipplanes[2].rightedge =
		qfrustum.view_clipplanes[3].rightedge = false;
	R_StudioInit();
	R_SpriteInit();
	R_InitTurb();
	GL_InitRandomTable();
