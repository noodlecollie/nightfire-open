...
#define ASSERT(x) \
	if ( !(x) ) \
	gEngfuncs.Host_Error("assert failed at %s:%i\n", __FILE__, __LINE__)
#define Assert(x) \
	if ( !(x) ) \
	gEngfuncs.Host_Error("assert failed at %s:%i\n", __FILE__, __LINE__)
...
#define CVAR_DEFINE(cv, cvname, cvstr, cvflags, cvdesc) \
	cvar_t cv = {cvname, cvstr, cvflags, 0.0f, (void*)CVAR_SENTINEL, cvdesc}
#define CVAR_DEFINE_AUTO(cv, cvstr, cvflags, cvdesc) \
	cvar_t cv = {#cv, cvstr, cvflags, 0.0f, (void*)CVAR_SENTINEL, cvdesc}
...
#define BLEND_ALPHA_LOW(alpha, src, screen) \
	(vid.alphamap[((alpha) << 18) | (((src)&0xff00) << 2) | ((screen) >> 6)] | ((screen)&0x3f))
#define BLEND_ALPHA(alpha, src, dst) \
	(alpha) > 3 ? BLEND_ALPHA_LOW(7 - 1 - (alpha), (dst), (src)) : BLEND_ALPHA_LOW((alpha)-1, (src), (dst))
#define BLEND_ADD(src, screen) \
	vid.addmap[((src)&0xff00) | ((screen) >> 8)] << 8 | ((screen)&0xff) | (((src)&0xff) >> 0);
#define BLEND_COLOR(src, color) \
	vid.modmap[((src)&0xff00) | ((color) >> 8)] << 8 | ((src) & (color)&0xff) | (((src)&0xff) >> 3);
