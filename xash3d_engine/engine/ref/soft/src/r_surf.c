...
static void (*surfmiptable[4])(
	void) = {R_DrawSurfaceBlock8_mip0, R_DrawSurfaceBlock8_mip1, R_DrawSurfaceBlock8_mip2, R_DrawSurfaceBlock8_mip3};
		...
		for ( t = 0, tacc = 0; t < tmax; t++, tacc += sample_size )
		{
			...
			for ( s = 0, sacc = 0; s < smax; s++, sacc += sample_size, bl += 1 )
			{
				...
				if ( dist < minlight )
				{
					// printf("dlight %f\n", dist);
					//*(void**)0 = 0;
					bl[0] += ((int)((rad - dist) * 256) *
							  gEngfuncs.LightToTexGamma((dl->color.r + dl->color.g + dl->color.b) / 3) * 3) /
						256;
		...
		for ( v = 0; v < height; v++ )
		{
...

			{
				...
				for ( u = 0; u < w; u++ )
				{
					...
					if ( alpha <
						 7 )  // && (vid.rendermode == kRenderTransAlpha || vid.rendermode == kRenderTransTexture ) )
					{
...
surfcache_t* D_CacheSurface(msurface_t* surface, int miplevel)
{
	...
	if ( cache && !cache->dlight && surface->dlightframe != tr.framecount && cache->image == r_drawsurf.image &&
	...
	{
		cache = D_SCAlloc(r_drawsurf.surfwidth, r_drawsurf.surfwidth * r_drawsurf.surfheight * 2);
