...
void R_DrawTriangle(void)
{
	...
	/*
	d_xdenom = ( aliastriangleparms.a->v[1] - aliastriangleparms.b->v[1] ) * ( aliastriangleparms.a->v[0] -
	aliastriangleparms.c->v[0] ) - ( aliastriangleparms.a->v[0] - aliastriangleparms.b->v[0] ) * (
	aliastriangleparms.a->v[1] - aliastriangleparms.c->v[1] );
	*/
...
static inline qboolean R_PolysetCheckBounds(pixel_t* lptex, int lsfrac, int ltfrac, int lcount)
{
	...
	lptex = lptex + a_ststepxwhole * lcount + ((lsfrac + (a_sstepxfrac * lcount)) >> 16) +
		((ltfrac + (a_tstepxfrac * lcount)) >> 16) * r_affinetridesc.skinwidth;
...
void R_PolysetCalcGradients(int skinwidth)
{
	...
	*/
	...
	__asm mov eax, dword ptr[r_p0 + 0] __asm mov ebx, dword ptr[r_p0 + 4] __asm sub eax,
		dword ptr[r_p2 + 0] __asm sub ebx, dword ptr[r_p2 + 4] __asm mov p00_minus_p20, eax __asm mov p01_minus_p21,
		ebx __asm fild dword ptr p00_minus_p20 __asm fild dword ptr p01_minus_p21 __asm mov eax,
		dword ptr[r_p1 + 0] __asm mov ebx, dword ptr[r_p1 + 4] __asm sub eax, dword ptr[r_p2 + 0] __asm sub ebx,
		dword ptr[r_p2 + 4] __asm fstp p01_minus_p21 __asm fstp p00_minus_p20 __asm mov p10_minus_p20,
		eax __asm mov p11_minus_p21,
		ebx __asm fild dword ptr p10_minus_p20 __asm fild dword ptr
			p11_minus_p21 __asm fstp p11_minus_p21 __asm fstp p10_minus_p20
	...
	-(1 / d_xdenom)
		...
		*/
		__asm mov eax,
		dword ptr[r_p0 + 16] __asm mov ebx, dword ptr[r_p1 + 16] __asm sub eax, dword ptr[r_p2 + 16] __asm sub ebx,
		dword ptr[r_p2 + 16]
	...
	r_lstepx |
		r_lstepy __asm fistp dword ptr[r_lstepx] __asm fistp dword ptr[r_lstepy]
		...
		*/
		__asm mov eax,
		dword ptr[r_p0 + 8] __asm mov ebx, dword ptr[r_p1 + 8] __asm sub eax, dword ptr[r_p2 + 8] __asm sub ebx,
		dword ptr[r_p2 + 8] __asm mov t0_int, eax __asm mov t1_int, ebx __asm fild t0_int;
	t0 __asm fild t1_int;
	t1 | t0 __asm fxch st(1);
	t0 | t1 __asm fstp t0;
	t1 __asm fst t1;
	...
	r_lstepy | r_lstepx __asm fxch st(1);
	r_lstepx |
		r_lstepy __asm fistp dword ptr[r_sstepx] __asm fistp dword ptr[r_sstepy]
		...
		*/
		__asm mov eax,
		dword ptr[r_p0 + 12] __asm mov ebx, dword ptr[r_p1 + 12] __asm sub eax, dword ptr[r_p2 + 12] __asm sub ebx,
		dword ptr[r_p2 + 12]
	...
	r_lstepy | r_lstepx __asm fxch st(1);
	r_lstepx |
		r_lstepy __asm fistp dword ptr[r_tstepx] __asm fistp dword ptr[r_tstepy]
		...
		*/
		__asm mov eax,
		dword ptr[r_p0 + 20] __asm mov ebx, dword ptr[r_p1 + 20] __asm sub eax, dword ptr[r_p2 + 20] __asm sub ebx,
		dword ptr[r_p2 + 20]
	...
	r_lstepy | r_lstepx __asm fxch st(1);
	r_lstepx |
		r_lstepy __asm fistp dword ptr[r_zistepx] __asm fistp dword ptr[r_zistepy]
		...
		*/
		__asm mov eax,
		d_pdrawspans __asm cmp eax, offset R_PolysetDrawSpans8_Opaque __asm mov eax, r_sstepx __asm mov ebx,
		r_tstepx __asm jne translucent
		// #if id386ALIAS
		__asm shl eax,
		16 __asm shl ebx,
		16 __asm jmp done_with_steps
			// #else
			translucent :
		__asm and eax,
	...
	do
	{
		...
		if ( lcount )
		{
			...
			do
			{
				...
				{
					*lpdest =
						BLEND_ALPHA(2, r_aliasblendcolor, *lpdest);  // vid.alphamap[r_aliasblendcolor + *lpdest*256];
				}
				lpdest++;
				lzi += r_zistepx;
				lpz++;
	...
	do
	{
		...
		if ( lcount )
		{
			...
			do
			{
				...
				{
					*lpdest =
						BLEND_ALPHA(5, r_aliasblendcolor, *lpdest);  // vid.alphamap[r_aliasblendcolor*256 + *lpdest];
				}
				lpdest++;
				lzi += r_zistepx;
				lpz++;
...
void R_PolysetFillSpans8(spanpackage_t* pspanpackage)
{
	...
	{
		lcount = d_aspancount - pspanpackage->count;
		// d_ptex + a_ststepxwhole * lcount  + ((a_sstepxfrac * lcount) >> 16) + ((a_tstepxfrac * lcount) >>
		// 16)*r_affinetridesc.skinwidth;
...
#endif
					pixel_t src = *lptex;
					//*lpdest = //vid.colormap[src & 0xff00|(llight>>8)] << 8 | (src & llight & 0xff) | ((src & 0xff) >>
					// 3);
...
void R_RasterizeAliasPolySmooth(void)
{
	...
	d_ptex = (pixel_t*)r_affinetridesc.pskin + (plefttop[2] >> 16) + (plefttop[3] >> 16) * r_affinetridesc.skinwidth;
...
// #if	id386ALIAS
	...
	if ( pedgetable->numleftedges == 2 )
	{
		...
		d_aspancount = plefttop[0] - prighttop[0];
		d_ptex =
			(pixel_t*)r_affinetridesc.pskin + (plefttop[2] >> 16) + (plefttop[3] >> 16) * r_affinetridesc.skinwidth;
