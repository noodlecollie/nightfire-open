...
#define FOR_EACH_COLOR(x) \
	for ( r##x = 0; r##x < BIT(3); r##x++ ) \
		for ( g##x = 0; g##x < BIT(3); g##x++ ) \
			for ( b##x = 0; b##x < BIT(2); b##x++ )
	...
	for ( i = 0; i < 1024; i++ )
	{
		...
		FOR_EACH_COLOR(2)
		{
			...
			for ( k = 0; k < 3; k++ )
			{
				...
				// save minor GBRGBRGB
				minor = MOVE_BIT(r, 1, 5) | MOVE_BIT(r, 0, 2) | MOVE_BIT(g, 2, 7) | MOVE_BIT(g, 1, 4) |
...
void R_AllocScreen(void)
{
	...
	swblit.pCreateBuffer(w, h, &swblit.stride, &swblit.bpp, &swblit.rmask, &swblit.gmask, &swblit.bmask);
	R_BuildScreenMap();
