...
linked list edgelist.  edgestoadd is assumed to be sorted on u, and non-empty (this is actually newedges[v]).  edgelist
is assumed to be sorted on u, with a sentinel at the end (actually, this is the active edge table starting at
edge_head.next).
==============
*/
...
void R_ScanEdges(void)
{
	...
	basespan_p = (espan_t*)((uintptr_t)(basespans + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
