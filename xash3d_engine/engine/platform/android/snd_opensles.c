...
static const char* SNDDMA_Android_Init(void)
{
	...
	result =
		(*engine)
			->CreateAudioPlayer(engine, &snddma_android_player, &source, &sink, 1, &interfaceID, &interfaceRequired);
	if ( result != SL_RESULT_SUCCESS )
	...
	result = (*snddma_android_player)
				 ->GetInterface(snddma_android_player, *pSL_IID_BUFFERQUEUE, &snddma_android_bufferQueue);
	if ( result != SL_RESULT_SUCCESS )
