...
qboolean R_Init_Video(const int type)
{
	...
	if ( FS_FileExists(GI->iconpath, true) )
	{
		Q_snprintf(
			...
			sizeof(buf),
			"%s/%s/%s",
			COM_CheckStringEmpty(host.rodir) ? host.rodir : host.rootdir,
			GI->gamefolder,
			GI->iconpath);
		Android_SetIcon(buf);
	}
...
static size_t VID_GenerateConfig(EGLint* attribs, size_t size)
{
		 ...
		 !(vid_android.gl_attribs[REF_GL_CONTEXT_PROFILE_MASK] & REF_GL_CONTEXT_PROFILE_ES) )
	{
		...
		vid_android.gl_attribs_set[REF_GL_CONTEXT_MAJOR_VERSION] &&
...
static size_t VID_GenerateContextConfig(EGLint* attribs, size_t size)
{
	...
	/*if( Q_strcmp( negl.extensions, " EGL_KHR_create_context ") )
	{
		if( vid_android.gl_attribs_set[REF_GL_CONTEXT_FLAGS] )
		{
			...
			attribs[i++] = vid_android.gl_attribs[REF_GL_CONTEXT_FLAGS] & ((REF_GL_CONTEXT_ROBUST_ACCESS_FLAG << 1) -
	1);
		}
...
qboolean SW_CreateBuffer(int width, int height, uint* stride, uint* bpp, uint* r, uint* g, uint* b)
{
	...
	nw.unlockAndPost(vid_android.window);
	Con_Printf(
		"SW_CreateBuffer: buffer %d %d %x %d %p\n",
		buffer.width,
		buffer.height,
		buffer.format,
		buffer.stride,
