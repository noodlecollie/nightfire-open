...
static struct
{
	...
	pthread_mutex_t framemutex;  // this mutex is locked while engine is running and unlocked while it reading events,
								 // used for pause in background.
...
DECLARE_JNI_INTERFACE(int, nativeInit, jobject array)
{
	...
	jni.messageBox =
		(*env)->GetStaticMethodID(env, jni.actcls, "messageBox", "(Ljava/lang/String;Ljava/lang/String;)V");
...
{
	(*jni.env)->CallStaticVoidMethod(
		jni.env,
		jni.actcls,
		jni.messageBox,
		(*jni.env)->NewStringUTF(jni.env, title),
		(*jni.env)->NewStringUTF(jni.env, text));
}
	...
	for ( i = 0; i < events.count; i++ )
	{
		switch ( events.queue[i].type )
		{
			...
			case event_touch_move:
				IN_TouchEvent(
					(touchEventType)events.queue[i].type,
					events.queue[i].arg,
					events.queue[i].touch.x,
					events.queue[i].touch.y,
					events.queue[i].touch.dx,
					events.queue[i].touch.dy);
				break;
			...
			case event_joyball:
				if ( !Joy_IsActive() )
					Joy_AddEvent();
				Joy_BallMotionEvent(events.queue[i].ball.ball, events.queue[i].ball.xrel, events.queue[i].ball.yrel);
				break;
			case event_joyhat:
				if ( !Joy_IsActive() )
					Joy_AddEvent();
			...
			case event_onpause:
				...
				switch ( host.status )
				{
					...
					case HOST_ERR_FATAL:
						Con_Reportf(
							S_WARN "Abnormal host state during onPause (%d), skipping config save!\n",
							host.status);
						break;
					default:
