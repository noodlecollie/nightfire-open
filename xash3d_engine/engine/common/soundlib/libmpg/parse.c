...
#define synchsafe_to_long(buf, res) \
	((((buf)[0] | (buf)[1] | (buf)[2] | (buf)[3]) & 0x80) \
		 ? 0 \
		 : (res = \
				(((ulong)(buf)[0]) << 21) | (((ulong)(buf)[1]) << 14) | (((ulong)(buf)[2]) << 7) | ((ulong)(buf)[3]), \
			1))
...
{
	if ( ((head & HDR_SYNC) != HDR_SYNC) || (!(HDR_LAYER_VAL(head))) || (HDR_BITRATE_VAL(head) == 0xf) ||
...
{
	ulong val = (((ulong)buf[*offset]) << 24) | (((ulong)buf[*offset + 1]) << 16) | (((ulong)buf[*offset + 2]) << 8) |
		((ulong)buf[*offset + 3]);
	*offset += 4;
...
static int check_lame_tag(mpg123_handle_t* fr)
{
	...
	if ( (fr->bsbuf[lame_offset] == 'I') && (fr->bsbuf[lame_offset + 1] == 'n') &&
		 (fr->bsbuf[lame_offset + 2] == 'f') && (fr->bsbuf[lame_offset + 3] == 'o') )
	{
		// we still have to see what there is
	}
	else if (
		(fr->bsbuf[lame_offset] == 'X') && (fr->bsbuf[lame_offset + 1] == 'i') && (fr->bsbuf[lame_offset + 2] == 'n') &&
	...
	if ( fr->bsbuf[lame_offset] != 0 )
	{
		...
		switch ( lame_vbr )
		{
			...
			default:
				fr->vbr = MPG123_VBR;
				break;  // 00 ==unknown is taken as VBR
		}
...
// PARSE_BAD: cannot get the framesize for some reason and shall silentry try the next possible header (if this is no
// free format stream after all...)
