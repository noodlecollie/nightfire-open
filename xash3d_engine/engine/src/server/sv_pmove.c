...
static void GAME_EXPORT pfnParticle(const float* origin, int color, float life, int zpos, int zvel)
{
	...
	v = (int)bound(-128.0f, (zpos * zvel) * 16.0f, 127.0f);
	...
	MSG_WriteByte(&sv.reliable_datagram, (int)bound(0.0f, life * 8, 255.0f));
}
...
pfnPlaySound(int channel, const char* sample, float volume, float attenuation, int fFlags, int pitch)
{
	edict_t* ent;
...
void SV_InitClientMove(void)
{
	...
	for ( i = 0; i < MAX_MAP_HULLS; i++ )
	{
		if ( svgame.dllFuncs.pfnGetHullBounds(i, host.player_mins[i], host.player_maxs[i]) )
			Con_Reportf(
				"SV: hull%i, player_mins: %g %g %g, player_maxs: %g %g %g\n",
				i,
				host.player_mins[i][0],
				host.player_mins[i][1],
	...
	svgame.pmove->COM_LoadFile = COM_LoadFile2;
...
void SV_SetupMoveInterpolant(sv_client_t* cl)
{
	...
	finalpush = ((float)host.realtime - latency - lerp_msec) + sv_unlagpush.value;

	if ( finalpush > host.realtime )
	{
		finalpush = (float)host.realtime;  // pushed too much ?
	}
...
void SV_RunCmd(sv_client_t* cl, usercmd_t* ucmd, int random_seed)
{
	...
	SV_PlayerRunThink(clent, (float)frametime, cl->timebase);
