...
=============
*/
static TYPEDESCRIPTION* SV_GetEntvarsDescription(int number)
{
...
static int SV_Multicast(int dest, const vec3_t origin, const edict_t* ent, qboolean usermessage, qboolean filter)
{
	...
	switch ( dest )
	{
		...
		case MSG_PAS:
		{
			...
			Mod_FatPVS(origin, FATPHS_RADIUS, fatphs, (int)world.fatbytes, false, (svs.maxclients == 1));
	...
	float scale)
{
	...
	MSG_WriteWord(msg, (int)(scale * 4096));
}
...
void SV_RestartAmbientSounds(void)
{
	...
	for ( i = 0; i < nSounds; i++ )
	{
		...
		SV_StartSound(
			SV_PEntityOfEntIndex(si->entnum, true),
			CHAN_STATIC,
			si->name,
			si->volume,
			si->attenuation,
...
void SV_PlaybackReliableEvent(sizebuf_t* msg, word eventindex, float delay, event_args_t* args)
{
	...
	if ( delay )
	{
		...
		MSG_WriteWord(msg, (int)(delay * 100.0f));
	}
	else
	{
...
edict_t* SV_FindEntityByString(edict_t* pStartEdict, const char* pszField, const char* pszValue)
{
	...
	while ( (desc = SV_GetEntvarsDescription(index++)) != NULL )
	{
	...
	const vec3_t pos)
{
	...
	{
		MSG_WriteByte(msg, (int)(attn * 64.0f));
	}
...
*/
void GAME_EXPORT
pfnEmitAmbientSound(edict_t* ent, float* pos, const char* sample, float vol, float attn, int flags, int pitch)
{
	int msg_dest;
	...
	if ( SV_BuildSoundMsg(&sv.multicast, ent, CHAN_STATIC, sample, (int)(vol * 255.0f), attn, flags, pitch, pos) )
	{
...
pfnTraceLine(const float* v1, const float* v2, int fNoMonsters, edict_t* pentToSkip, TraceResult* ptr)
{
	trace_t trace;
...
*/
static void GAME_EXPORT
pfnTraceHull(const float* v1, const float* v2, int fNoMonsters, int hullNumber, edict_t* pentToSkip, TraceResult* ptr)
{
	trace_t trace;
	...
	trace = SV_Move(
		v1,
		sv.worldmodel->hulls[hullNumber].clip_mins,
		sv.worldmodel->hulls[hullNumber].clip_maxs,
		v2,
		fNoMonsters,
		pentToSkip,
		false);
...
=============
*/
static int GAME_EXPORT pfnTraceMonsterHull(
	edict_t* pEdict,
	const float* v1,
	const float* v2,
	int fNoMonsters,
...
void GAME_EXPORT pfnParticleEffect(const float* org, const float* dir, float color, float count)
{
	...
	v = (int)bound(-128.0f, dir[0] * 16.0f, 127.0f);
	MSG_WriteChar(&sv.datagram, v);
	v = (int)bound(-128.0f, dir[1] * 16.0f, 127.0f);
	MSG_WriteChar(&sv.datagram, v);
	v = (int)bound(-128.0f, dir[2] * 16.0f, 127.0f);
	MSG_WriteChar(&sv.datagram, v);
	MSG_WriteByte(&sv.datagram, (int)count);
	MSG_WriteByte(&sv.datagram, (int)color);
...
void GAME_EXPORT pfnMessageBegin(int msg_dest, int msg_num, const float* pOrigin, edict_t* ed)
{
	...
	svgame.msg_trace = sv_trace_messages.value != 0 && msg_num > svc_lastmsg && Q_strcmp(svgame.msg_name, "ReqState");
...
void GAME_EXPORT pfnMessageEnd(void)
{
	...
	if ( svgame.msg_index < 0 )
	{
		if ( svgame.msg_size_index != -1 )
		{
			...
			realsize = (word)svgame.msg_realsize;
	...
	else if ( svgame.msg[svgame.msg_index].size != -1 )
	{
		...
		// compare sizes
		if ( expsize != svgame.msg_realsize )
		{
			Con_Printf(
...
void GAME_EXPORT pfnWriteString(const char* src)
{
	...
	while ( 1 )
	{
		...
		if ( --rem <= 0 )
		{
			Con_Printf(S_ERROR "pfnWriteString: exceeds %zu symbols\n", len);
...
=============
*/
void GAME_EXPORT pfnBuildSoundMsg(
	edict_t* pSource,
	int chan,
	const char* samp,
	float fvol,
...
void SV_AllocStringPool(void)
{
...
#ifdef USE_MMAP
	{
		...
		{
			void* mapptr =
				mmap((void*)((unsigned long)start & ~(pagesize - 1)), arrlen, PROT_READ | PROT_WRITE, flags, 0, 0);
...
string_t GAME_EXPORT SV_AllocString(const char* szValue)
{
	...
	return (string_t)(newString - svgame.globals->pStringBase);
#else
...
{
	return (int)((byte*)pEdict - (byte*)svgame.edicts);
}
...
static qboolean pfnIsHLTVClient(const edict_t* edict)
{
	if ( !edict )
	{
		return false;
	...
	sv_client_t* client = SV_ClientFromEdict(edict, false);
	...
	return (client->flags & FCL_HLTV_PROXY) != 0;
}

static struct writable_file_s* pfnOpenWritableFile(const char* path)
{
	struct writable_file_s* file = Mem_Calloc(svgame.mempool, sizeof(struct writable_file_s));
	...
	file->filePtr = FS_Open(path, "wb", true);
...
static void pfnCloseWritableFile(struct writable_file_s* file)
{
	if ( !file )
	{
		return;
...
static qboolean
pfnWriteElementsToFile(struct writable_file_s* file, const void* data, size_t elementSize, size_t elementCount)
{
	if ( !file )
	{
		Log_Printf(
	...
	const size_t bytesToWrite = (elementSize * elementCount);
...
static const byte* pfnGetPvsForPoint(const float* vecPoint)
{
	if ( !vecPoint )
	{
		return false;
...
static qboolean pfnPointInPvs(const float* vecPoint, const byte* pvs)
{
	if ( !vecPoint )
	{
		return false;
	...
	mleaf_t* leaf = Mod_PointInLeaf(vecPoint, sv.worldmodel->nodes);

	if ( !leaf || leaf->contents == CONTENTS_SOLID || leaf->cluster < 0 )
	{
		return false;
	}
...
void GAME_EXPORT pfnCrosshairAngle(const edict_t* pClient, float pitch, float yaw)
{
	...
	MSG_WriteChar(&client->netchan.message, (int)(pitch * 5.0f));
	MSG_WriteChar(&client->netchan.message, (int)(yaw * 5.0f));
}
...
*/
void GAME_EXPORT
pfnFadeClientVolume(const edict_t* pEdict, int fadePercent, int fadeOutSeconds, int holdTime, int fadeInSeconds)
{
	sv_client_t* cl;
...
=============
*/
void GAME_EXPORT pfnRunPlayerMove(
	edict_t* pClient,
	...
	byte msec)
{
	...
	if ( !FBitSet(cl->flags, FCL_FAKECLIENT) )
	{
		...
		if ( pClient && pClient->v.netname != 0 )
		{
			name = STRING(pClient->v.netname);
		}

		Con_Printf(S_WARN "Received pfnRunPlayerMove request from client %s who was not a bot\n", name);
		return;
	}
...
void GAME_EXPORT SV_PlaybackEventFull(const struct event_fire_args_s* inArgs)
{
	...
	{
		Mod_FatPVS(pvspoint, FATPHS_RADIUS, fatphs, (int)world.fatbytes, false, (svs.maxclients == 1));
		mask = fatphs;  // using the FatPVS like a PHS
	}
	...
	for ( slot = 0, cl = svs.clients; slot < svs.maxclients; slot++, cl++ )
	{
		...
		ei->entity_index = (short)invokerIndex;
		ei->packet_index = -1;
		ei->flags = localFlags;
		ei->args = args;
	}
...
byte* pfnSetFatPVS(const float* org)
{
	...
	{
		// merge PVS
		Mod_FatPVS(org, FATPVS_RADIUS, fatpvs, (int)world.fatbytes, true, fullvis);
	}
...
byte* pfnSetFatPAS(const float* org)
{
	...
	if ( !FBitSet(sv.hostflags, SVF_MERGE_VISIBILITY) )
	{
		...
		Mod_FatPVS(viewPos, FATPHS_RADIUS, fatphs, (int)world.fatbytes, false, fullvis);
	}
	else
	{
		// merge PHS
		Mod_FatPVS(org, FATPHS_RADIUS, fatphs, (int)world.fatbytes, true, fullvis);
	}
...
int GAME_EXPORT pfnCheckVisibility(const edict_t* ent, byte* pset)
{
	...
	else
	{
		...
		((edict_t*)ent)->leafnums[ent->num_leafs] = (short)leafnum;
...
{
	int idx = (int)(sv.current_client - svs.clients);
...
static void Wrapper_Info_RemoveKey(char* s, const char* key)
{
