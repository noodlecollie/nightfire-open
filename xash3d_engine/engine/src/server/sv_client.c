...
void SV_GetPlayerCount(int* players, int* bots)
{
	...
	for ( i = 0; i < svs.maxclients; i++ )
	{
		...
		{
			if ( FBitSet(svs.clients[i].flags, FCL_FAKECLIENT) && bots )
			{
				(*bots)++;
			}
			else if ( players )
...
void SV_ConnectClient(netadr_t from)
{
	...
	if ( !NET_IsLocalAddress(from) && sv_password.string[0] &&
		 Q_stricmp(sv_password.string, Info_ValueForKey(userinfo, "password")) )
	{
...
{
	return idx > 0 && idx <= svs.maxclients;
}
...
static qboolean SV_EntList_f(sv_client_t* cl)
{
	...
	for ( i = 0; i < svgame.numEntities; i++ )
	{
		...
		if ( Cmd_Argc() > 1 )
		{
			if ( !Q_stricmpext(Cmd_Argv(1), STRING(ent->v.classname)) &&
				 !Q_stricmpext(Cmd_Argv(1), STRING(ent->v.targetname)) )
			{
				continue;
			}
		}
