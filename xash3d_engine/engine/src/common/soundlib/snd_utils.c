...
*/
// stub
static const loadwavfmt_t load_null[] = {{NULL, NULL, NULL}};
...
*/
// stub
static const streamfmt_t stream_null[] = {{NULL, NULL, NULL, NULL, NULL, NULL, NULL}};
...
qboolean Sound_ResampleInternal(wavdata_t* sc, int inrate, int inwidth, int outrate, int outwidth)
{
	...
	if ( handled )
		Con_Reportf(
			"Sound_Resample: from [%d bit %d Hz] to [%d bit %d Hz]\n",
			inwidth * 8,
			inrate,
			outwidth * 8,
			outrate);
		...
		Con_Reportf(
			S_ERROR "Sound_Resample: unsupported from [%d bit %d Hz] to [%d bit %d Hz]\n",
			inwidth * 8,
			inrate,
			outwidth * 8,
			outrate);
