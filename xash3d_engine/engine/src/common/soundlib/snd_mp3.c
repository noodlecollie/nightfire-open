...
static qboolean Sound_ParseID3Tag(const byte* buffer, fs_offset_t filesize)
{
	...
	{
		Con_Printf(
			S_ERROR "Sound_ParseID3Tag: invalid ID3v2 tag version 2.%d.%d. Convert to ID3v2.4!\n",
			header->major_ver,
			header->minor_ver);
		return false;
	}
	...
	if ( FBitSet(header->flags, ID3V2_HEADER_EXTENDED_HEADER) )
	{
		...
		{
			Con_Printf(
				S_ERROR "Sound_ParseID3Tag: invalid extended header length %u, possibly broken file.\n",
				ext_length);
			return false;
		}
