...
int decode_update(mpg123_handle_t* mh)
{
	...
	if ( b < 0 )
		return MPG123_ERR;
	if ( b == 1 )
		mh->new_format = 1;  // store for later...
	...
	if ( !(mh->p.flags & MPG123_FORCE_MONO) )
	{
		if ( mh->af.channels == 1 )
			...
			mh->single = SINGLE_STEREO;
	}
	else
		mh->single = (mh->p.flags & MPG123_FORCE_MONO) - 1;
	...
	if ( change )
	{
		...
		if ( mh->fresh )
		{
			...
			if ( mh->num < mh->firstframe )
				b = get_next_frame(mh);
			if ( b < 0 )
				return b;  // Could be error, need for more, new format...
		}
	}
...
static void frame_buffercheck(mpg123_handle_t* fr)
{
	...
	{
		// there can be more than one frame of padding at the end, so we ignore the whole frame if we are beyond
		// lastframe.
	...
	if ( fr->firstoff && fr->num == fr->firstframe )
	{
		...
		if ( (mpg_off_t)fr->buffer.fill > byteoff )
		{
			...
			if ( fr->own_buffer )
				fr->buffer.p = fr->buffer.data + byteoff;
			else
				memmove(fr->buffer.data, fr->buffer.data + byteoff, fr->buffer.fill);
		}
		else
			fr->buffer.fill = 0;
...
int mpg123_scan(mpg123_handle_t* mh)
{
	...
	// scan through the _whole_ file, since the current position is no count but computed assuming constant samples per
	// frame. also, we can just keep the current buffer and seek settings. Just operate on input frames here.
