...
void init_layer3(void)
{
	...
	for ( i = 0; i < 18; i++ )
	{
		win[0][i] = win[1][i] = DOUBLE_TO_REAL(
			0.5 * sin(M_PI / 72.0 * (double)(2 * (i + 0) + 1)) / cos(M_PI * (double)(2 * (i + 0) + 19) / 72.0));
		win[0][i + 18] = win[3][i + 18] = DOUBLE_TO_REAL(
	...
	for ( i = 0; i < 12; i++ )
		win[2][i] =
			DOUBLE_TO_REAL(0.5 * sin(M_PI / 24.0 * (double)(2 * i + 1)) / cos(M_PI * (double)(2 * i + 7) / 24.0));
...
static int III_get_side_info(mpg123_handle_t* fr, III_sideinfo* si, int stereo, int ms_stereo, long sfreq, int single)
{
	...
	if ( si->main_data_begin > fr->bitreservoir )
	{
		...
		if ( fr->lsf == 0 )
		{
			...
			fr->wordpointer[0] = (byte)fr->bitreservoir;
...
static int III_dequantize_sample(
	mpg123_handle_t* fr,
		...
		for ( i = 0; i < 2; i++ )
		{
			...
			for ( ; lp; lp--, mc-- )
			{
				...
				if ( x == 15 && h->linbits )
				{
					...
					if ( MSB_MASK )
						*xrpnt = REAL_MUL_SCALE_LAYER3(-ispow[x], v);
					else
						*xrpnt = REAL_MUL_SCALE_LAYER3(ispow[x], v);
				...
				if ( y == 15 && h->linbits )
				{
					...
					if ( MSB_MASK )
						*xrpnt = REAL_MUL_SCALE_LAYER3(-ispow[y], v);
					else
						*xrpnt = REAL_MUL_SCALE_LAYER3(ispow[y], v);
...
static void
III_i_stereo(float xr_buf[2][SBLIMIT][SSLIMIT], int* scalefac, gr_info_t* gr_info, int sfreq, int ms_stereo, int lsf)
{
...
static void
III_hybrid(float fsIn[SBLIMIT][SSLIMIT], float tsOut[SSLIMIT][SBLIMIT], int ch, gr_info_t* gr_info, mpg123_handle_t* fr)
{
...
int do_layer3(mpg123_handle_t* fr)
{
	...
	for ( gr = 0; gr < granules; gr++ )
	{
		...
		if ( fr->lsf )
			...
			part2bits = III_get_scale_factors_1(fr, scalefacs[0], gr_info);
		...
		if ( stereo == 2 )
		{
			...
			if ( fr->lsf )
				...
				part2bits = III_get_scale_factors_1(fr, scalefacs[1], gr_info);
