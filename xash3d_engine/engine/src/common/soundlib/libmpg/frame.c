...
void frame_init_par(mpg123_handle_t* fr, mpg123_parm_t* mp)
{
	...
	if ( mp == NULL )
		frame_default_parm(&fr->p);
	else
		memcpy(&fr->p, mp, sizeof(mpg123_parm_t));
...
int frame_buffers_reset(mpg123_handle_t* fr)
{
	...
	// wondering: could it be actually _wanted_ to retain buffer contents over different files? (special gapless / cut
	// stuff)
	...
	*get_frame = 0;
	...
	if ( fr->index.fill )
	{
		...
		if ( fi >= fr->index.fill )
		{
			...
			// when fuzzy seek is allowed, we have some limited tolerance for the frames we want to read rather then
			// jump over.
...
// the frame seek... this is not simply the seek to fe * fr->spf samples in output because we think of _input_ frames
// here. seek to frame offset 1 may be just seek to 200 samples offset in output since the beginning of first frame is
// delay/padding. hm, is that right? OK for the padding stuff, but actually, should the decoder delay be better totally
// hidden or not? with gapless, even the whole frame position could be advanced further than requested (since Homey
// don't play dat).
