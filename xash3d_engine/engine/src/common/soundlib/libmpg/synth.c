...
int synth_1to1(float* bandPtr, int channel, mpg123_handle_t* fr, int final)
{
...

	{
		...
		for ( j = (BLOCK / 4); j;
			  j--, b0 += 0x400 / BLOCK - BACKPEDAL, window += 0x800 / BLOCK - BACKPEDAL, samples += step )
		{
			float sum;
	...
	if ( final )
		fr->buffer.fill += BLOCK * sizeof(short);
...
int set_synth_functions(mpg123_handle_t* fr)
{
	...
	fr->synth_mono =
		fr->af.channels == 2 ? fr->synths.mono2stereo[resample][basic_format] : fr->synths.mono[resample][basic_format];
