...
qboolean Cmd_GetMapList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("maps/%s*.bsp", s), true, (int)con_gamemaps->value);
...
qboolean Cmd_GetDemoList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("%s*.dem", s), true, true);
...
qboolean Cmd_GetMovieList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("media/%s*.avi", s), true, false);
...
qboolean Cmd_GetMusicList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("media/%s*.*", s), true, false);
...
qboolean Cmd_GetSavesList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va(DEFAULT_SAVE_DIRECTORY "%s*.sav", s), true, true);  // lookup only in gamedir
...
qboolean Cmd_GetConfigList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("%s*.cfg", s), true, false);
...
qboolean Cmd_GetSoundList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("%s%s*.*", DEFAULT_SOUNDPATH, s), true, false);
...
qboolean Cmd_GetItemsList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("%s/%s*.txt", clgame.itemspath, s), true, false);
...
qboolean Cmd_GetCustomList(const char* s, char* completedname, int length)
{
	...
	t = FS_SearchFiles(va("%s*.hpk", s), true, false);
...
qboolean Cmd_CheckMapsList_R(qboolean fRefresh, qboolean onlyingamedir)
{
	...
	t = FS_SearchFiles("maps/*.bsp", false, onlyingamedir);
	...
	for ( i = 0; i < t->numfilenames; i++ )
	{
		...
		if ( f )
		{
			...
			if ( !Mod_TestBmodelLumps(f, t->filenames[i], fileData, (size_t)bytesRead, true, &entities) )
			{
				Con_DPrintf("Cmd_CheckMapsList_R: Ignoring %s as lumps failed to validate\n", t->filenames[i]);
			...
			{
				Con_DPrintf("Cmd_CheckMapsList_R: Ignoring %s as no spawn points were found\n", t->filenames[i]);
			}
		}
	}
