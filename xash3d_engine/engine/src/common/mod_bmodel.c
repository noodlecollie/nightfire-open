...
typedef union dnode32_u
{
	dnode_t* nodes;
	dnode32_t* nodes32;
} dnode32_u;

typedef union dleaf32_u
{
	dleaf_t* leafs;
	dleaf32_t* leafs32;
} dleaf32_u;

typedef union dclipnode32_u
{
	dclipnode_t* clipnodes;
	dclipnode32_t* clipnodes32;
} dclipnode32_u;

typedef union dmarkface32_u
{
	dmarkface_t* markfaces;
	dmarkface32_t* markfaces32;
} dmarkface32_u;

typedef union dedge32_u
{
	dedge_t* edges;
	dedge32_t* edges32;
} dedge32_u;

typedef union dface32_u
{
	dface_t* surfaces;
...
typedef struct
{
	...
	dclipnode32_u clipnodes;
	size_t numclipnodes;

	dtexinfo_t* texinfo;
	size_t numtexinfo;

	dmarkface32_u markfaces;
	size_t nummarkfaces;
...
#define CHECK_OVERFLOW \
	BIT(0)  // if some of lumps will be overflowed this non fatal for us. But some lumps are critical. mark them
...
typedef struct bspheaders_s
{
	...
	const dclientents_header_t* nfopenClientEntsHeader;
} bspheaders_t;
...
static bspheaders_t Mod_QueryBSPHeaders(const byte* data, size_t length)
{
		 ...
		 sizeof(dheader_t) + sizeof(dnfopenextraheader_t) <= length )
	{
		...
		if ( nfopenHeader->id == NFOPEN_EXTRAHEADER_ID )
		{
			const byte* dataBegin = (const byte*)nfopenHeader + sizeof(*nfopenHeader) +
				(nfopenHeader->numLumps * sizeof(dnfopenextralump_t));
	...
	{
		const dnfopenextralump_t* clientEntsLump =
			LoadNFOpenExtraLump(headers.nfopenHeader, (uint32_t)NFOPEN_LUMP_CLIENTENTS);

		const byte* clientEntsBegin =
			(const byte*)headers.nfopenHeader + clientEntsLump->offsetFromBeginningOfExtraHeader;

		if ( clientEntsBegin + sizeof(dclientents_header_t) <= data + length )
		{
			headers.nfopenClientEntsHeader = (const dclientents_header_t*)clientEntsBegin;
		}
	}
...
static qboolean Mod_VerifyBSPHeaders(const bspheaders_t* headers)
{
	...
	if ( headers->nfopenClientEntsHeader &&
		 headers->nfopenClientEntsHeader->version != NFOPEN_CLIENT_ENT_HEADER_VERSION )
	{
		// Just warn about this - the loading function will also check the version
		// and will decline to load the client entities if it doesn't match.
		// It's very annoying to have this reject the entire map file for old maps
...
static fs_offset_t Mod_CalculateMipTexSize(mip_t* mt, qboolean palette)
{
	...
	return sizeof(*mt) + ((mt->width * mt->height * 85) >> 6) + (palette ? MIPTEX_CUSTOM_PALETTE_SIZE_BYTES : 0);
}
...
static qboolean Mod_NameImpliesTextureIsAnimated(texture_t* tex)
{
	...
	if ( !(tex->name[1] >= '0' && tex->name[1] <= '9') && !(tex->name[1] >= 'a' && tex->name[1] <= 'j') )
	{
...
void Mod_PrintWorldStats_f(void)
{
	...
	for ( i = 0; i < SIZE_OF_ARRAY(worldstats); i++ )
	{
		...
		{
			totalmemory += Mod_GlobUsage(stat->lumpname, (int)stat->count, (int)stat->maxcount);
		}
		else
		{
			totalmemory += Mod_ArrayUsage(stat->lumpname, (int)stat->count, (int)stat->maxcount, (int)stat->entrysize);
		}
	}
...
static void Mod_FatPVS_RecursiveBSPNode(const vec3_t org, float radius, byte* visbuffer, int visbytes, mnode_t* node)
{
	...
	{
		byte* vis = Mod_DecompressPVS(((mleaf_t*)node)->compressed_vis, (int)world.visbytes);
...
static void Mod_BoxLeafnums_r(leaflist_t* ll, mnode_t* node)
{
	...
	while ( 1 )
	{
		...
		else
		{
			...
			if ( ll->topnode == -1 )
			{
				ll->topnode = (int)(node - worldmodel->nodes);
			}
...
static qboolean Mod_CheckWaterAlphaSupport(dbspmodel_t* bmod)
{
	...
	for ( i = 0, leaf = loadmodel->leafs; i < loadmodel->numleafs; i++, leaf++ )
	{
		...
		{
			pvs = Mod_DecompressPVS(leaf->compressed_vis, (int)world.visbytes);
...
static void Mod_CalcSurfaceExtents(msurface_t* surf)
{
	...
	for ( i = 0; i < surf->numedges; i++ )
	{
		...
		for ( j = 0; j < 2; j++ )
		{
			val = (float)DotProductPrecise(v->position, surf->texinfo->vecs[j]) + surf->texinfo->vecs[j][3];
		...
		for ( j = 0; j < 2; j++ )
		{
			val = (float)DotProductPrecise(v->position, info->lmvecs[j]) + info->lmvecs[j][3];
	...
	for ( i = 0; i < 2; i++ )
	{
		bmins[i] = (int)floorf(mins[i] / sample_size);
		bmaxs[i] = (int)ceilf(maxs[i] / sample_size);

		surf->texturemins[i] = (short)(bmins[i] * sample_size);
		surf->extents[i] = (short)((bmaxs[i] - bmins[i]) * sample_size);

		if ( FBitSet(tex->flags, TEX_WORLD_LUXELS) )
		{
			lmmins[i] = floorf(lmmins[i]);
			lmmaxs[i] = ceilf(lmmaxs[i]);

			info->lightmapmins[i] = (short)lmmins[i];
			info->lightextents[i] = (short)(lmmaxs[i] - lmmins[i]);
		}
		else
		{
...
static void Mod_CreateFaceBevels(msurface_t* surf)
{
	...
	for ( i = 0; i < surf->numedges; i++ )
	{
		...
		dest->type = (byte)PlaneTypeForNormal(dest->normal);
...
static int RemapClipNodes_r(dclipnode32_t* srcnodes, hull_t* hull, int nodenum)
{
	...
	for ( i = 0; i < 2; i++ )
	{
		out->children[i] = (short)RemapClipNodes_r(srcnodes, hull, src->children[i]);
	}
...
static qboolean Mod_LoadColoredLighting(dbspmodel_t* bmod)
{
	...
	if ( (size_t)litdatasize != (bmod->lightdatasize * 3) )
	{
		Con_Printf(
...
static void Mod_LoadDeluxemap(dbspmodel_t* bmod)
{
	...
	if ( (size_t)deluxdatasize != bmod->lightdatasize )
	{
		Con_Reportf(
			S_ERROR "%s has mismatched size (%llu should be %zu)\n",
			path,
...
static void Mod_LoadSubmodels(dbspmodel_t* bmod)
{
	...
	loadmodel->numsubmodels = (int)bmod->numsubmodels;
...
static void Mod_LoadPlanes(dbspmodel_t* bmod)
{
	...
	loadmodel->numplanes = (int)bmod->numplanes;
...
static void Mod_LoadVertexes(dbspmodel_t* bmod)
{
	...
	loadmodel->numvertexes = (int)bmod->numvertexes;
...
static void Mod_LoadSurfEdges(dbspmodel_t* bmod)
{
	...
	loadmodel->numsurfedges = (int)bmod->numsurfedges;
}
...
static void Mod_LoadTextureData(dbspmodel_t* bmod, int textureIndex, qboolean allowWAD)
{
	...
	if ( allowWAD && FBitSet(REF_GET_PARM(PARM_TEX_FLAGS, texture->gl_texturenum), TF_HAS_LUMA) )
	{
		...
		{
			const size_t size = Mod_CalculateMipTexSize(mipTex, usesCustomPalette);
			texture->fb_texturenum =
				(unsigned short)ref.dllFuncs.GL_LoadTexture(texName, (byte*)mipTex, size, TF_MAKELUMA);
		}
		else
		{
			char texpath[MAX_VA_STRING];
			...
			texture->fb_texturenum = (unsigned short)ref.dllFuncs.GL_LoadTexture(texName, src, srcSize, TF_MAKELUMA);
...
static void Mod_LoadTexInfo(dbspmodel_t* bmod)
{
	...
	loadmodel->numtexinfo = (int)bmod->numtexinfo;
	in = bmod->texinfo;
	...
	for ( i = 0; i < bmod->numtexinfo; i++, in++, out++ )
	{
		...
		if ( faceinfo != NULL && in->faceinfo != -1 && (size_t)in->faceinfo < bmod->numfaceinfo )
		{
...
static void Mod_LoadNodes(dbspmodel_t* bmod)
{
	...
	loadmodel->numnodes = (int)bmod->numnodes;
...
static void Mod_LoadLeafs(dbspmodel_t* bmod)
{
	...
	loadmodel->numleafs = (int)bmod->numleafs;
	...
	for ( i = 0; i < bmod->numleafs; i++, out++ )
	{
		...
		if ( bmod->isworld )
		{
			out->cluster = (int)(i - 1);  // solid leaf 0 has no visdata
			...
			{
				if ( (size_t)p < bmod->visdatasize )
				{
					out->compressed_vis = loadmodel->visdata + p;
				}
				else
				{
					Con_Reportf(S_WARN "Mod_LoadLeafs: invalid visofs for leaf #%zu\n", i);
				}
			}
		}
		else
...
static void Mod_LoadClipnodes(dbspmodel_t* bmod)
{
	...
	{
		dclipnode_t* in = bmod->clipnodes.clipnodes;
		...
		for ( i = 0; i < bmod->numclipnodes; i++, out++, in++ )
		{
			...
			if ( (size_t)out->children[0] >= bmod->numclipnodes )
			{
				out->children[0] -= 65536;
			}

			if ( (size_t)out->children[1] >= bmod->numclipnodes )
			{
	...
	loadmodel->numclipnodes = (int)bmod->numclipnodes;
}
		...
		case 1:
			if ( !Mod_LoadColoredLighting(bmod) )
			{
				loadmodel->lightdata = out =
					(color24*)Mem_Malloc(loadmodel->mempool, bmod->lightdatasize * sizeof(color24));
				in = bmod->lightdata;
...
static void Mod_LoadClientModels(const dbspmodel_t* bmod)
{
	...
	for ( size_t modelIndex = 0; modelIndex < loadmodel->clientEntities->modelCount; ++modelIndex )
	{
		...
		VectorCopy(inModel->lightingOriginOffset, outModel->lightingOriginOffset);
	}
}
