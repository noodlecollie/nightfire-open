...
qboolean Image_LoadPNG(const char* name, const byte* buffer, fs_offset_t filesize)
{
	...
	{
		Con_DPrintf(
			S_ERROR "Image_LoadPNG: Invalid image size %ux%u (%s)\n",
			png_hdr.ihdr_chunk.width,
			png_hdr.ihdr_chunk.height,
			name);
		return false;
	}
	...
	{
		Con_DPrintf(
			S_ERROR "Image_LoadPNG: Unknown compression method %u (%s)\n",
			png_hdr.ihdr_chunk.compression,
			name);
		return false;
	}
	...
	CRC32_ProcessBuffer(
		&crc32_check,
		buf_p + sizeof(png_hdr.sign) + sizeof(png_hdr.ihdr_len),
