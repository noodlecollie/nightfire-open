...
qboolean Image_LoadTGA(const char* name, const byte* buffer, fs_offset_t filesize)
{
	...
	buf_p += 2;
	targa_header.width = image.width = *(short*)buf_p;
	buf_p += 2;
	targa_header.height = image.height = *(short*)buf_p;
	buf_p += 2;
	...
	if ( targa_header.image_type == 1 || targa_header.image_type == 9 )
	{
		...
		{
			Con_DPrintf(
				S_ERROR "Image_LoadTGA: (%s) only 24 and 32 bit colormaps are supported for type 1 and 9\n",
				name);
			return false;
		}
	}
