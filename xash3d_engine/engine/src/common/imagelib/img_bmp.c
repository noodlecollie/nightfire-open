...
qboolean Image_LoadBMP(const char* name, const byte* buffer, fs_offset_t filesize)
{
	...
	if ( (fs_offset_t)bhdr.fileSize != filesize )
	{
		...
		Con_Reportf(
			S_WARN "Image_LoadBMP: %s have incorrect file size %li should be %i\n",
			name,
			filesize,
			bhdr.fileSize);
	}
	...
	{
		Con_Reportf(
			S_ERROR "Image_LoadBMP: %s have incorrect file size %li should be greater than %li (palette)\n",
			name,
			filesize,
			estimatedSize);
		return false;
	...
	if ( filesize < estimatedSize )
	{
		...
		Con_Reportf(
			S_ERROR "Image_LoadBMP: %s have incorrect file size %li should be greater than %li (pixels)\n",
			name,
			filesize,
			estimatedSize);
		return false;
