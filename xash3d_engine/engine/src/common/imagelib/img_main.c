...
static qboolean
Image_ProbeLoadBuffer_(const loadpixformat_t* fmt, const char* name, const byte* buf, size_t size, int override_hint)
{
	if ( override_hint > 0 )
...
static qboolean
Image_ProbeLoadBuffer(const loadpixformat_t* fmt, const char* name, const byte* buf, size_t size, int override_hint)
{
	if ( size <= 0 )
		return false;
...
qboolean FS_SaveImage(const char* filename, rgbdata_t* pix)
{
	...
	if ( pix->flags & (IMAGE_CUBEMAP | IMAGE_SKYBOX) )
	{
		...
		for ( format = image.saveformats; format && format->formatstring; format++ )
		{
			if ( !Q_stricmp(ext, format->ext) )
			{
				for ( i = 0; i < 6; i++ )
				{
					Q_snprintf(path, sizeof(path), format->formatstring, savename, box[i].suf, format->ext);
	...
	else
	{
		for ( format = image.saveformats; format && format->formatstring; format++ )
		{
			if ( !Q_stricmp(ext, format->ext) )
			{
				Q_snprintf(path, sizeof(path), format->formatstring, savename, "", format->ext);
				if ( format->savefunc(path, pix) )
				{
					// clear any force flags
					image.force_flags = 0;
