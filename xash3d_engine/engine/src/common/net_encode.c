...
static const delta_field_t pm_fields[] = {
	{PHYS_DEF(gravity)},      {PHYS_DEF(stopspeed)},     {PHYS_DEF(maxspeed)},        {PHYS_DEF(spectatormaxspeed)},
	{PHYS_DEF(accelerate)},   {PHYS_DEF(airaccelerate)}, {PHYS_DEF(wateraccelerate)}, {PHYS_DEF(friction)},
	{PHYS_DEF(edgefriction)}, {PHYS_DEF(waterfriction)}, {PHYS_DEF(bounce)},          {PHYS_DEF(stepsize)},
	{PHYS_DEF(maxvelocity)},  {PHYS_DEF(zmax)},          {PHYS_DEF(waveHeight)},      {PHYS_DEF(footsteps)},
	{PHYS_DEF(skyName)},      {PHYS_DEF(rollangle)},     {PHYS_DEF(rollspeed)},       {PHYS_DEF(skycolor_r)},
	{PHYS_DEF(skycolor_g)},   {PHYS_DEF(skycolor_b)},    {PHYS_DEF(skyvec_x)},        {PHYS_DEF(skyvec_y)},
	{PHYS_DEF(skyvec_z)},     {PHYS_DEF(fog_settings)},  {PHYS_DEF(wateralpha)},      {PHYS_DEF(skydir_x)},
	{PHYS_DEF(skydir_y)},     {PHYS_DEF(skydir_z)},      {PHYS_DEF(skyangle)},        {NULL},
};

static const delta_field_t ev_fields[] = {
	{EVNT_DEF(flags)},        {EVNT_DEF(entindex)},
	{EVNT_DEF(origin[0])},    {EVNT_DEF(origin[1])},
	{EVNT_DEF(origin[2])},    {EVNT_DEF(angles[0])},
	{EVNT_DEF(angles[1])},    {EVNT_DEF(angles[2])},
	{EVNT_DEF(velocity[0])},  {EVNT_DEF(velocity[1])},
	{EVNT_DEF(velocity[2])},  {EVNT_DEF(ducking)},
	{EVNT_DEF(fparam1)},      {EVNT_DEF(fparam2)},
	{EVNT_DEF(iparam1)},      {EVNT_DEF(iparam2)},
	{EVNT_DEF(bparam1)},      {EVNT_DEF(bparam2)},
	{EVNT_DEF(vecparam1[0])}, {EVNT_DEF(vecparam1[1])},
	{EVNT_DEF(vecparam1[2])}, {NULL},
};
...
static const delta_field_t wd_fields[] = {
	...
	{WPDT_DEF(m_flInaccuracy)},
	{WPDT_DEF(m_PrimaryAttackMechanicIndex)},
	{WPDT_DEF(m_SecondaryAttackMechanicIndex)},
	{WPDT_DEF(m_EnqueuedMechanicIndex)},
	{WPDT_DEF(m_EnqueuedAttackMode)},
	{WPDT_DEF(m_NextEnqueuedAttackStep)},
	{WPDT_DEF(iuser1)},
	{WPDT_DEF(iuser2)},
	{WPDT_DEF(iuser3)},
	{WPDT_DEF(iuser4)},
...
static const delta_field_t ent_fields[] = {
	{ENTS_DEF(entityType)},      {ENTS_DEF(origin[0])},
	{ENTS_DEF(origin[1])},       {ENTS_DEF(origin[2])},
	{ENTS_DEF(angles[0])},       {ENTS_DEF(angles[1])},
	{ENTS_DEF(angles[2])},       {ENTS_DEF(modelindex)},
	{ENTS_DEF(sequence)},        {ENTS_DEF(frame)},
	{ENTS_DEF(colormap)},        {ENTS_DEF(skin)},
	{ENTS_DEF(solid)},           {ENTS_DEF(effects)},
	{ENTS_DEF(scale)},           {ENTS_DEF(eflags)},
	{ENTS_DEF(rendermode)},      {ENTS_DEF(renderamt)},
	{ENTS_DEF(rendercolor.r)},   {ENTS_DEF(rendercolor.g)},
	{ENTS_DEF(rendercolor.b)},   {ENTS_DEF(renderfx)},
	{ENTS_DEF(movetype)},        {ENTS_DEF(animtime)},
	{ENTS_DEF(framerate)},       {ENTS_DEF(body)},
	{ENTS_DEF(controller[0])},   {ENTS_DEF(controller[1])},
	{ENTS_DEF(controller[2])},   {ENTS_DEF(controller[3])},
	{ENTS_DEF(blending[0])},     {ENTS_DEF(blending[1])},
	{ENTS_DEF(blending[2])},     {ENTS_DEF(blending[3])},
	{ENTS_DEF(velocity[0])},     {ENTS_DEF(velocity[1])},
	{ENTS_DEF(velocity[2])},     {ENTS_DEF(mins[0])},
	{ENTS_DEF(mins[1])},         {ENTS_DEF(mins[2])},
	{ENTS_DEF(maxs[0])},         {ENTS_DEF(maxs[1])},
	{ENTS_DEF(maxs[2])},         {ENTS_DEF(aiment)},
	{ENTS_DEF(owner)},           {ENTS_DEF(friction)},
	{ENTS_DEF(gravity)},         {ENTS_DEF(team)},
	{ENTS_DEF(playerclass)},     {ENTS_DEF(health)},
	{ENTS_DEF(spectator)},       {ENTS_DEF(weaponmodel)},
	{ENTS_DEF(gaitsequence)},    {ENTS_DEF(basevelocity[0])},
	{ENTS_DEF(basevelocity[1])}, {ENTS_DEF(basevelocity[2])},
	{ENTS_DEF(usehull)},         {ENTS_DEF(oldbuttons)},  // probably never transmitted
	{ENTS_DEF(onground)},        {ENTS_DEF(iStepLeft)},
	{ENTS_DEF(flFallVelocity)},  {ENTS_DEF(fov)},
	{ENTS_DEF(weaponanim)},      {ENTS_DEF(startpos[0])},
	{ENTS_DEF(startpos[1])},     {ENTS_DEF(startpos[2])},
	{ENTS_DEF(endpos[0])},       {ENTS_DEF(endpos[1])},
	{ENTS_DEF(endpos[2])},       {ENTS_DEF(impacttime)},
	{ENTS_DEF(starttime)},       {ENTS_DEF(iuser1)},
	{ENTS_DEF(iuser2)},          {ENTS_DEF(iuser3)},
	{ENTS_DEF(iuser4)},          {ENTS_DEF(fuser1)},
	{ENTS_DEF(fuser2)},          {ENTS_DEF(fuser3)},
	{ENTS_DEF(fuser4)},          {ENTS_DEF(vuser1[0])},
	{ENTS_DEF(vuser1[1])},       {ENTS_DEF(vuser1[2])},
	{ENTS_DEF(vuser2[0])},       {ENTS_DEF(vuser2[1])},
	{ENTS_DEF(vuser2[2])},       {ENTS_DEF(vuser3[0])},
	{ENTS_DEF(vuser3[1])},       {ENTS_DEF(vuser3[2])},
	{ENTS_DEF(vuser4[0])},       {ENTS_DEF(vuser4[1])},
	{ENTS_DEF(vuser4[2])},       {NULL},
};
...
void Delta_InitFields(void)
{
	...
	if ( !afile )
		Sys_Error("DELTA_Load: couldn't load file %s\n", DELTA_PATH);
...
#ifdef _DEBUG
	if ( numbits < 32 && (uint)abs(iValue) >= (uint)BIT(numbits) )
	{
		Msg("Network field value overflow (source '%s' field '%s'): abs value %u is out of range of %d-bit container "
			"(max value %u).\n",
...
qboolean Delta_CompareField(delta_t* pField, void* from, void* to, double timebase)
{
	...
	else if ( pField->flags & DT_SHORT )
	{
		...
		fromF = (int)Delta_ClampIntegerField(pField, fromF, bSigned, pField->bits);
		toF = (int)Delta_ClampIntegerField(pField, toF, bSigned, pField->bits);
	...
	{
		val_a = (float)Q_rint((*(float*)((byte*)from + pField->offset)) * 100.0);
		val_b = (float)Q_rint((*(float*)((byte*)to + pField->offset)) * 100.0);
	...
	else if ( pField->flags & DT_TIMEWINDOW_BIG )
	{
		...
		if ( !Q_equal(pField->multiplier, 1.0) )
		{
			...
			val_a = (float)((timebase * pField->multiplier) - val_a);
			val_b = (float)((timebase * pField->multiplier) - val_b);
		}
		else
		{
			val_a = (float)(timebase - val_a);
			val_b = (float)(timebase - val_b);
		}
...
int Delta_TestBaseline(entity_state_t* from, entity_state_t* to, qboolean player, double timebase)
{
	...
	for ( i = 0; i < dt->numFields; i++, pField++ )
	{
		...
		if ( !Delta_CompareField(pField, from, to, timebase) )
		{
			...
			{
				countBits += (int)Q_strlen((char*)((byte*)to + pField->offset)) * 8;
			}
			else
			{
...
qboolean Delta_WriteField(sizebuf_t* msg, delta_t* pField, void* from, void* to, double timebase)
{
	...
	if ( pField->flags & DT_BYTE )
	{
		...
		if ( !Q_equal(pField->multiplier, 1.0) )
		{
			iValue = (uint)(iValue * pField->multiplier);
		}
...
qboolean Delta_ReadField(sizebuf_t* msg, delta_t* pField, void* from, void* to, double timebase)
{
	...
	if ( pField->flags & DT_BYTE )
	{
		...
		if ( bSigned )
		{
			*(int8_t*)((uint8_t*)to + pField->offset) = (int8_t)iValue;
		}
		else
		{
			*(uint8_t*)((uint8_t*)to + pField->offset) = (uint8_t)iValue;
		}
	}
	else if ( pField->flags & DT_SHORT )
	{
		...
		if ( bSigned )
		{
			*(int16_t*)((uint8_t*)to + pField->offset) = (int16_t)iValue;
		}
		else
		{
			*(uint16_t*)((uint8_t*)to + pField->offset) = (uint16_t)iValue;
		}
	}
	else if ( pField->flags & DT_INTEGER )
	{
	...
	else if ( pField->flags & DT_TIMEWINDOW_8 )
	{
		bSigned = true;  // timewindow is always signed
		iValue = MSG_ReadBitLong(msg, pField->bits, bSigned);
		flTime = (float)((timebase * 100.0 - (int)iValue) / 100.0);
	...
	else if ( pField->flags & DT_TIMEWINDOW_BIG )
	{
		...
		if ( !Q_equal(pField->multiplier, 1.0) )
		{
			flTime = (float)((timebase * pField->multiplier - (int)iValue) / pField->multiplier);
		}
		else
		{
			flTime = (float)(timebase - (int)iValue);
		}
