...
{
	// If the first item in the list is empty,
	// this means the hard-coded master server address
	// is empty, implying it is disabled.

	return !ml.list || !ml.list->address[0];
}
...
static void NET_AnnounceToMaster(master_t* m)
{
	...
	{
		Con_Printf(
			S_NOTE "sent heartbeat to %s (%s, 0x%x)\n",
			m->address,
			NET_AdrToString(m->adr),
			m->heartbeat_challenge);
	}
}
...
void NET_SaveMasters(void)
{
	...
	if ( NET_MasterServersDisabled() )
	{
		Con_Reportf("Master servers are disabled in this build. Not writing master server list.\n");
		return;
	}
