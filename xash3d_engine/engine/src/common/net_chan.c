...
qboolean NetSplit_GetLong(netsplit_t* ns, netadr_t* from, byte* data, size_t* length)
{
	...
	// (int)packet->id, (int)packet->index, (int)*length );
	...
	if ( p->received == packet->count )
	{
		...
		*length = len;
		...
		// ), (int)packet->id, (int)p->received, (int)packet->length );
...
void NetSplit_SendLong(netsrc_t sock, size_t length, void* data, netadr_t to, unsigned int maxpacket, unsigned int id)
{
	...
	while ( packet.index < packet.count )
	{
		...
		// (int)packet.id, (int)packet.index );
...
void Netchan_ReportFlow(netchan_t* chan)
{
	...
	Q_strcpy(incoming, sizeof(incoming), Q_pretifymem((float)chan->flow[FLOW_INCOMING].totalbytes, 3));
	Q_strcpy(outgoing, sizeof(outgoing), Q_pretifymem((float)chan->flow[FLOW_OUTGOING].totalbytes, 3));
...
void Netchan_TransmitBits(netchan_t* chan, int length, byte* data)
{
	...
	// frag payload can be a file chunk, if so, it needs to be parsed on the receiving end and reliable payload +
	// unreliable payload need to be passed on to the message queue.  The processing routine needs to be able to handle
	// the case where a message comes in and a file transfer completes
	...
	if ( MSG_GetNumBytesWritten(&send) < 16 &&
		 !NET_IsLocalAddress(chan->remote_address) )  // packet too small for some networks
	{
