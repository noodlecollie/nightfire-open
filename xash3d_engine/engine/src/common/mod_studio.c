...
====================
*/
static void Mod_AddToStudioCache(
	float frame,
	int sequence,
	vec3_t angles,
	vec3_t origin,
...
====================
*/
static mstudiocache_t* Mod_CheckStudioCache(
	model_t* model,
	float frame,
	int sequence,
	vec3_t angles,
...
====================
*/
static void Mod_StudioCalcRotations(
	int boneused[],
	int numbones,
...
void* R_StudioGetAnim(studiohdr_t* m_pStudioHeader, model_t* m_pSubModel, mstudioseqdesc_t* pseqdesc)
{
	...
	if ( !Mod_CacheCheck((cache_user_t*)&(paSequences[pseqdesc->seqgroup])) )
	{
		...
		COM_ExtractFilePath(m_pSubModel->name, modelpath, sizeof(modelpath));
...
====================
*/
static void SV_StudioSetupBones(
	model_t* pModel,
	float frame,
	int sequence,
	const vec3_t angles,
...
int Mod_StudioGetBodyGroupCount(model_t* model)
{
	...
	header = (const studiohdr_t*)Mod_StudioExtradata(model);
	return header ? header->numbodyparts : 0;
}
...
qboolean Mod_StudioGetSequenceBounds(model_t* model, int anim, vec3_t outVecMins, vec3_t outVecMaxs)
{
	...
	if ( outVecMins )
	{
		VectorCopy(sequence->bbmin, outVecMins);
	}

	if ( outVecMaxs )
	{
		VectorCopy(sequence->bbmax, outVecMaxs);
	}
...
void Mod_LoadStudioModel(model_t* mod, const void* buffer, qboolean* loaded)
{
	...
	if ( !Host_IsDedicated() )
	{
		...
		else
		{
			...
			// NOTE: we dont't want to keep raw textures in memory. just cutoff model pointer above texture base
...
void Mod_InitStudioAPI(void)
{
	...
	if ( pBlendIface &&
		 pBlendIface(SV_BLENDING_INTERFACE_VERSION, &pBlendAPI, &gStudioAPI, &studio_transform, &studio_bones) )
	{
