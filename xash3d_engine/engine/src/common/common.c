...
=============
*/
byte* GAME_EXPORT COM_LoadFile(const char* filename, int* pLength)
{
	string name;
	byte *file, *pfile;
	fs_offset_t iLength;
...
=============
*/
byte* COM_LoadFile2(const char* filename, int usehunk, int* pLength)
{
	(void)usehunk;
	return COM_LoadFile(filename, pLength);
}
