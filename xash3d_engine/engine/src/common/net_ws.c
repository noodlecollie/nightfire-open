...
=============
*/
static net_gai_state_t
NET_StringToSockaddr(const char* s, struct sockaddr_storage* sadr, qboolean nonblocking, int family)
{
	int ret = 0, port;
	char* colon;
	char copy[128];
	...
	else
	{
		...
		{
			memcpy(
				&((struct sockaddr_in6*)sadr)->sin6_addr,
...
qboolean NET_GetLong(byte* pData, int size, size_t* outSize, int splitsize)
{
	...
	if ( net.split_flags[packet_number] != sequence_number )
	{
		...
		if ( net_showpackets && net_showpackets->value == 4.0f )
			Con_Printf(
				"<-- Split packet %i of %i, %i bytes %i seq\n",
				packet_number + 1,
				packet_count,
				size,
				sequence_number);
	...
	{
		Con_DPrintf(
			"NET_GetLong: Ignoring duplicated split packet %i of %i ( %i bytes )\n",
			packet_number + 1,
			packet_count,
			size);
	}
...
void NET_Init(void)
{
	...
	net6_address = Cvar_Get(
		"net6_address",
		"0",
		FCVAR_PRIVILEGED | FCVAR_READ_ONLY,
...
static void HTTP_FreeFile(httpfile_t* file, qboolean error)
{
	...
	if ( error )
	{
		...
		if ( http_autoremove->value == 1 )  // remove broken file
			...
			Con_Printf(
				"cannot download %s from any server. "
...
static qboolean HTTP_ProcessStream(httpfile_t* curfile)
{
	...
	while ( (res = recv(curfile->socket, buf, BUFSIZ - curfile->header_size, 0)) >
			0 )  // if we got there, we are receiving data
	{
		curfile->blocktime = 0;
...
void HTTP_Run(void)
{
	...
	for ( curfile = http.first_file; curfile; curfile = curfile->next )
	{
		...
		if ( curfile->state < HTTP_REQUEST )  // Request not formatted
		{
			...
			curfile->query_length = Q_snprintf(
				curfile->buf,
				sizeof(curfile->buf),
				"GET %s%s HTTP/1.0\r\n"
				"Host: %s\r\n"
		...
		if ( curfile->state < HTTP_REQUEST_SENT )  // Request not sent
		{
			...
			while ( curfile->bytes_sent < curfile->query_length )
			{
				int res = send(
					curfile->socket,
					curfile->buf + curfile->bytes_sent,
					curfile->query_length - curfile->bytes_sent,
					0);
