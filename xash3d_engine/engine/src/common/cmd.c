...
*/
static int
Cmd_AddCommandEx(const char* funcname, const char* cmd_name, xcommand_t function, const char* cmd_desc, int iFlags)
{
	cmd_t *cmd, *cur, *prev;
...
static void Cmd_ExecuteStringWithPrivilegeCheck(const char* text, qboolean isPrivileged)
{
	...
	if ( !host.apply_game_config )
	{
		...
		if ( a )
		{
			Cbuf_InsertTextToBuffer(isPrivileged ? &cmd_text : &filteredcmd_text, a->value);
			return;
		}
	}
...
void Cmd_ForwardToServer(void)
{
	...
	{
		Q_strcat(str, sizeof(str), Cmd_Argv(0));
		Q_strcat(str, sizeof(str), " ");
	}

	if ( Cmd_Argc() > 1 )
	{
		Q_strcat(str, sizeof(str), Cmd_Args());
	}
	else
	{
		Q_strcat(str, sizeof(str), "\n");
