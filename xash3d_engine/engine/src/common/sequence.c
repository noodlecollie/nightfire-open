...
static size_t Sequence_GetNameValueString(char* token, size_t len)
{
	...
	if ( !Sequence_IsNameValueChar(*g_scan) )
	{
		if ( *g_scan == '#' || *g_scan == '$' )
			Con_Reportf(
				S_ERROR
				"Parsing error on line %d of %s.seq: cannot have more than one '%c' per line; '%c' must be at the "
			...
			Con_Reportf(
				S_ERROR "Parsing error on line %d of %s.seq: expected name/value, found illegal character '%c'\n",
				g_lineNum,
				g_sequenceParseFileName,
				*g_scan);
	}
...
static void Sequence_ValidateNameValueString(char* token)
{
	...
	for ( scan = token; *scan; scan++ )
	{
		if ( !Sequence_IsNameValueChar(*scan) )
			Con_Reportf(
				S_ERROR "Parsing error on line %d of %s.seq: name/value string \"%s\" had illegal character '%c'\n",
				g_lineNum,
				g_sequenceParseFileName,
				token,
				*scan);
...
static size_t Sequence_GetToken(char* token, size_t size)
{
	...
	if ( !Sequence_IsSymbol(*g_scan) )
		Con_Reportf(
			S_ERROR "Parsing error on line %d of %s.seq: expected token, found '%c' instead\n",
			g_lineNum,
			g_sequenceParseFileName,
			*g_scan);
...
static size_t Sequence_GetLine(char* line, int lineMaxLen)
{
	...
	if ( !read )
		Con_Reportf(
			S_ERROR "Syntax Error on line %d of %s.seq: expected sentence definition or '}', found End-Of-File!\n",
			g_lineNum,
			g_sequenceParseFileName);
	...
	if ( lineLen >= lineMaxLen )
		Con_Reportf(
			S_ERROR "Syntax Error on line %d of %s.seq: line was too long (was %d chars; max is %d chars)\n",
			g_lineNum,
			g_sequenceParseFileName,
			lineLen,
			lineMaxLen - 1);
...
static qboolean Sequence_IsCommandAModifier(sequenceCommandEnum_e commandEnum)
{
	...
	Con_Reportf(
		S_ERROR "Internal error caused by line %d of %s.seq: unknown command enum = %d\n",
		g_lineNum,
		g_sequenceParseFileName,
		commandEnum);
	return false;
...
static char Sequence_ParseModifier(sequenceCommandLine_s* defaults)
{
	...
	if ( modifierEnum == SEQUENCE_COMMAND_ERROR )
		Con_Reportf(
			S_ERROR "Parsing error on line %d of %s.seq: unknown modifier \"%s\"\n",
			g_lineNum,
			g_sequenceParseFileName,
			modifierName);
	...
	if ( !Sequence_IsCommandAModifier(modifierEnum) )
		Con_Reportf(
			S_ERROR "Parsing error on line %d of %s.seq: \"%s\" is a #command, not a $modifier\n",
			g_lineNum,
			g_sequenceParseFileName,
			modifierName);
	...
	if ( delimiter != '=' )
		Con_Reportf(
			S_ERROR "Parsing error on or after line %d of %s.seq: after modifier \"%s\", expected '=', found '%c'\n",
			g_lineNum,
			g_sequenceParseFileName,
			modifierName,
			delimiter);
	...
	if ( !Sequence_ConfirmCarriageReturnOrSymbol(',') )
		Con_Reportf(
			S_ERROR
			"Parsing error on line %d of %s.seq: after value(s) for modifier \"%s\", expected ',' or End-Of-Line; "
			"found '%c'\n",
			g_lineNum,
			g_sequenceParseFileName,
			modifierName,
...
static char Sequence_ParseCommand(sequenceCommandLine_s* newCommandLine)
{
	...
	if ( commandEnum == SEQUENCE_COMMAND_ERROR )
		Con_Reportf(
			S_ERROR "Parsing error on line %d of %s.seq: unknown command \"%s\"\n",
			g_lineNum,
			g_sequenceParseFileName,
			commandName);
	...
	if ( ch != '=' )
		Con_Reportf(
			S_ERROR "Parsing error on or before line %d of %s.seq: after command \"%s\", expected '=', found '%c'\n",
			g_lineNum,
			g_sequenceParseFileName,
			commandName,
			ch);
...
static char Sequence_ParseLine(char start, sequenceEntry_s* entry)
{
	...
	switch ( start )
	{
		...
		default:
			Con_Reportf(
				S_ERROR
				"Parsing error on line %d of %s.seq: line must begin with either '#' (command) or '$' (modifier); "
				"found '%c'\n",
				g_lineNum,
				g_sequenceParseFileName,
				start);
...
static void Sequence_CreateDefaultsCommand(sequenceEntry_s* entry)
{
	...
	cmd->modifierBitField = SEQUENCE_MODIFIER_EFFECT_BIT | SEQUENCE_MODIFIER_POSITION_BIT |
		SEQUENCE_MODIFIER_COLOR_BIT | SEQUENCE_MODIFIER_COLOR2_BIT | SEQUENCE_MODIFIER_FADEIN_BIT |
		SEQUENCE_MODIFIER_FADEOUT_BIT | SEQUENCE_MODIFIER_HOLDTIME_BIT | SEQUENCE_MODIFIER_FXTIME_BIT;
...
static char Sequence_ParseEntry(void)
{
	...
	if ( symbol != '{' )
		Con_Reportf(
			S_ERROR
			"Parsing error on line %d of %s.seq: expected '{' to start a\n new entry block; found '%c' instead!",
			g_lineNum,
			g_sequenceParseFileName,
			symbol);
	...
	if ( !Sequence_IsEntrySafe(entry) )
		Con_Reportf(
			S_ERROR
			"Logic error in file %s.seq before line %d: execution of entry \"%%%s\" would cause an infinite loop!",
			g_sequenceParseFileName,
			g_lineNum,
			entry->entryName);
...
static void Sequence_AddSentenceToGroup(char* groupName, char* data)
{
	...
	if ( !group )
	{
		...
		if ( !group )
			Con_Reportf(
				S_ERROR "Unable to allocate sentence group %s at line %d in file %s.seq",
				groupName,
				g_lineNum,
				g_sequenceParseFileName);
	}
...
static char Sequence_ParseGlobalDataBlock(void)
{
	...
	if ( Q_stricmp(token, "Sentences") )
		Con_Reportf(
			S_ERROR
			"Syntax error in file %s.seq on line %d: found global data block symbol '!' with unknown data type \"%s\"",
			g_sequenceParseFileName,
			g_lineNum,
			token);
...
static qboolean Sequence_ExpandGosubsForEntry(sequenceEntry_s* entry)
{
	...
	for ( cmd = entry->firstCommand; cmd; cmd = cmd->nextCommandLine )
	{
		...
		if ( !gosubEntry )
			Con_Reportf(
				S_ERROR "Error in %s.seq: Gosub in entry \"%s\" specified unknown entry \"%s\"\n",
				entry->fileName,
				entry->entryName,
	...
	while ( symbol )
	{
		switch ( symbol )
		{
			...
			default:
				Con_Reportf(
					S_ERROR
					"Parsing error on line %d of %s.seq: At file scope, lines must begin with '$' (modifier) or '%%' "
					"(entry block) or '!' (sentence / global data block); found '%c'\n",
					g_lineNum,
					g_sequenceParseFileName,
					symbol);
		}
...
static void Sequence_ParseFile(const char* fileName, qboolean isGlobal)
{
	...
	Q_strcpy(g_sequenceParseFileName, sizeof(g_sequenceParseFileName), fileName);
...
sequenceEntry_s* Sequence_Get(const char* fileName, const char* entryName)
{
	...
	{
		if ( (!fileName ||
			  !Q_stricmp(fileName, scan->fileName)) &&  // a1ba: add filename check, even if originally it is ignored
