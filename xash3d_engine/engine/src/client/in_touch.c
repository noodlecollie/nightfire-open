...
static inline int Touch_ExportButtonToConfig(file_t* f, touch_button_t* button, qboolean keepAspect)
{
	...
	FS_Printf(
		f,
		"touch_addbutton \"%s\" \"%s\" \"%s\" %f %f %f %f %d %d %d %d %d",
		B(name),
		B(texturefile),
		newCommand,
		B(x1),
...
static void Touch_GenerateCode_f(void)
{
	...
	for ( button = touch.list_user.first; button; button = button->next )
	{
		...
		Con_Printf(
			"TOUCH_ADDDEFAULT( \"%s\", \"%s\", \"%s\", %f, %f, %f, %f, color, %d, %f, %d );\n",
			B(name),
			B(texturefile),
			B(command),
			B(x1),
			...
			B(x2),
			B(y2),
			(B(type) == touch_command) ? (fabs(aspect - 1.0f) < 0.0001) ? 2 : 1 : 0,
			aspect,
			flags);
	}
}
...
static void
Touch_FadeButtons(touchbuttonlist_t* list, const char* name, float speed, float end, float start, qboolean privileged)
{
	touch_button_t* button;
	for ( button = list->first; button; button = button->next )
	{
...
static touch_button_t* Touch_AddButton(
	touchbuttonlist_t* list,
	const char* name,
	const char* texture,
	const char* command,
	float x1,
...
static void Touch_LoadDefaults_f(void)
{
	...
	{
		touch_button_t* button;
		float x1 = g_DefaultButtons[i].x1, y1 = g_DefaultButtons[i].y1, x2 = g_DefaultButtons[i].x2,
		...
		if ( g_DefaultButtons[i].aspect && g_DefaultButtons[i].round == round_aspect )
		{
			if ( g_DefaultButtons[i].texturefile[0] == '#' )
				y2 = y1 +
					((float)clgame.scrInfo.iCharHeight / (float)clgame.scrInfo.iHeight) * g_DefaultButtons[i].aspect +
					touch.swidth * 2 / SCR_H;
			else
				y2 = y1 + (x2 - x1) * (SCR_W / SCR_H) * g_DefaultButtons[i].aspect;
		}

		IN_TouchCheckCoords(&x1, &y1, &x2, &y2);
		button = Touch_AddButton(
			&touch.list_user,
			g_DefaultButtons[i].name,
			g_DefaultButtons[i].texturefile,
			g_DefaultButtons[i].command,
			x1,
			y1,
			x2,
			y2,
			g_DefaultButtons[i].color,
			true);
...
static void Touch_InitEditor(void)
{
	...
	temp = Touch_AddButton(
		&touch.list_edit,
		"close",
		"touch_default/edit_close",
		"touch_disableedit",
		0,
		y,
		x,
	...
	touch.hidebutton = Touch_AddButton(
		&touch.list_edit,
		"showhide",
		"touch_default/edit_hide",
		"touch_toggleselection",
		0,
		y,
		x,
		y + 0.1f,
...
void Touch_Init(void)
{
	...
	touch_exp_mult =
		Cvar_Get("touch_exp_mult", "0", FCVAR_FILTERABLE, "exponent multiplier, usually 20-200, 0 to disable");
	...
	touch_config_file =
		Cvar_Get("touch_config_file", "touch.cfg", FCVAR_ARCHIVE | FCVAR_PRIVILEGED, "current touch profile file");
	touch_precise_amount =
		Cvar_Get("touch_precise_amount", "0.5", FCVAR_FILTERABLE, "sensitivity multiplier for precise-look");
	...
	touch_move_indicator =
		Cvar_Get("touch_move_indicator", "0.0", FCVAR_FILTERABLE, "indicate move events (0 to disable)");
	touch_joy_texture =
		Cvar_Get("touch_joy_texture", "touch_default/joy", FCVAR_FILTERABLE, "texture for move indicator");
...
static void Touch_DrawTexture(float x1, float y1, float x2, float y2, int texture, byte r, byte g, byte b, byte a)
{
	...
	ref.dllFuncs
		.R_DrawStretchPic(TO_SCRN_X(x1), TO_SCRN_Y(y1), TO_SCRN_X(x2 - x1), TO_SCRN_Y(y2 - y1), 0, 0, 1, 1, texture);
}
		...
		if ( Touch_IsVisible(button) )
		{
			...
			if ( button->texturefile[0] == '#' )
				Touch_DrawText(
					...
					touch.swidth / SCR_H + B(y1),
					B(x2),
					B(y2),
					button->texturefile + 1,
					color,
					B(aspect) ? B(aspect) : 1);
...
void Touch_Draw(void)
{
	...
	if ( (touch.move_finger != -1) && touch.move_button && touch_move_indicator->value )
	{
		...
		ref.dllFuncs.R_DrawStretchPic(
			TO_SCRN_X(touch.move_start_x - GRID_X * touch_move_indicator->value),
			TO_SCRN_Y(touch.move_start_y - GRID_Y * touch_move_indicator->value),
			TO_SCRN_X(GRID_X * 2 * touch_move_indicator->value),
			TO_SCRN_Y(GRID_Y * 2 * touch_move_indicator->value),
			0,
			0,
			1,
			1,
		...
		ref.dllFuncs.Color4ub(255, 255, 255, 255);
		ref.dllFuncs.R_DrawStretchPic(
			TO_SCRN_X(touch.move_start_x + touch.side * width - GRID_X * touch_move_indicator->value),
...
static qboolean
Touch_ButtonPress(touchbuttonlist_t* list, touchEventType type, int fingerID, float x, float y, float dx, float dy)
{
	touch_button_t* button;
	...
	for ( button = list->last; button; button = button->prev )
	{
		...
		{
			// button bounds check
			if ( (x > button->x1 && x < button->x2) && (y < button->y2 && y > button->y1) )
			{
