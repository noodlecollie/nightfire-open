...
static BOOL(
	_stdcall* pDrawDibDraw)(HDRAWDIB, HDC, int, int, int, int, LPBITMAPINFOHEADER, void*, int, int, int, int, uint);
...
static MMRESULT(
	_stdcall*
		pacmStreamOpen)(LPHACMSTREAM, HACMDRIVER, LPWAVEFORMATEX, LPWAVEFORMATEX, LPWAVEFILTER, DWORD, DWORD, DWORD);
...
static int(_stdcall* pAVIStreamRead)(
	PAVISTREAM pavi,
	LONG lStart,
	LONG lSamples,
	void* lpBuffer,
...
qboolean AVI_ACMConvertAudio(movie_state_t* Avi)
{
	...
	pAVIStreamRead(
		Avi->audio_stream,
		Avi->cpa_blocknum * Avi->cpa_blockalign,
		Avi->cpa_blockalign,
		Avi->cpa_srcbuffer,
		Avi->cpa_blockalign,
		NULL,
...
int AVI_GetAudioChunk(movie_state_t* Avi, char* audiodata, int offset, int length)
{
	...
	if ( Avi->audio_codec == WAVE_FORMAT_PCM )
	{
		...
		pAVIStreamRead(
			Avi->audio_stream,
			offset / Avi->audio_bytes_per_sample,
...
void AVI_OpenVideo(movie_state_t* Avi, const char* filename, qboolean load_audio, int quiet)
{
	...
	while ( 1 )
	{
		...
		else if ( stream_info.fccType == streamtypeAUDIO && Avi->audio_stream == NULL && load_audio )
		{
			...
			if ( Avi->audio_codec != WAVE_FORMAT_PCM )
			{
				...
				Avi->audio_bytes_per_sample = Avi->audio_header->nBlockAlign;
	...
	Avi->hBitmap =
		CreateDIBSection(Avi->hDC, (BITMAPINFO*)(&bmih), DIB_RGB_COLORS, (void**)(&Avi->pframe_data), NULL, 0);
