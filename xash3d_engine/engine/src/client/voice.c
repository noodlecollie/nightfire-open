...
static uint Voice_GetOpusCompressedData(byte* out, uint maxsize, uint* frames)
{
		  ...
		  ofs += frame_size_bytes )
	{
		...
		bytes = opus_custom_encode(
			voice.encoder,
			(const opus_int16*)(voice.input_buffer + ofs),
			voice.frame_size,
...
static void Voice_ApplyGainAdjust(int16_t* samples, int count)
{
	...
	for ( ;; )
	{
		...
		if ( blockOffset % voice.autogain.block_size == 0 )
		{
			...
			voice.autogain.gain_multiplier =
				(voice.autogain.next_gain - voice.autogain.current_gain) / (voice.autogain.block_size - 1);
		}
...
void Voice_AddIncomingData(int ent, const byte* data, uint size, uint frames)
{
	...
	for ( ;; )
	{
		...
		frame_samples = opus_custom_decode(
			...
			data + ofs,
			compressed_size,
			(opus_int16*)voice.decompress_buffer + samples,
			voice.frame_size);
