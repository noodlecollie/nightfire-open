	...
	while ( 1 )
	{
		...
		if ( !R_BeamPoints(p1, p2, modelIndex, 0, 2, 0, 255, 0, 0, 0, 255.0f, 0.0f, 0.0f) )
		{
			if ( !model || model->type != mod_sprite )
				...
				Con_Printf(S_ERROR "not enough free beams!\n");
			break;
		}
	}
...
==============
*/
BEAM* GAME_EXPORT R_BeamCirclePoints(
	int type,
	vec3_t start,
	vec3_t end,
	int modelIndex,
...
==============
*/
BEAM* GAME_EXPORT R_BeamEntPoint(
	int startEnt,
	vec3_t end,
	int modelIndex,
	float life,
...
*/
void GAME_EXPORT
R_StreakSplash(const vec3_t pos, const vec3_t dir, int color, int count, float speed, int velocityMin, int velocityMax)
{
	vec3_t vel, vel2;
	particle_t* p;
	int i;
...
===============
*/
void GAME_EXPORT R_UserTracerParticle(
	float* org,
	float* vel,
	float life,
	int colorIndex,
