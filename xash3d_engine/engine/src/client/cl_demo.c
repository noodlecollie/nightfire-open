...
int GAME_EXPORT CL_GetDemoComment(const char* demoname, char* comment)
{
	...
	if ( (demohdr.net_protocol != PROTOCOL_VERSION && demohdr.net_protocol != PROTOCOL_LEGACY_VERSION) ||
...
==================
*/
static void CL_DemoGetName(int lastnum, char* filename, size_t bufferSize)
{
	lastnum = bound(0, lastnum, 9999);
	Q_snprintf(filename, bufferSize, "demo%04d", lastnum);
}
...
void CL_PlayDemo_f(void)
{
	...
	{
		Con_Printf(
			S_ERROR "playdemo: demo protocol outdated (%i should be %i)\n",
	...
	if ( demo.header.net_protocol != PROTOCOL_VERSION && demo.header.net_protocol != PROTOCOL_LEGACY_VERSION )
	{
		Con_Printf(
			S_ERROR "playdemo: net protocol outdated (%i should be %i)\n",
