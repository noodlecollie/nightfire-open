...
static int SND_GetChannelTimeLeft(const channel_t* ch)
{
	...
	if ( ch->isSentence )  // sentences are special, count all remaining words
	{
		...
		// see VOX_LoadWord
		for ( i = ch->wordIndex + 1; i < SIZE_OF_ARRAY(ch->words); i++ )
		{
			wavdata_t* sc;
			int end;
...
===================
*/
static uint S_RawSamplesStereo(
...
void S_StreamAviSamples(void* Avi, int entnum, float fvol, float attn, float synctime)
{
	...
	while ( ch->s_rawend < soundtime + ch->max_samples )
	{
		...
		if ( r > 0 )
		{
			...
			break;  // no more samples for this frame
	}
}
	...
	if ( s_show.value != 0.0f )
	{
		...
		for ( i = 0, total = 1, ch = channels; i < MAX_CHANNELS; i++, ch++ )
		{
			if ( ch->sfx && (ch->leftvol || ch->rightvol) )
			{
				...
				Con_NXPrintf(
					&info,
					"chan %i, pos (%.f %.f %.f) ent %i, lv%3i rv%3i %s\n",
					i,
					ch->origin[0],
					ch->origin[1],
					ch->origin[2],
		...
		Con_NXPrintf(
			&info,
			"room_type: %i (%s) ----(%i)---- painted: %i\n",
			idsp_room,
			Cvar_VariableString("dsp_coeff_table"),
			total - 1,
			paintedtime);
	}
