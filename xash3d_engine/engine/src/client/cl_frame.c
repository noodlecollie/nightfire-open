...
*/
qboolean
CL_FindInterpolationUpdates(cl_entity_t* ent, float targettime, position_history_t** ph0, position_history_t** ph1)
{
...
void CL_LinkPacketEntities(frame_t* frame)
{
	...
	for ( i = 0; i < frame->num_entities; i++ )
	{
		...
		if ( !parametric && ent->curstate.movetype != MOVETYPE_COMPOUND )
		{
			...
			{
				if ( ent->curstate.animtime != 0.0f &&
					 (ent->model->type == mod_alias || ent->model->type == mod_studio) )
				{
