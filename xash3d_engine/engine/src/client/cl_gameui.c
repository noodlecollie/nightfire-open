...
static void GAME_EXPORT UI_DrawLogo(const char* filename, float x, float y, float width, float height)
{
	...
	if ( !AVI_IsActive(cin_state) )
	{
		...
		COM_DefaultExtension(path, sizeof(path), ".avi");
...
static int pfnGetRenderers(size_t index, char* shortName, size_t size1, char* readableName, size_t size2)
{
	if ( index >= ref.numRenderers )
	{
		return 0;
	}

	if ( shortName && size1 > 0 )
	{
		Q_strncpy(shortName, ref.rendererNames[index].shortName, size1);
	}

	if ( readableName && size2 > 0 )
	{
		Q_strncpy(readableName, ref.rendererNames[index].readableName, size2);
	}
...
qboolean UI_LoadProgs(void)
{
	...
	else  // otherwise, fallback to old and deprecated extensions
	{
		...
		if ( GiveTextApi )
		{
			...
			Con_Reportf(S_WARN
						"Text API is deprecated! If you are mod developer, consider moving to Extended Menu API!\n");
			if ( GiveTextApi(
		...
		gameui.dllFuncs2.pfnAddTouchButtonToList =
			(ADDTOUCHBUTTONTOLIST)COM_GetProcAddress(gameui.hInstance, "AddTouchButtonToList");
		if ( gameui.dllFuncs2.pfnAddTouchButtonToList )
		{
