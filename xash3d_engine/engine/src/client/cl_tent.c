...
#include "EngineGamesAPI/wadfile.h"  // acess decal size
...
void CL_PrepareTEnt(TEMPENTITY* pTemp, model_t* pmodel)
{
	...
	pTemp->frameMax = (float)Q_max(0, frameCount - 1);
...
==============
*/
TEMPENTITY* GAME_EXPORT
CL_TempEntAllocCustom(const vec3_t org, model_t* model, int high, void (*pfn)(TEMPENTITY*, float, float))
{
	TEMPENTITY* pTemp;
...
void GAME_EXPORT R_FizzEffect(cl_entity_t* pent, int modelIndex, int density)
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		pTemp->entity.curstate.frame = (float)COM_RandomLong(0, (int)pTemp->frameMax);
		// Set sprite scale
...
void GAME_EXPORT R_Bubbles(const vec3_t mins, const vec3_t maxs, float height, int modelIndex, int count, float speed)
{
	...
	for ( i = 0; i < count; i++ )
	{
		origin[0] = (float)COM_RandomLong((int)mins[0], (int)maxs[0]);
		origin[1] = (float)COM_RandomLong((int)mins[1], (int)maxs[1]);
		origin[2] = (float)COM_RandomLong((int)mins[2], (int)maxs[2]);
		...
		pTemp->x = origin[0];
		pTemp->y = origin[1];
		angle = COM_RandomFloat(-M_PI_F, M_PI_F);
...
*/
void GAME_EXPORT
R_BubbleTrail(const vec3_t start, const vec3_t end, float height, int modelIndex, int count, float speed)
{
	TEMPENTITY* pTemp;
	...
	for ( i = 0; i < count; i++ )
	{
		...
		zspeed = (float)COM_RandomLong(80, 140);
		VectorSet(pTemp->entity.baseline.origin, speed * cosine, speed * sine, zspeed);
		pTemp->die = (float)cl.time + ((height - (origin[2] - start[2])) / zspeed) - 0.1f;
		pTemp->entity.curstate.frame = (float)COM_RandomLong(0, (int)pTemp->frameMax);
...
void GAME_EXPORT R_MuzzleFlash(const vec3_t pos, int type)
{
	...
	if ( index == 0 )
	{
		pTemp->entity.angles[2] = (float)COM_RandomLong(0, 20);  // rifle flash
	}
	else
	{
...
void GAME_EXPORT R_BloodSprite(const vec3_t org, int colorIndex, int modelIndex, int modelIndex2, float size)
{
	...
	{
		splatter = (int)(size + (COM_RandomLong(1, 8) + COM_RandomLong(1, 8)));
		...
		for ( i = 0; i < splatter; i++ )
		{
			...
			if ( (pTemp = CL_TempEntAlloc(org, pModel2)) != NULL )
			{
				...
				pTemp->entity.curstate.frame = (float)COM_RandomLong(1, (int)pTemp->frameMax);
	...
	char flags)
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		{
			pTemp->entity.curstate.body = COM_RandomLong(0, (int)pTemp->frameMax);
		}
...
*/
TEMPENTITY*
R_TempModel(const vec3_t pos, const vec3_t dir, const vec3_t angles, float life, int modelIndex, int soundtype)
{
	// alloc a new tempent
	TEMPENTITY* pTemp;
	model_t* pmodel;
...
TEMPENTITY* R_DefaultSprite(const vec3_t pos, int spriteIndex, float framerate)
{
	...
	pTemp->die = (float)cl.time + (float)pTemp->frameMax / framerate;
	pTemp->entity.curstate.frame = 0;
...
void GAME_EXPORT R_SparkShower(const vec3_t pos)
{
	...
	pTemp->entity.curstate.scale = (float)cl.time;
	pTemp->die = (float)cl.time + 0.5f;
}
	...
	int flags)
{
	...
	if ( life )
		...
		pTemp->die = (float)cl.time + (pTemp->frameMax * 0.1f) + 1.0f;
	pTemp->entity.curstate.frame = 0;
...
void GAME_EXPORT R_Sprite_Explode(TEMPENTITY* pTemp, float scale, int flags)
{
	...
	pTemp->entity.curstate.rendermode = noadditive ? kRenderNormal : drawalpha ? kRenderTransAlpha : kRenderTransAdd;
...
void GAME_EXPORT R_Sprite_Smoke(TEMPENTITY* pTemp, float scale)
{
	...
	pTemp->entity.curstate.rendercolor.r = (byte)iColor;
	pTemp->entity.curstate.rendercolor.g = (byte)iColor;
	pTemp->entity.curstate.rendercolor.b = (byte)iColor;
...
*/
void GAME_EXPORT
R_Spray(const vec3_t pos, const vec3_t dir, int modelIndex, int count, int speed, int spread, int rendermode)
{
	TEMPENTITY* pTemp;
	float noise;
	float znoise;
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( rendermode != kRenderGlow )
		{
			...
			if ( pTemp->frameMax > 1 )
			{
				...
				pTemp->die = (float)cl.time + (pTemp->frameMax * 0.1f);
				pTemp->entity.curstate.framerate = 10;
			}
			else
				pTemp->die = (float)cl.time + 0.35f;
		...
		VectorScale(
			...
			COM_RandomFloat((speed * 0.8f), (speed * 1.2f)),
			pTemp->entity.baseline.origin);
	}
}
...
===============
*/
void GAME_EXPORT R_Sprite_Trail(
	int type,
	vec3_t start,
	vec3_t end,
	int modelIndex,
	...
	float speed)
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		pTemp->entity.curstate.frame = (float)COM_RandomLong(0, (int)pTemp->frameMax);
		pTemp->die = (float)cl.time + life + COM_RandomFloat(0.0f, 4.0f);
	}
}
...
==============
*/
void GAME_EXPORT R_Projectile(
	const vec3_t origin,
	const vec3_t velocity,
	int modelIndex,
	int life,
	int owner,
	void (*hitcallback)(TEMPENTITY*, pmtrace_t*))
{
	TEMPENTITY* pTemp;
	model_t* pmodel;
	vec3_t dir;
	...
	pTemp->clientIndex = (short)bound(1, owner, cl.maxclients);
	pTemp->entity.baseline.renderamt = 255;
...
void GAME_EXPORT R_PlayerSprites(int client, int modelIndex, int count, int size)
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( pTemp->frameMax > 1 )
		{
			...
			pTemp->die = (float)cl.time + (pTemp->frameMax * 0.05f);
		}
		else
		{
			pTemp->die = (float)cl.time + 0.35f;
...
void GAME_EXPORT R_FireField(float* org, int radius, int modelIndex, int count, int flags, float life)
{
	...
	{
		VectorCopy(org, pos);
		pos[0] += COM_RandomFloat((float)(-radius), (float)radius);
		pos[1] += COM_RandomFloat((float)(-radius), (float)radius);

		if ( !FBitSet(flags, TEFIRE_FLAG_PLANAR) )
		{
			pos[2] += COM_RandomFloat((float)(-radius), (float)radius);
		}
...
*/
void GAME_EXPORT
R_MultiGunshot(const vec3_t org, const vec3_t dir, const vec3_t noise, int count, int decalCount, int* decalIndices)
{
	pmtrace_t trace;
	vec3_t right, up;
...
void CL_ParseTempEntity(sizebuf_t* msg)
{
	...
	switch ( type )
	{
		...
		case TE_EXPLOSION:
			...
			frameRate = (float)MSG_ReadByte(&buf);
		...
		case TE_DECALHIGH:
			...
			CL_DecalShoot(
				CL_DecalIndex(decalIndex),
				entityIndex,
				modelIndex,
				pos,
				type == TE_BSPDECAL ? FDECAL_PERMANENT : 0);
			break;
		case TE_IMPLOSION:
			pos[0] = MSG_ReadCoord(&buf);
			pos[1] = MSG_ReadCoord(&buf);
		...
		case TE_STREAK_SPLASH:
			...
			R_StreakSplash(pos, pos2, color, count, vel, (int)(-random), (int)random);
			break;
		case TE_DLIGHT:
			dl = CL_AllocDlight(0);
			...
			dl->color.r = (byte)MSG_ReadByte(&buf);
			dl->color.g = (byte)MSG_ReadByte(&buf);
			dl->color.b = (byte)MSG_ReadByte(&buf);
			dl->die = (float)cl.time + (float)(MSG_ReadByte(&buf) * 0.1f);
		...
		case TE_BOX:
			...
			r = (float)MSG_ReadByte(&buf);
			g = (float)MSG_ReadByte(&buf);
			b = (float)MSG_ReadByte(&buf);
			if ( type == TE_LINE )
			{
				R_ParticleLine(pos, pos2, (byte)r, (byte)g, (byte)b, life);
			}
			else
			{
				R_ParticleBox(pos, pos2, (byte)r, (byte)g, (byte)b, life);
			}
			break;
		case TE_LARGEFUNNEL:
			pos[0] = MSG_ReadCoord(&buf);
		...
		case TE_FIZZ:
			entityIndex = MSG_ReadShort(&buf);
			modelIndex = MSG_ReadShort(&buf);
			scale = (float)MSG_ReadByte(&buf);  // same as density
			pEnt = CL_GetEntityByIndex(entityIndex);
			R_FizzEffect(pEnt, modelIndex, (int)scale);
			break;
		case TE_MODEL:
			pos[0] = MSG_ReadCoord(&buf);
			pos[1] = MSG_ReadCoord(&buf);
		...
		case TE_SPRITE_SPRAY:
			...
			if ( type == TE_SPRAY )
			{
				...
				R_Spray(pos, pos2, modelIndex, count, (int)vel, (int)random, flags);
			}
			else
			{
				R_Sprite_Spray(pos, pos2, modelIndex, count, (int)(vel * 2.0f), (int)random);
			}
			break;
		...
		case TE_PROJECTILE:
			...
			pos2[2] = MSG_ReadCoord(&buf);
			modelIndex = MSG_ReadShort(&buf);
			life = (float)MSG_ReadByte(&buf);
			color = MSG_ReadByte(&buf);  // playernum
			R_Projectile(pos, pos2, modelIndex, (int)life, color, NULL);
			break;
		case TE_PLAYERSPRITES:
			...
			count = MSG_ReadByte(&buf);
			random = (float)MSG_ReadByte(&buf);
			R_PlayerSprites(color, modelIndex, count, (int)random);
			break;
		case TE_PARTICLEBURST:
			...
			color = MSG_ReadByte(&buf);
			life = (float)(MSG_ReadByte(&buf) * 0.1f);
			R_ParticleBurst(pos, (int)scale, color, life);
			break;
		case TE_FIREFIELD:
			pos[0] = MSG_ReadCoord(&buf);
			pos[1] = MSG_ReadCoord(&buf);
			...
			flags = MSG_ReadByte(&buf);
			life = (float)(MSG_ReadByte(&buf) * 0.1f);
			R_FireField(pos, (int)scale, modelIndex, count, flags, life);
			break;
...
dlight_t* CL_AllocDlight(int key)
{
	...
	for ( i = 0, dl = cl_dlights; i < MAX_DLIGHTS; i++, dl++ )
	{
		if ( dl->die < (float)cl.time && dl->key == 0 )
		{
			memset(dl, 0, sizeof(*dl));
			dl->key = key;
			return dl;
...
void CL_UpdateFlashlight(cl_entity_t* ent)
{
	...
	dl->color.r = (byte)bound(0, falloff * 255, 255);
	dl->color.g = (byte)bound(0, falloff * 255, 255);
	dl->color.b = (byte)bound(0, falloff * 255, 255);
...
void CL_AddEntityEffects(cl_entity_t* ent)
{
	...
	if ( FBitSet(ent->curstate.effects, EF_BRIGHTLIGHT) )
	{
		...
		if ( ent->player )
			dl->radius = 400;  // don't flickering
		else
			dl->radius = COM_RandomFloat(400, 431);
		VectorCopy(ent->origin, dl->origin);
		dl->die = (float)cl.time + 0.001f;
		dl->origin[2] += 16.0f;
	}
...
void CL_AddModelEffects(cl_entity_t* ent)
{
	...
	if ( FBitSet(ent->model->flags, STUDIO_ROCKET) )
	{
		...
		{
			dl->radius = (float)Q_max(0, ent->curstate.renderamt - 55);
		}
		else
		{
			dl->radius = 200;
...
void CL_TestLights(void)
{
	...
	numLights = (int)bound(1, cl_testlights->value, MAX_DLIGHTS);
	...
	for ( i = 0; i < numLights; i++ )
	{
		...
		dl->radius = (float)Q_max(64, 200 - 5 * numLights);
...
*/
void GAME_EXPORT
CL_FireCustomDecal(int textureIndex, int entityIndex, int modelIndex, float* pos, int flags, float scale)
{
...
int GAME_EXPORT CL_DecalIndex(int id)
{
	...
	if ( cl.decal_index[id] == 0 )
	{
		...
		cl.decal_index[id] = (short)ref.dllFuncs.GL_LoadTexture(host.draw_decals[id].path, NULL, 0, TF_DECAL);
