...
#include "XashDefs/protocol.h"  // get the protocol version
...
static void Con_LoadConsoleFont(int fontNumber, cl_font_t* font)
{
	...
	else
	{
		...
		{
			if ( Q_snprintf(path, sizeof(path), "font%i_%s.fnt", fontNumber, Cvar_VariableString("con_charset")) > 0 )
			{
...
int table_cp1251[64] = {0x0402, 0x0403, 0x201A, 0x0453, 0x201E, 0x2026, 0x2020, 0x2021, 0x20AC, 0x2030, 0x0409,
...
void Con_RunConsole(void)
{
	...
	if ( FBitSet(con_charset->flags, FCVAR_CHANGED) || FBitSet(con_fontscale->flags, FCVAR_CHANGED) ||
		 FBitSet(con_fontnum->flags, FCVAR_CHANGED) || FBitSet(cl_charset->flags, FCVAR_CHANGED) )
	{
