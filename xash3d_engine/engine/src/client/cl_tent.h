...
void R_BulletImpactParticlesForSurface(const vec3_t pos, enum _SurfaceProp surfaceProp);
...
struct tempent_s*
CL_TempEntAllocCustom(const vec3_t org, model_t* model, int high, void (*callback)(struct tempent_s*, float, float));
...
struct beam_s* R_BeamPoints(
	...
	float g,
	float b);
struct beam_s* R_BeamCirclePoints(
	int type,
	vec3_t start,
	vec3_t end,
	int modelIndex,
...
struct beam_s* R_BeamRing(
	...
	float b);
struct beam_s*
R_BeamFollow(int startEnt, int modelIndex, float life, float width, float r, float g, float b, float brightness);
