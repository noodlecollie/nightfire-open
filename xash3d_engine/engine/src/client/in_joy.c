...
static void Joy_ProcessTrigger(const engineAxis_t engineAxis, short value)
{
	...
	else if ( joyaxis[engineAxis].val < trigThreshold && joyaxis[engineAxis].prevval >= trigThreshold )  // we're
																										 // unpressing
																										 // (inverted)
	{
...
static int Joy_GetHatValueForAxis(const engineAxis_t engineAxis)
{
	...
	if ( joyaxis[engineAxis].val > threshold && joyaxis[engineAxis].prevval <= threshold )  // ignore random press
	{
		return positive;
	}
...
static void Joy_ProcessStick(const engineAxis_t engineAxis, short value)
{
	...
	if ( (engineAxis == JOY_AXIS_SIDE || engineAxis == JOY_AXIS_FWD) && (CL_IsInMenu() || CL_IsInConsole()) )
	{
		int val = 0;
...
#if !defined(XASH_SDL)
	*dpitch += joy_pitch->value * (float)joyaxis[JOY_AXIS_PITCH].val / (float)SHRT_MAX *
		host.realframetime;  // abs axis rotate is frametime related
