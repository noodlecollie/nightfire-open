...
*/
void GAME_EXPORT
pfnPlaySound(int ent, float* org, int chan, const char* samp, float vol, float attn, int flags, int pitch)
{
...
*/
void GAME_EXPORT
CL_PlayerTraceExt(float* start, float* end, int traceFlags, int (*pfnIgnore)(physent_t* pe), pmtrace_t* tr)
{
	if ( !tr )
		return;
	*tr = PM_PlayerTraceExt(
...
static void GAME_EXPORT pfnDrawStringLen(const char* str, int* width, int* height)
{
	int flags = FONT_DRAW_HUD | FONT_DRAW_NOLF;
