...
{
	return g_BitStringEndMasks[numBits % BITS_PER_INT];
}
...
public:
	...
	int* GetInts()
	{
		return m_numInts == 1 ? &m_iBitStringStorage : m_pInt;
	}
...
protected:
	...
	CFixedBitStringBase(int numBits)
	{
		Assert(numBits == NUM_BITS);
	}  // doesn't make sense, really. Supported to simplify templates & allow easy replacement of variable

	void ValidateOperand(const CFixedBitStringBase<NUM_BITS>& operand) const
	{
...
inline bool CBitStringT<BASE_OPS>::IsAllClear(void) const
{
	...
	(const_cast<CBitStringT*>(this))->GetInts()[this->GetNumInts() - 1] &=
		~CBitStringT<BASE_OPS>::GetEndMask();  // external semantics of const retained
