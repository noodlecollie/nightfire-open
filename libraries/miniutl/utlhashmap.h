...
#define FOR_EACH_HASHMAP(mapName, iteratorName) \
	for ( int iteratorName = 0; iteratorName < (mapName).MaxElement(); ++iteratorName ) \
...
public:
	...
	struct IterateKeyElemProxyAlias
	{
		...
		ElemType_t& Element(IndexType_t i)
		{
			return *reinterpret_cast<IteratorNode_t*>(&reinterpret_cast<CUtlHashMap*>(this)->m_memNodes.Element(i));
		}
		const ElemType_t& Element(IndexType_t i) const
		{
			return *reinterpret_cast<const IteratorNode_t*>(
				&reinterpret_cast<const CUtlHashMap*>(this)->m_memNodes.Element(i));
		}
		IndexType_t IteratorNext(IndexType_t i) const
		{
			const CUtlHashMap* pSelf = reinterpret_cast<const CUtlHashMap*>(this);
			while ( ++i < pSelf->MaxElement() )
			{
				if ( pSelf->IsValidIndex(i) )
					return i;
			}
			return -1;
		}
...
inline int
CUtlHashMap<K, T, L, H>::InsertUnconstructed(const KeyType_t& key, int* piNodeExistingIfDupe, bool bAllowDupes)
{
	...
	while ( iBucket >= m_nMinRehashedBucket && !m_bitsMigratedBuckets.GetBit(iBucket) )
	{
