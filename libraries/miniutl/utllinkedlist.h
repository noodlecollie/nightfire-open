...
#define FOR_EACH_LL(listName, iteratorName) \
	for ( int iteratorName = (listName).CUtlLinkedListHead(); iteratorName != (listName).InvalidIndex(); \
		  iteratorName = (listName).Next(iteratorName) )

#define FOR_EACH_LL_BACK(listName, iteratorName) \
	for ( int iteratorName = (listName).CUtlLinkedListTail(); iteratorName != (listName).InvalidIndex(); \
		  iteratorName = (listName).Previous(iteratorName) )

#define FOR_EACH_LL_FAST(listName, iteratorName) \
	for ( int iteratorName = 0; iteratorName < (listName).MaxElementIndex(); ++iteratorName ) \
		if ( !(listName).IsValidIndex(iteratorName) ) \
			continue; \
		else
...
public:
	...
	{
		return i == INVALID_LLIST_IDX ? Tail() : Previous(i);
	}
	...
	I CUtlLinkedListHead() const
	{
		return Head();
	}
	I CUtlLinkedListTail() const
	{
		return Tail();
	}
...
{
	return (i < m_TotalElements) && (i >= 0) && ((m_Memory[i].m_Previous != i) || (m_Memory[i].m_Next == i));
}
