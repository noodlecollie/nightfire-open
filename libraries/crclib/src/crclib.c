...
void MD5Final(MD5Digest_t* outDigest, MD5Context_t* ctx)
{
	...
	*p++ = 0x80;
	...
	if ( outDigest )
	{
		memcpy(outDigest->data, ctx->buf, sizeof(outDigest->data));
	}
...
char* MD5_Print(const MD5Digest_t* hash)
{
	...
	if ( hash )
	{
		for ( int i = 0; i < 16; i++ )
		{
			szReturn[i * 2] = Hex2Char(((hash->data[i]) >> 4) & 0x0F);
			szReturn[(i * 2) + 1] = Hex2Char(hash->data[i] & 0x0F);
		}
	}
...
uint32_t COM_HashKey(const char* string, uint32_t hashSize)
{
	...
	for ( char ch = *string++; ch; ch = *string++ )
	{
		hashKey = (hashKey << 5) + hashKey + ((unsigned char)ch & 0xDF);
	}
